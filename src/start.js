...
export default async function handler(req, res) {
  ...
  // PRIORITY ABSOLUTE: render public pages BEFORE anything else to prevent redirect loops
  try {
    const method = (req.method||'GET').toUpperCase();
    const url = String(req.url || req.originalUrl || '');
    if ((method === 'GET' || method === 'HEAD') && (
      /^\/gestor\/primeiroacesso(?:[?#].*)?$/.test(url) ||
      /^\/primeiroacesso(?:[?#].*)?$/.test(url) ||
      /^\/gestor\/login(?:[?#].*)?$/.test(url) ||
      /^\/login(?:[?#].*)?$/.test(url) ||
      // Condomínios (compatibilidade: /condominios-login)
      /^\/condominios\/login(?:[?#].*)?$/.test(url) ||
      /^\/condominios-login(?:[?#].*)?$/.test(url) ||
      /^\/condominios\/primeiroacesso(?:[?#].*)?$/.test(url) ||
      /^\/condominios-primeiroacesso(?:[?#].*)?$/.test(url) ||
      /^\/portal-morador\/login(?:[?#].*)?$/.test(url) ||
      /^\/portal_morador\/login(?:[?#].*)?$/.test(url)
    )) {
      const { default: ejs } = await import('ejs');
      const __filename = fileURLToPath(import.meta.url);
      const __dirname = path.dirname(__filename);
      const ROOT = path.join(__dirname, '..');
      const isPortalLogin = /^\/(portal-morador|portal_morador)\/login/i.test(url);
      const isCompatCondominiosLogin = !isPortalLogin && /^\/condominios-login(?:[?#].*)?$/i.test(url);
      const isCompatCondominiosPA = !isPortalLogin && /^\/condominios-primeiroacesso(?:[?#].*)?$/i.test(url);
      const segMatch = !isPortalLogin ? url.match(/^\/([a-z0-9_-]+)\/(login|primeiroacesso)(?:[?#].*)?$/i) : null;
      const seg = (isCompatCondominiosLogin || isCompatCondominiosPA)
        ? 'condominios'
        : (segMatch ? String(segMatch[1] || '').toLowerCase() : null);
      const isLogin = !isPortalLogin && (
        isCompatCondominiosLogin ||
        /^\/(gestor\/)?login/i.test(url) ||
        (segMatch && String(segMatch[2] || '').toLowerCase() === 'login')
      );
      const isPA = !isPortalLogin && (
        isCompatCondominiosPA ||
        /^\/(gestor\/)?primeiroacesso/i.test(url) ||
        (segMatch && String(segMatch[2] || '').toLowerCase() === 'primeiroacesso')
      );
      const viewFile = isPortalLogin
        ? path.join(ROOT, 'views', 'portal-morador', 'login.ejs')
        : path.join(ROOT, 'views', 'gestor', isLogin ? 'logingestor.ejs' : 'primeiroacesso.ejs');
      const viewsRoot = path.join(ROOT, 'views');
      // Parse query params para passar erro/mensagem
  const queryMatch = url.match(/\?(.+?)(?:#|$)/);
      const queryStr = queryMatch ? queryMatch[1] : '';
      const params = new URLSearchParams(queryStr);
  // Modo de diagnóstico: retorna texto simples para confirmar quem atendeu
  const raw = params.get('raw');
      const erro = params.get('erro') || null;
      const email = params.get('email') || '';
      let mensagem = null;
      if (!isPortalLogin && isPA && erro) {
        // Mensagens para primeiro acesso
        switch (erro) {
      ...
      if (raw === '1' || raw === 'true') {
        const tag = isPortalLogin ? 'portal-login' : (isLogin ? 'login' : 'primeiroacesso');
        res.statusCode = 200;
        res.setHeader('Content-Type', 'text/plain; charset=utf-8');
        res.setHeader('Cache-Control', 'no-store, no-cache, must-revalidate');
        try {
          res.setHeader('X-WDG-Portal-UI-Rev', PORTAL_UI_REV);
          res.setHeader('X-WDG-Portal-Asset-Version', String(getAssetVersion()));
        } catch {}
        try { res.setHeader('X-Start-Direct', tag); } catch {}
        return res.end(`[start] direct ${tag}`);
      }
      let basePath = '/gestor';
      let moduleLabel = 'Gestor';
      if (isPortalLogin) {
        basePath = '/portal-morador';
        moduleLabel = 'Portal do Morador';
      } else if (seg && seg !== 'gestor') {
        basePath = '/' + seg;
        moduleLabel = seg === 'condominios' ? 'Condomínios' : (seg.charAt(0).toUpperCase() + seg.slice(1));
      }
      const locals = isPortalLogin
        ? { basePath, moduleLabel, erro, mensagem: null, email }
        : { basePath, moduleLabel, erro, mensagem };
  const html = await ejs.renderFile(viewFile, locals, { root: viewsRoot });
      res.statusCode = 200;
      ...
      try { res.setHeader('X-Start-Direct', isPortalLogin ? 'portal-login' : (isLogin ? 'login' : 'primeiroacesso')); } catch {}
      return res.end(html);
    }
  } catch(e) {
    console.error('[start] falha render direto:', e.message);
