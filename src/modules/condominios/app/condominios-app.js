  ...
  try {
    ...
    try {
      ...
      if (scope.mailboxId === 'pessoal') {
        ...
        if (uid && mongoose.isValidObjectId(uid)) unitKey = String(uid);
      } else {
        const mb = await loadMailboxOrThrow(scope.mailboxId);
        unitKey = mb?.unidade_id ? String(mb.unidade_id) : '';
      }

      if (unitKey && mongoose.isValidObjectId(unitKey)) {
        const settings = await getOrInitMsgSettingsForUnidade(unitKey);
        const suspendedPersonalGlobal = !!(settings && settings.suspender_caixas_pessoais);
        const suspendedGroupGlobal = !!(settings && settings.suspender_caixas_grupo);
        const legacy = normalizeOwnerKeyList(settings?.pessoais_suspensas || []);
        const portalList = normalizeOwnerKeyList(settings?.pessoais_suspensas_portal || []);
        const colabList = normalizeOwnerKeyList(settings?.pessoais_suspensas_colaborador || []);
        const hasNew = (portalList.length + colabList.length) > 0;
        const suspendedOwnersPortal = new Set(hasNew ? portalList : legacy);
        const suspendedOwnersColab = new Set(hasNew ? colabList : legacy);

        if (scope.mailboxId === 'pessoal') {
          const ownerLower = String(scope.owner || '').trim().toLowerCase();
          const baseEmail = ownerKeyBaseEmailLower(ownerLower);
          const originPortal = String(req.headers['x-wdg-portal'] || '').trim() === '1';
          const suspendedSet = originPortal ? suspendedOwnersPortal : suspendedOwnersColab;
          const baseSuspended = !!(baseEmail && Array.from(suspendedSet).some(v => ownerKeyBaseEmailLower(v) === baseEmail));
          if (suspendedPersonalGlobal || (ownerLower && (suspendedSet.has(ownerLower) || (baseEmail && suspendedSet.has(baseEmail)) || baseSuspended))) {
            return res.status(403).json({ error: 'Caixa pessoal suspensa para este condomínio.' });
          }
        } else {
          if (suspendedGroupGlobal) {
            return res.status(403).json({ error: 'Caixas de grupo estão suspensas para este condomínio.' });
          }
        }
      }
    } catch { /* noop */ }
    ...
    if (scope.mailboxId !== 'pessoal') {
      ...
      if (mailboxIsGroup(mailbox)) {
        const allowPublicPortal = !!(fromPortal && !admin && mailboxIsPublic(mailbox) && !mailboxIsMember(mailbox, ctxUser));
        const perms = mailboxGetUserPerms(mailbox, ctxUser);
        const canRead = allowPublicPortal || admin || mailboxHasPerm(perms, 'administrar') || mailboxHasPerm(perms, 'lerMensagem');
        if (!canRead) return res.status(403).json({ error: 'Sem permissão para ler mensagens desta caixa.' });
      }
    }

    const doc = await CondMsgMessage.findOne({ _id: id, ativo: { $ne: false } });
    if (!doc) return res.status(404).json({ error: 'Mensagem não encontrada' });

    const hasStateAccess = (() => {
      try {
        if (scope.mailboxId !== 'pessoal') return false;
        if (!ownerCandidatesLower.length) return false;
        const states = Array.isArray(doc?.states) ? doc.states : [];
        const st = states.find(s => String(s?.mailbox_id || '').trim() === 'pessoal' && ownerCandidatesLower.includes(String(s?.owner || '').trim().toLowerCase()) && !s?.excluida_em);
        return !!st;
      } catch {
        return false;
      }
    ...
    const deletedForScope = (() => {
      try {
        if (scope.mailboxId !== 'pessoal') return false;
        if (!ownerCandidatesLower.length) return false;
        const states = Array.isArray(doc?.states) ? doc.states : [];
        const scoped = states.filter(s => String(s?.mailbox_id || '').trim() === 'pessoal' && ownerCandidatesLower.includes(String(s?.owner || '').trim().toLowerCase()));
    ...
    const hasNameAccess = (() => {
      try {
        if (!nameFallback) return false;
        const msgUnitId = doc?.unidade_id ? String(doc.unidade_id) : '';
        if (!msgUnitId || msgUnitId !== nameFallback.unidadeId) return false;
        const list = [...(doc?.to || []), ...(doc?.cc || [])];
        return list.some(m => {
          const n = String(m?.nome || m?.name || '').trim();
          return n && nameFallback.nameRx.test(n);
        });
      } catch {
        return false;
      }
    ...
    if (deletedForScope) return res.status(404).json({ error: 'Mensagem não encontrada' });

    const allowed = admin || isScopeSender(doc, scope) || hasScopeAsRecipient(doc, scope) || hasStateAccess || hasNameAccess;
    if (!allowed) return res.status(403).json({ error: 'Acesso negado' });

    // Enriquecimento (best-effort): garante que to/cc tenham display "Nome (Bloco - Andar - Tipo Nº)" ou "Nome (Colaborador)".
    try { await enrichRecipientsDisplayForDocs([doc]); } catch { /* noop */ }

    ensureMessageState(doc, scope);
    const mailboxEmailCache = new Map();
    const emailForSender = await resolveSenderEmailFromMessageDoc(doc, mailboxEmailCache, ctxUser);
    const senderName = emailForSender ? await resolveUserDisplayNameByEmail(emailForSender) : '';
    const item = toMessageDetailItem(doc, scope, { senderName, senderEmail: emailForSender });

    // Thread: retorna a sequência (raiz + respostas/encaminhamentos)
    let threadItems = [];
    const rootId = doc.thread_root_id || doc._id;
    const raw = await CondMsgMessage.find({
      ativo: { $ne: false },
      $or: [
        { _id: rootId },
        { thread_root_id: rootId }
      ]
    }).sort({ createdAt: 1 }).limit(220);

    const rawAllowed = (raw || [])
      .filter(d => {
        try {
          if (admin || isScopeSender(d, scope) || hasScopeAsRecipient(d, scope)) return true;
          if (scope.mailboxId === 'pessoal' && ownerCandidatesLower.length) {
            const states = Array.isArray(d?.states) ? d.states : [];
            const ok = states.some(s => String(s?.mailbox_id || '').trim() === 'pessoal' && ownerCandidatesLower.includes(String(s?.owner || '').trim().toLowerCase()) && !s?.excluida_em);
            if (ok) return true;
          }
          if (nameFallback) {
            const msgUnitId = d?.unidade_id ? String(d.unidade_id) : '';
            if (msgUnitId && msgUnitId === nameFallback.unidadeId) {
    ...
    // Enriquecimento em lote (best-effort) para a thread.
    try { await enrichRecipientsDisplayForDocs(rawAllowed); } catch { /* noop */ }

    threadItems = await Promise.all(rawAllowed
      .map(async (d) => {
        ensureMessageState(d, scope);
        const email = await resolveSenderEmailFromMessageDoc(d, mailboxEmailCache, ctxUser);
        const nm = email ? await resolveUserDisplayNameByEmail(email) : '';
        return toMessageDetailItem(d, scope, { senderName: nm, senderEmail: email });
      }));

    threadItems = (threadItems || []).filter(Boolean);

    return res.json({ ok: true, item, thread: threadItems });
  } catch (e) {
    console.error('[condominios][GET /api/msg/messages/:id] erro:', e);
...
app.get('/api/msg/messages/:id([0-9a-fA-F]{24})/historico-acessos.pdf', async (req, res) => {
  try {
    ...
    if (mongoose.connection.readyState !== 1) {
      ...
      if (mongoose.connection.readyState !== 1) {
        try { res.set('Retry-After', '5'); } catch {}
        return res.status(503).end('DB indisponível');
      }
    }
    ...
    if (scope.mailboxId !== 'pessoal') {
      const mailbox = await loadMailboxOrThrow(scope.mailboxId);
      if (!(await canAccessMsgMailbox(mailbox, ctxUser, req))) return res.status(403).end('Acesso negado');
    }

    const msgDoc = await CondMsgMessage.findOne({ _id: id, ativo: { $ne: false } }).lean();
    if (!msgDoc) return res.status(404).end('Mensagem não encontrada');

    const hasStateAccess = (() => {
      try {
        if (scope.mailboxId !== 'pessoal') return false;
        if (!ownerCandidatesLower.length) return false;
        const states = Array.isArray(msgDoc?.states) ? msgDoc.states : [];
        const st = states.find(s => String(s?.mailbox_id || '').trim() === 'pessoal' && ownerCandidatesLower.includes(String(s?.owner || '').trim().toLowerCase()));
        return !!st;
      } catch {
        return false;
      }
    ...
    const hasNameAccess = (() => {
      try {
        if (!nameFallback) return false;
        const msgUnitId = msgDoc?.unidade_id ? String(msgDoc.unidade_id) : '';
        if (!msgUnitId || msgUnitId !== nameFallback.unidadeId) return false;
        const list = [...(msgDoc?.to || []), ...(msgDoc?.cc || [])];
        return list.some(m => {
          const n = String(m?.nome || m?.name || '').trim();
          return n && nameFallback.nameRx.test(n);
        });
    ...
    const allowed = admin || isScopeSender(msgDoc, scope) || hasScopeAsRecipient(msgDoc, scope) || hasStateAccess || hasNameAccess;
    if (!allowed) return res.status(403).end('Acesso negado');

    const emailForSender = String(msgDoc.from_owner || '').trim().toLowerCase()
      || (String(msgDoc.createdBy || '').includes('@') ? String(msgDoc.createdBy || '').trim().toLowerCase() : '');
    const senderName = emailForSender ? await resolveUserDisplayNameByEmail(emailForSender) : '';
    const item = toMessageDetailItem(msgDoc, scope, { senderName });

    // No Portal do Morador, a unidade exibida deve ser a unidade ativa da sessão do Portal
    // (não a unidade vinculada à mensagem ou a unidade do Gestor no mesmo navegador).
    // (fromPortal já calculado acima)
    const isMasterOrAdmin = userCanScopeAll(ctxUser);

    // Portal: unidade ativa pode vir em unidadeId/unidade_principal_id
    const portalUnidadeId = getUserUnidadeId(ctxUser) ? String(getUserUnidadeId(ctxUser)) : '';

    // Condomínios: para Master/Admin, NÃO pode herdar unidadeId/unidade_principal_id da sessão;
    // só considera vínculo real em unidade_id.
    const condUnidadeId = (() => {
      try {
        const u = ctxUser?.unidade_id;
        const cand = u && typeof u === 'object' ? (u._id || u.id || '') : (u || '');
        const explicit = String(cand || '').trim();
        if (explicit) return explicit;
        if (isMasterOrAdmin) return '';
        return portalUnidadeId;
      } catch { return ''; }
    })();
    ...
    // - Portal do Morador: unidade ativa do Portal (habitação em uso)
    // - Gestão de Condomínio: unidade vinculada ao usuário emitente
    // - Gestor (Master/Admin) sem unidade: usa logo WD Gestor
    let unidadeId = '';
    if (!isMasterOrAdmin) {
      if (fromPortal) {
        unidadeId = portalUnidadeId || msgUnidadeId;
      } else if (condUnidadeId) {
        unidadeId = condUnidadeId;
      } else {
        unidadeId = msgUnidadeId;
      }
    }
    ...
    const formatDateBr = dt => {
      ...
      try {
        return new Intl.DateTimeFormat('pt-BR', { timeZone: PDF_TZ, day: '2-digit', month: '2-digit', year: 'numeric' }).format(d);
      } catch {
        return '';
      }
    };
    const formatTimeBr = dt => {
      ...
      if (!isFinite(d.getTime())) return '';
      try {
        return new Intl.DateTimeFormat('pt-BR', { timeZone: PDF_TZ, hour: '2-digit', minute: '2-digit', hour12: false }).format(d);
      } catch {
        return '';
      }
    };
    const formatDateTimeBr = dt => {
      const d = dt instanceof Date ? dt : new Date(dt);
      if (!isFinite(d.getTime())) return '';
      const a = formatDateBr(d);
      const b = formatTimeBr(d);
      return a && b ? `${a} ${b}` : (a || b);
    };
    ...
    const useWdGestorFallback = isMasterOrAdmin && !masterHeaderUnidadeDoc;

    const unidadeNome = useWdGestorFallback
      ? 'WD Gestor'
      : (stripLeadingCodigo(unidadeDoc?.nome, unidadeDoc?.codigo) || 'Condomínio');
    const unidadeEndereco = useWdGestorFallback
      ? '—'
      : (buildEnderecoHeader(unidadeDoc) || '—');
    const tels = useWdGestorFallback
      ? []
      : [unidadeDoc?.telefoneCelular, unidadeDoc?.telefoneFixo]
      .map(formatTelefoneRel)
      .filter(Boolean);
    const telLine = tels.length ? Array.from(new Set(tels)).join(' / ') : '—';
    const emails = useWdGestorFallback
      ? []
      : [unidadeDoc?.emailPrincipal, unidadeDoc?.emailFiscal]
      .map(v => String(v || '').trim().toLowerCase())
      .filter(Boolean);
    const emailLine = emails.length ? Array.from(new Set(emails)).join(' / ') : '—';
    const cnpjLine = useWdGestorFallback
      ? '—'
      : (formatCnpjRel(unidadeDoc?.cnpj) || '—');

    const wdGestorLogoValue = 'images/logoWDGestor.png';
    const rawLogoValue = String(unidadeDoc?.logo || '').trim();
    const logoValue = useWdGestorFallback
      ? wdGestorLogoValue
      : (rawLogoValue || (isMasterOrAdmin ? wdGestorLogoValue : ''));
    const selfOrigin = (() => {
      try {
        const xfProto = String(req.headers['x-forwarded-proto'] || '').trim();
        const proto = xfProto || String(req.protocol || 'http').trim();
    ...
    // Prefixo do módulo (ex.: '/condominios') para montar URLs internas.
    // Usa req.originalUrl para não depender de suposições ('/condominios') nem de req.baseUrl (que pode ser '').
    const modulePrefix = (() => {
      try {
        const raw = String(req.originalUrl || '').trim();
        const idx = raw.indexOf('/api/msg/messages/');
        const prefix = (idx >= 0) ? raw.slice(0, idx) : String(req.baseUrl || '').trim();
    ...
    const loadLogoBuffer = async (value) => {
      ...
      const fetchToImageBuffer = async (url) => {
        try {
          ...
          const resp = await fetchImpl(url, {
            headers: {
              // Repassa credenciais/contexto para permitir buscar /api/usuarios/foto quando necessário.
              ...(req?.headers?.cookie ? { cookie: String(req.headers.cookie) } : {}),
              ...(req?.headers?.authorization ? { authorization: String(req.headers.authorization) } : {}),
              ...(req?.headers?.['x-wdg-portal'] ? { 'x-wdg-portal': String(req.headers['x-wdg-portal']) } : {})
            }
          });
      ...
      // Preferir resolver como arquivo local quando vier como URL relativa (/images/... /uploads/...)
      // (mais robusto em serverless e evita depender de fetch/host)
      if (s.startsWith('/')) {
        ...
        try {
          ...
          candidates.push(path.resolve(ROOT, 'public', cleaned));
          candidates.push(path.resolve(ROOT, 'uploads', cleaned));
          candidates.push(path.resolve(ROOT, 'public', 'uploads', cleaned));
          candidates.push(path.resolve(ROOT, 'images', cleaned));
      ...
      // URL relativa do próprio sistema (/uploads/... ou /api/usuarios/foto?email=...)
      if (s.startsWith('/') && selfOrigin) {
        const buf = await fetchToImageBuffer(selfOrigin + s);
        if (buf) return buf;
      }
      ...
      // Normaliza caminhos: no banco pode vir com \\ (Windows) e até com drive (C:\\...).
      let cleaned = String(s || '').trim();
      cleaned = cleaned.replace(/\\/g, '/');
      cleaned = cleaned.replace(/^[a-zA-Z]:\/+/, '');
      cleaned = cleaned.replace(/^\/+/, '');
      const candidates = [];
      try {
        // Nunca use caminho absoluto vindo do banco (segurança + compatibilidade no Vercel).
        candidates.push(path.resolve(ROOT, cleaned));
        candidates.push(path.resolve(ROOT, 'public', cleaned));
        candidates.push(path.resolve(ROOT, 'uploads', cleaned));
        candidates.push(path.resolve(ROOT, 'public', 'uploads', cleaned));
        ...
        // Compat: alguns ambientes usam cwd diferente
        candidates.push(path.resolve(process.cwd(), cleaned));
        candidates.push(path.resolve(process.cwd(), 'public', cleaned));
        candidates.push(path.resolve(process.cwd(), 'uploads', cleaned));
        candidates.push(path.resolve(process.cwd(), 'public', 'uploads', cleaned));
    ...
    const resolveAvatarFotoValueByEmail = async (emailRaw) => {
      const email = String(emailRaw || '').trim().toLowerCase();
      if (!email || !email.includes('@')) return '';
      let foto = '';
      let userId = null;
      ...
      try {
        const userDoc = await User.findOne({ email }).select('_id foto fotoUrl foto_url photo avatar avatarUrl avatar_url').lean();
        if (userDoc && userDoc._id) userId = userDoc._id;
        foto = pickFotoValue(userDoc) || foto;
      } catch {}
      if (!foto) {
        try {
    ...
    const resolveAvatarFotoValueById = async (idRaw) => {
      ...
      try {
        const userDoc = await User.findById(id).select('_id foto fotoUrl foto_url photo avatar avatarUrl avatar_url').lean();
        foto = pickFotoValue(userDoc) || foto;
      } catch {}
      if (!foto) {
        try {
          const condDoc = await CondUsuario.findById(id).select('foto fotoUrl foto_url photo avatar avatarUrl avatar_url').lean();
          foto = pickFotoValue(condDoc) || foto;
        } catch {}
      }
      if (!foto) {
        try {
          const funcDoc = await Funcionario.findOne({ usuario_id: id }).select('foto').lean();
          if (funcDoc && funcDoc.foto) foto = String(funcDoc.foto || '').trim();
        } catch {}
      }
      return String(foto || '').trim();
    ...
    const loadAvatarBufferByEmail = async (emailRaw) => {
      const email = String(emailRaw || '').trim().toLowerCase();
      if (!email || !email.includes('@')) return null;
      const key = `email:${email}`;
      if (avatarCache.has(key)) return avatarCache.get(key);

      const fotoValue = await resolveAvatarFotoValueByEmail(email);
      const buf = fotoValue ? await loadLogoBuffer(fotoValue) : null;
      avatarCache.set(key, buf);
      return buf;
    };

    const loadAvatarBufferByFotoUrl = async (fotoUrlRaw) => {
      const s = String(fotoUrlRaw || '').trim();
      if (!s) return null;
      const key = `url:${s}`;
      if (avatarCache.has(key)) return avatarCache.get(key);
      const buf = await loadLogoBuffer(s);
      avatarCache.set(key, buf);
      return buf;
    };

    const drawAvatar = (buf, x, y, size) => {
      const r = Math.max(2, Math.floor(size / 2));
      try {
        doc.save();
        // fundo neutro (caso a imagem tenha transparência ou falhe)
        ...
        if (buf) {
          ...
          const ix = x + (size - w) / 2;
          const iy = y + (size - h) / 2;
          doc.image(img, ix, iy, { width: w, height: h });
        }
        doc.restore();
      } catch {
    ...
    // Agrega acessos por destinatário com regras do Portal:
    // - Caixas públicas: "Nome da caixa (Usuário que acessou)"
    // - Caixa pessoal: "Usuário (Bloco X, Andar, Apartamento N)" ou "Usuário (Colaborador)"
    const members = [...(Array.isArray(msgDoc?.to) ? msgDoc.to : []), ...(Array.isArray(msgDoc?.cc) ? msgDoc.cc : [])];
    const memberByMailboxId = new Map();
    for (const m of members) {
      const mb = String(m?.mailboxId || '').trim();
      if (mb && !memberByMailboxId.has(mb)) memberByMailboxId.set(mb, m);
    }

    const isEmailishLocal = (value) => {
      const s = String(value || '').trim();
      return !!(s && s.includes('@') && s.length >= 5);
    };

    const accessEvents = Array.isArray(item?.acessos) ? item.acessos : [];

    // Pré-carrega nomes de caixas (fallback) e perfis/habitações (para caixa pessoal)
    const mailboxIdsToLoad = new Set();
    const actorEmails = new Set();

    for (const a of accessEvents) {
      const mb = String(a?.mailboxId || a?.mailbox_id || '').trim();
      if (mb && mb !== 'pessoal' && mongoose.isValidObjectId(mb)) mailboxIdsToLoad.add(mb);
      const owner = String(a?.owner || '').trim().toLowerCase();
      const userRaw = String(a?.user || '').trim();
      const email = isEmailishLocal(owner) ? owner : (isEmailishLocal(userRaw) ? userRaw.trim().toLowerCase() : '');
      if (email) actorEmails.add(email);
    }

    const mailboxNameById = new Map();
    if (mailboxIdsToLoad.size) {
      try {
        const mbDocs = await CondMsgMailbox.find({
          _id: { $in: Array.from(mailboxIdsToLoad).map(id => new mongoose.Types.ObjectId(id)) }
        })
          .select('_id nome name')
          .lean();
        (mbDocs || []).forEach(mb => {
          const id = mb?._id ? String(mb._id) : '';
          if (!id) return;
          const nm = String(mb?.nome || mb?.name || '').trim();
          if (nm) mailboxNameById.set(id, nm);
        });
      } catch { /* noop */ }
    }

    // Resolve nome/habitação para emails (somente para caixa pessoal)
    const profileByEmail = new Map();
    const habById = new Map();
    const blocoById = new Map();
    const andarById = new Map();

    if (unidadeId && mongoose.isValidObjectId(unidadeId) && actorEmails.size) {
      try {
        const unitObjId = new mongoose.Types.ObjectId(unidadeId);
        const emailList = Array.from(actorEmails);

        const [moradores, proprietarios] = await Promise.all([
          CondMorador.find({ unidade_id: unitObjId, email: { $in: emailList }, ativo: { $ne: false } })
            .select('email nome habitacao_id')
            .lean(),
          CondProprietario.find({ unidade_id: unitObjId, contato_email: { $in: emailList }, ativo: { $ne: false } })
            .select('_id contato_email nome')
            .lean()
        ]);
        ...
        (moradores || []).forEach(m => {
          ...
          profileByEmail.set(email, { tipo: 'MORADOR', nome, habitacao_id: hid });
          if (hid && mongoose.isValidObjectId(hid)) habitacaoIds.add(hid);
        });

        // Proprietário: tenta inferir uma habitação ativa pela unidade
        const propIds = (proprietarios || []).map(p => (p && p._id ? String(p._id) : null)).filter(Boolean);
        const habByProp = new Map();
        if (propIds.length) {
          const habs = await CondHabitacao.find({
            unidade_id: unitObjId,
            proprietario_id: { $in: propIds.filter(mongoose.isValidObjectId).map(id => new mongoose.Types.ObjectId(id)) },
            ativa: { $ne: false }
          })
            .select('_id proprietario_id bloco_id andar_id numero tipo')
            .sort({ numero: 1 })
            .lean();
          (habs || []).forEach(h => {
            const pid = h?.proprietario_id ? String(h.proprietario_id) : '';
        ...
        (proprietarios || []).forEach(p => {
          const email = String(p?.contato_email || '').trim().toLowerCase();
          if (!email) return;
          if (profileByEmail.has(email)) return;
          const nome = String(p?.nome || '').trim();
          const pid = p?._id ? String(p._id) : '';
          const hab = pid ? (habByProp.get(pid) || null) : null;
          const hid = hab && hab._id ? String(hab._id) : '';
          profileByEmail.set(email, { tipo: 'PROPRIETARIO', nome, habitacao_id: hid });
          if (hid && mongoose.isValidObjectId(hid)) habitacaoIds.add(hid);
        });

        if (habitacaoIds.size) {
          const habDocs = await CondHabitacao.find({ _id: { $in: Array.from(habitacaoIds).map(id => new mongoose.Types.ObjectId(id)) } })
            .select('_id bloco_id andar_id numero tipo')
            .lean();
          const blocoIds = new Set();
          const andarIds = new Set();
          (habDocs || []).forEach(h => {
            ...
            if (!hid) return;
            habById.set(hid, h);
            if (h?.bloco_id && mongoose.isValidObjectId(String(h.bloco_id))) blocoIds.add(String(h.bloco_id));
            if (h?.andar_id && mongoose.isValidObjectId(String(h.andar_id))) andarIds.add(String(h.andar_id));
          });

          const [blocos, andares] = await Promise.all([
            blocoIds.size
              ? CondBloco.find({ _id: { $in: Array.from(blocoIds).map(id => new mongoose.Types.ObjectId(id)) } })
                .select('_id nome codigo')
                .lean()
              : [],
            andarIds.size
              ? CondAndar.find({ _id: { $in: Array.from(andarIds).map(id => new mongoose.Types.ObjectId(id)) } })
                .select('_id nome codigo')
                .lean()
              : []
          ]);
          (blocos || []).forEach(b => { if (b?._id) blocoById.set(String(b._id), b); });
          (andares || []).forEach(a => { if (a?._id) andarById.set(String(a._id), a); });
        }
      } catch { /* noop */ }
    }

    const buildHabitacaoLabelComma = (habDoc, blocoDoc, andarDoc) => {
      if (!habDoc) return '';
      const parts = [];
      if (blocoDoc && (blocoDoc.nome || blocoDoc.codigo)) {
        parts.push(`Bloco ${String(blocoDoc.nome || blocoDoc.codigo).trim()}`);
      ...
      if (andarDoc && (andarDoc.nome || andarDoc.codigo)) {
        // Mantém como está (ex.: "Térreo") sem forçar "Andar"
        parts.push(String(andarDoc.nome || andarDoc.codigo).trim());
      }
      const tipo = String(habDoc.tipo || '').trim() || 'Habitação';
      const numero = String(habDoc.numero || '').trim();
      if (numero) parts.push(`${tipo} ${numero}`);
    ...
    const resolveMailboxDisplayName = (mbId) => {
      const id = String(mbId || '').trim();
      if (!id) return '';
      const m = memberByMailboxId.get(id);
      const fromMember = (m && (String(m?.name || '').trim() || String(m?.nome || '').trim()))
        ? (String(m?.name || m?.nome || '').trim())
        : '';
      return fromMember || mailboxNameById.get(id) || id;
    };

    const resolveActorEmail = (a) => {
      const owner = String(a?.owner || '').trim().toLowerCase();
      const userRaw = String(a?.user || '').trim();
      if (isEmailishLocal(owner)) return owner;
      if (isEmailishLocal(userRaw)) return userRaw.toLowerCase();
      return '';
    };

    const resolveActorBaseName = async (a) => {
      const email = resolveActorEmail(a);
      if (email) {
        const prof = profileByEmail.get(email) || null;
        const profNome = String(prof?.nome || '').trim();
        if (profNome) return profNome;
        const resolved = await resolveUserDisplayNameByEmail(email);
        if (resolved) return resolved;
        return email;
      }
      const userRaw = String(a?.user || '').trim();
      if (userRaw) return userRaw;
      const ownerRaw = String(a?.owner || '').trim();
      return ownerRaw || '—';
    };

    const resolveActorSuffixForPrivate = (a) => {
      const email = resolveActorEmail(a);
      if (email) {
        const prof = profileByEmail.get(email) || null;
        if (prof && (prof.tipo === 'MORADOR' || prof.tipo === 'PROPRIETARIO')) {
          const hid = String(prof.habitacao_id || '').trim();
          const hab = hid ? (habById.get(hid) || null) : null;
          const bloco = hab?.bloco_id ? (blocoById.get(String(hab.bloco_id)) || null) : null;
          const andar = hab?.andar_id ? (andarById.get(String(hab.andar_id)) || null) : null;
          const habLabel = buildHabitacaoLabelComma(hab, bloco, andar);
          return habLabel ? ` (${habLabel})` : ' (Habitação)';
        }
      }
      return ' (Colaborador)';
    };
    ...
    for (const a of accessEvents) {
      ...
      const mb = String(a?.mailboxId || a?.mailbox_id || '').trim() || 'pessoal';
      const owner = String(a?.owner || '').trim().toLowerCase();
      const userRaw = String(a?.user || '').trim();
      const actorKey = (resolveActorEmail(a) || userRaw || owner || '').trim().toLowerCase();
      if (!actorKey) continue;

      const key = (mb && mb !== 'pessoal')
        ? `mb:${mb}::${actorKey}`
        : `u:${actorKey}`;
      ...
      cur.count += 1;
      if (at.getTime() < cur.firstAt.getTime()) cur.firstAt = at;
      if (at.getTime() > cur.lastAt.getTime()) cur.lastAt = at;
      agg.set(key, cur);
    }

    // Monta linhas com rótulos finais
    const rows = [];
    for (const r of Array.from(agg.values())) {
      let destinatario = '—';
      if (r.mailboxId && r.mailboxId !== 'pessoal') {
        const caixa = resolveMailboxDisplayName(r.mailboxId);
        const actorName = await resolveActorBaseName({ owner: r.owner, user: r.userRaw });
        destinatario = actorName ? `${caixa} (${actorName})` : caixa;
      } else {
        const actorName = await resolveActorBaseName({ owner: r.owner, user: r.userRaw });
        const suffix = resolveActorSuffixForPrivate({ owner: r.owner, user: r.userRaw });
        destinatario = `${actorName || '—'}${suffix}`;
      }
      rows.push({
        ...r,
        destinatario
    ...
    rows.sort((a, b) => String(a.destinatario || '').localeCompare(String(b.destinatario || ''), 'pt-BR', { sensitivity: 'base' }));

    res.setHeader('Content-Type', 'application/pdf');
    res.setHeader('Content-Disposition', 'inline; filename="historico-acessos-' + String(item?.protocolo || id) + '.pdf"');
    try {
    ...
    const drawHeaderFooter = (pageNumber, totalPages) => {
      ...
      const emitente = String(ctxUser?.nome || ctxUser?.name || ctxUser?.email || 'Usuário').trim();
      const emitLabel = `Gerado por ${emitente} em ${formatDateBr(now)} às ${formatTimeBr(now)}`;
      ...
      doc.restore();
      try { doc.page.margins = prevMargins; } catch {}
      doc.x = prevX;
      doc.y = prevY;
    };
    ...
    doc.text('CAIXA DE MENSAGEM - HISTÓRICO DE ACESSOS', left, doc.y, { width: contentW, align: 'center' });
    doc.moveDown(0.8);

    // Metadados
    const metaLabel = (k, v) => {
      doc.font('Helvetica-Bold').fontSize(10).fillColor('#0f172a');
      doc.text(`${k}: `, left, doc.y, { continued: true });
      doc.font('Helvetica').fontSize(10).fillColor('#0f172a');
      doc.text(String(v || '—'));
      doc.moveDown(0.35);
    };

    const paraList = [...(Array.isArray(item?.to) ? item.to : [])]
      .map(m => String(m?.display || m?.nome || m?.name || m?.email || m?.mailboxName || m?.mailboxId || '').trim())
      .filter(Boolean);
    const deValue = String(item?.from?.display || item?.from?.nome || item?.from?.name || item?.from?.email || item?.from?.mailboxName || item?.from?.mailboxId || '').trim() || '—';

    metaLabel('Protocolo', String(item?.protocolo || '').trim() || '—');
    metaLabel('Data', formatDateTimeBr(item?.createdAt) || '—');
    metaLabel('Assunto', String(item?.assunto || '').trim() || '—');
    metaLabel('De', deValue);
    metaLabel('Para', paraList.length ? paraList.join(' ; ') : '—');

    doc.moveDown(0.6);
    doc.font('Helvetica-Bold').fontSize(12).fillColor('#0f172a');
    doc.text('Acessos', left, doc.y, { width: contentW, align: 'center' });
    doc.moveDown(0.6);

    // Tabela
    const colDestW = Math.round(contentW * 0.40);
    const colQtdW = Math.round(contentW * 0.14);
    const colPrimW = Math.round(contentW * 0.23);
    const colUltW = contentW - colDestW - colQtdW - colPrimW;
    const rowH = 22;
    const tableX = left;

    const drawRowBox = (y, isHeader) => {
      try {
        doc.save();
        doc.rect(tableX, y, contentW, rowH)
          .lineWidth(0.6)
          .strokeColor('#e2e8f0')
          .stroke();
        if (isHeader) {
    ...
    const drawTableHeader = () => {
      ...
      const padY = y + 6;
      doc.text('Destinatário', tableX + 6, padY, { width: colDestW - 12, align: 'center', lineBreak: false, ellipsis: true });
      doc.text('Quantidade', tableX + colDestW + 6, padY, { width: colQtdW - 12, align: 'center', lineBreak: false, ellipsis: true });
      doc.text('Primeiro acesso', tableX + colDestW + colQtdW + 6, padY, { width: colPrimW - 12, align: 'center', lineBreak: false, ellipsis: true });
      doc.text('Último acesso', tableX + colDestW + colQtdW + colPrimW + 6, padY, { width: colUltW - 12, align: 'center', lineBreak: false, ellipsis: true });
      doc.y = y + rowH;
    };
    ...
    if (!rows.length) {
      ...
      doc.text('Sem histórico de acessos.', left, doc.y + 6, { width: contentW, align: 'center' });
      doc.moveDown(1.2);
    } else {
      for (const r of rows) {
        ensureSpace(rowH + 6);
        // Se mudou de página, reimprime cabeçalho
        try {
          const bottomLimit = doc.page.height - doc.page.margins.bottom;
        ...
        drawRowBox(y, false);
        const padY = y + 6;
        const dest = String(r?.destinatario || '—');
        const qtd = String(r?.count || 0);
        const primeiro = formatDateTimeBr(r?.firstAt) || '—';
        const ultimo = formatDateTimeBr(r?.lastAt) || '—';

        doc.fillColor('#0f172a');
        doc.font('Helvetica').fontSize(10);
        doc.text(dest, tableX + 6, padY, { width: colDestW - 12, align: 'left', lineBreak: false, ellipsis: true });
        doc.text(qtd, tableX + colDestW + 6, padY, { width: colQtdW - 12, align: 'center', lineBreak: false, ellipsis: true });
        doc.text(primeiro, tableX + colDestW + colQtdW + 6, padY, { width: colPrimW - 12, align: 'center', lineBreak: false, ellipsis: true });
        doc.text(ultimo, tableX + colDestW + colQtdW + colPrimW + 6, padY, { width: colUltW - 12, align: 'center', lineBreak: false, ellipsis: true });
    ...
    for (let i = 0; i < total; i++) {
      ...
      drawHeaderFooter(i + 1, total);
    }

    doc.end();
  } catch (e) {
    console.error('[condominios][GET /api/msg/messages/:id/historico-acessos.pdf] erro:', e);
    try { return res.status(500).end('Falha ao gerar PDF'); } catch { return res.end(); }
  }
});

// API: impressão de mensagem em PDF (mesmo cabeçalho/rodapé do histórico)
app.get('/api/msg/messages/:id([0-9a-fA-F]{24})/imprimir.pdf', async (req, res) => {
  try {
    let ctxUser = getCtxUser(req);
    if (!ctxUser) return res.status(401).end('Não autenticado');
    ...
    const msgDoc = await CondMsgMessage.findOne({ _id: id, ativo: { $ne: false } });
    if (!msgDoc) return res.status(404).end('Mensagem não encontrada');
    ...
    const order = String(req.query?.order || '').trim().toLowerCase() === 'reverse' ? 'reverse' : 'normal';

    const mailboxEmailCache = new Map();
    const baseSenderEmail = await resolveSenderEmailFromMessageDoc(msgDoc, mailboxEmailCache, ctxUser);
    try {
      res.set('X-WDG-PDF-TZ', 'America/Sao_Paulo');
      res.set('X-WDG-PDF-FromPortal', String(req?.headers?.['x-wdg-portal'] || '').trim() === '1' ? '1' : '0');
      res.set('X-WDG-PDF-SenderEmail', String(baseSenderEmail || '').trim().toLowerCase());
    } catch { /* noop */ }

    // Thread: raiz + respostas/encaminhamentos
    const rootId = msgDoc.thread_root_id || msgDoc._id;
    ...
    const canSeeDoc = (d) => {
      try {
        ...
        if (scope.mailboxId === 'pessoal' && ownerCandidatesLower.length) {
          const states = Array.isArray(d?.states) ? d.states : [];
          const ok = states.some(s => String(s?.mailbox_id || '').trim() === 'pessoal' && ownerCandidatesLower.includes(String(s?.owner || '').trim().toLowerCase()));
          if (ok) return true;
        }
        if (nameFallback) {
          const msgUnitId = d?.unidade_id ? String(d.unidade_id) : '';
    ...
    for (const d of (Array.isArray(rawThread) ? rawThread : [])) {
      try {
        if (!canSeeDoc(d)) continue;
        ensureMessageState(d, scope);
        const email = await resolveSenderEmailFromMessageDoc(d, mailboxEmailCache, ctxUser);
        const nm = email ? await resolveUserDisplayNameByEmail(email) : '';
        const it = toMessageDetailItem(d, scope, { senderName: nm, senderEmail: email });
        if (it) threadItems.push(it);
      } catch { /* noop */ }
    }

    const baseItem = (() => {
      try {
        // best-effort: inclui senderEmail para permitir avatar correto no PDF
        return toMessageDetailItem(msgDoc, scope, { senderName: '', senderEmail: baseSenderEmail });
      } catch { return null; }
    })();
    ...
    const loadLogoBuffer = async (value) => {
      ...
      const normalizeRasterBuffer = async (buf) => {
        try {
      ...
      if (/^data:image\/(png|jpe?g|webp);base64,/i.test(s)) {
        try {
          ...
          return await normalizeRasterBuffer(buf);
        } catch { return null; }
      }
      const fetchToImageBuffer = async (url) => {
        try {
      ...
      if (s.startsWith('/')) {
        ...
        if (foundPath) {
          try {
            ...
            if (ext === '.webp') {
              const sharpMod = (await import('sharp')).default;
              const buf = await sharpMod(foundPath).png().toBuffer();
              return await normalizeRasterBuffer(buf);
            }
            const buf = await fs.promises.readFile(foundPath);
            return await normalizeRasterBuffer(buf);
          } catch { return null; }
        }
      }

      // URL relativa do próprio sistema (/images/... /uploads/... /condominios/api/...)
      if (s.startsWith('/') && selfOrigin) {
        const buf = await fetchToImageBuffer(selfOrigin + s);
        if (buf) return buf;
      }
      ...
      try {
        ...
        return await normalizeRasterBuffer(raw);
      } catch { return null; }
    };
    ...
    res.setHeader('Content-Type', 'application/pdf');
    res.setHeader('Content-Disposition', 'inline; filename="mensagem-' + String(baseItem?.protocolo || id) + '.pdf"');
    try {
      res.setHeader('Cache-Control', 'no-store, no-cache, must-revalidate, proxy-revalidate');
      res.setHeader('Pragma', 'no-cache');
      res.setHeader('Expires', '0');
    ...
    const formatBytesRel = (n) => {
      const v = Number(n) || 0;
      if (!v) return '';
      const kb = 1024;
      const mb = kb * 1024;
      if (v >= mb) return `${(v / mb).toFixed(1).replace(/\.0$/, '')} MB`;
      if (v >= kb) return `${Math.round(v / kb)} KB`;
      return `${v} B`;
    };

    const main = baseItem || (threadItems.length ? threadItems[0] : null);
    ...
    try {
      ...
      doc.text('CAIXA DE MENSAGENS', left, doc.y, { width: contentW, align: 'center' });
      doc.moveDown(0.6);
    } catch { /* noop */ }

    // Título (assunto) — como na página, antes dos cards.
    try {
      const subjectTitle = String(main?.assunto || '').trim();
      if (subjectTitle) {
        ensureSpace(42);
        doc.fillColor('#0f172a');
        doc.font('Helvetica-Bold').fontSize(16);
        doc.text(subjectTitle, left, doc.y, { width: contentW, align: 'left' });
        doc.moveDown(0.4);
        doc.moveTo(left, doc.y).lineTo(doc.page.width - right, doc.y).lineWidth(0.6).strokeColor('#e2e8f0').stroke();
        doc.moveDown(0.8);
      }
    } catch { /* noop */ }

    // Thread (cada mensagem no formato solicitado)
    const threadToPrint = order === 'reverse' ? [...threadItems].reverse() : threadItems;
    const placeholderAvatarBuffer = await loadLogoBuffer('images/usuario.png');
    const mailboxAvatarBuffer = await loadLogoBuffer('images/perfil.png');

    const mailboxPublicCache = new Map();
    const isMailboxPublicById = async (mailboxIdRaw) => {
      const mailboxId = String(mailboxIdRaw || '').trim();
      if (!mailboxId || mailboxId === 'pessoal') return false;
      if (!mongoose.isValidObjectId(mailboxId)) return false;
      if (mailboxPublicCache.has(mailboxId)) return !!mailboxPublicCache.get(mailboxId);
      try {
        const mb = await CondMsgMailbox.findById(mailboxId)
          .select('ativo type link_type public publica isPublic visivel_publico visivelPublico operators')
          .lean();
        const linkType = String(mb?.link_type || '').trim().toLowerCase();
        // Qualquer caixa vinculada (link_type preenchido) representa uma entidade (habitação/unidade/etc)
        // e não deve herdar avatar do createdBy.
        const isLinkedMailbox = !!linkType;
        const isPub = !!(mb && mb.ativo !== false && (isLinkedMailbox || mailboxIsPublic(mb)));
        mailboxPublicCache.set(mailboxId, isPub);
        return isPub;
      } catch {
        mailboxPublicCache.set(mailboxId, false);
        return false;
      }
    };

    const mailboxOwnerEmailCache = new Map();
    const resolveMailboxOwnerEmail = async (mailboxIdRaw) => {
      const mailboxId = String(mailboxIdRaw || '').trim();
      if (!mailboxId || mailboxId === 'pessoal') return '';
      if (!mongoose.isValidObjectId(mailboxId)) return '';
      if (mailboxOwnerEmailCache.has(mailboxId)) return String(mailboxOwnerEmailCache.get(mailboxId) || '');
      try {
        const createdByRaw = String(await resolveMailboxCreatedByEmail(mailboxId, mailboxEmailCache) || '').trim();
        if (isEmailish(createdByRaw)) {
          const em = createdByRaw.trim().toLowerCase();
          mailboxOwnerEmailCache.set(mailboxId, em);
          return em;
        }
        // Alguns legados gravam nome no createdBy; tenta mapear nome -> e-mail
        if (createdByRaw && !createdByRaw.includes('@')) {
          const mapped = await resolveUserEmailByDisplayName(createdByRaw, mailboxEmailCache);
          const em = isEmailish(mapped) ? String(mapped).trim().toLowerCase() : '';
          mailboxOwnerEmailCache.set(mailboxId, em);
          return em;
        }
        mailboxOwnerEmailCache.set(mailboxId, '');
        return '';
      } catch {
        mailboxOwnerEmailCache.set(mailboxId, '');
        return '';
    ...
    let _avatarDebugSent = false;
    const _wantPdfDebugHeaders = String(req.query?.debug || '').trim() === '1';
    let _pdfTimeDebugSent = false;
    for (const m of (Array.isArray(threadToPrint) ? threadToPrint : [])) {
      try {
        const caixaLabel = String(m?.from?.display || '').trim() || '—';
        const when = formatDateTimeBr(m?.createdAt) || '—';
        const proto = String(m?.protocolo || '').trim();
        const assunto = String(m?.assunto || main?.assunto || '').trim() || '—';
        const paraList = [...(Array.isArray(m?.to) ? m.to : [])]
          .map(mm => String(mm?.display || mm?.nome || mm?.name || mm?.email || mm?.mailboxName || mm?.mailboxId || '').trim())
          .filter(Boolean);
        const paraValue = paraList.length ? paraList.join(' ; ') : '—';

        const ccList = [...(Array.isArray(m?.cc) ? m.cc : [])]
          .map(mm => String(mm?.display || mm?.nome || mm?.name || mm?.email || mm?.mailboxName || mm?.mailboxId || '').trim())
          .filter(Boolean);
        const ccValue = ccList.length ? ccList.join(' ; ') : '';

        const body = String(m?.bodyText || '').trim() || stripHtmlSimple(m?.bodyHtml || '');
        const bodyTxt = body || '—';

        // Foto do remetente: deve refletir a caixa remetente.
        // Importante: quando o remetente é uma caixa pública, NÃO deve aparecer a foto do usuário que está imprimindo
        // (nem do criador/operador da caixa). Caixas públicas não têm avatar de usuário.
        const senderMailboxId = String(m?.from?.mailboxId || '').trim();
        const senderMailboxName = String(m?.from?.mailboxName || '').trim();
        const senderDisplay = String(m?.from?.display || '').trim();

        // Mensagens legadas/inconsistentes podem vir sem from_mailbox_id.
        // Atenção: não dá pra inferir "caixa" só pelo nome (ex.: 'Master User' != 'pessoal').
        // Regra: só tratar como caixa quando:
        // - mailboxId existe e não é 'pessoal', OU
        // - mailboxId está vazio, mas o display parece ser uma caixa (ex.: display != createdBy) ou contém keywords.
        const fromEmailExplicit = String(m?.from?.email || '').trim();
        const fromOwnerLabel = String(m?.from?.owner || '').trim();
        const fromCreatedByLabel = String(m?.from?.createdBy || '').trim();

        const normalizeLabel = (s) => String(s || '').trim().toLowerCase().replace(/\s+/g, ' ');
        const displayNorm = normalizeLabel(senderDisplay || senderMailboxName);
        const createdByNorm = normalizeLabel(fromCreatedByLabel);

        const displayLooksMailboxByKeywords = (() => {
          const s = displayNorm;
          if (!s) return false;
          // Heurística leve para o domínio (evita confundir nomes de pessoas)
          return /\b(condom[ií]nio|condominio|caixa|grupo|unidade|moradas)\b/i.test(s);
        })();

        // Importante: em dados legados, from.email pode vir vazio e o e-mail/id ficar em from.createdBy/from.owner.
        // Não trate como caixa só porque display != createdBy quando createdBy é e-mail (isso é esperado).
        const senderHasAnyUserKey = (() => {
          try {
            const candidates = [fromEmailExplicit, fromOwnerLabel, fromCreatedByLabel]
              .map(v => String(v || '').trim())
              .filter(Boolean);
            for (const c of candidates) {
              if (isEmailish(c)) return true;
              if (mongoose.isValidObjectId(c)) return true;
            }
            return false;
          } catch { return false; }
        })();

        const displayLooksMailboxByLabelMismatch = (() => {
          // Só considere mismatch como indício de caixa quando createdBy parece ser um rótulo/nome,
          // não um identificador de usuário (e-mail/id).
          if (!displayNorm || !createdByNorm) return false;
          if (createdByNorm.includes('@')) return false;
          if (mongoose.isValidObjectId(fromCreatedByLabel)) return false;
          return displayNorm !== createdByNorm;
        })();

        const senderIsMailbox = (() => {
          if (senderMailboxId) return senderMailboxId !== 'pessoal';
          // Sem mailboxId: se houver evidência de usuário (e-mail/id), trate como pessoal.
          if (senderHasAnyUserKey) return false;
          // Caso contrário, use heurísticas leves do domínio.
          if (displayLooksMailboxByKeywords) return true;
          if (displayLooksMailboxByLabelMismatch) return true;
          return false;
        })();

        const senderIsMailboxDbId = !!(senderIsMailbox && senderMailboxId && senderMailboxId !== 'pessoal' && mongoose.isValidObjectId(senderMailboxId));

        const fromOwnerRaw = fromOwnerLabel;
        const fromCreatedByRaw = String(m?.from?.createdBy || '').trim();

        const pickFromKey = () => {
          const candidates = [fromEmailExplicit, fromOwnerRaw, fromCreatedByRaw]
            .map(v => String(v || '').trim())
            .filter(Boolean);
          // Preferir e-mail quando existir; senão aceitar ObjectId (legado)
          for (const c of candidates) if (c.includes('@')) return c.trim().toLowerCase();
          for (const c of candidates) if (mongoose.isValidObjectId(c)) return c;
          return '';
        };
        const fromKey = pickFromKey();

        const wantPortalCompat = String(req.headers?.['x-wdg-portal'] || '').trim() === '1';
        const portalPrefix = '/portal-morador';

        const buildPhotoUrl = (bp, key) => {
          const k = String(key || '').trim();
          const base = String(bp || '').trim();
          if (!k) return '';
          if (k.includes('@')) return `${base}/api/usuarios/foto?email=${encodeURIComponent(k.toLowerCase())}`;
          if (mongoose.isValidObjectId(k)) return `${base}/api/usuarios/foto?id=${encodeURIComponent(k)}`;
          return '';
        };

        const buildPhotoUrlCandidates = (key) => {
          const bases = [];
          const mPrefix = String(modulePrefix || '').trim();
          if (mPrefix) bases.push(mPrefix);
          else bases.push('');
          if (wantPortalCompat) bases.unshift(portalPrefix);
          return Array.from(new Set(bases.map(s => String(s || '').trim()))).filter(v => typeof v === 'string');
        };

        const pickFotoValueFromUserObj = (u) => {
        ...
        // Se o remetente é uma caixa (não-pessoal), nunca usar foto de usuário (owner/createdBy/email)
        // pois isso vaza a foto de quem criou/imprimiu. Para caixas públicas: sem imagem.
        if (senderIsMailbox) {
          const isPub = senderIsMailboxDbId ? await isMailboxPublicById(senderMailboxId) : true;
          if (isPub) {
            avatarBuffer = mailboxAvatarBuffer;
            avatarStage = 'mailbox-public-icon';
          } else {
            // Caixa privada: usar foto do usuário dono da caixa (createdBy)
            const ownerEmail = senderIsMailboxDbId ? await resolveMailboxOwnerEmail(senderMailboxId) : '';
            if (ownerEmail) {
              try {
                const fotoValue = await resolveAvatarFotoValueByEmail(ownerEmail);
                if (fotoValue) {
                  avatarBuffer = await loadLogoBuffer(fotoValue);
                  if (avatarBuffer) avatarStage = 'mailbox-owner-db-email';
                }
              } catch { /* noop */ }
              if (!avatarBuffer) {
                try {
                  for (const bp of buildPhotoUrlCandidates(ownerEmail)) {
                    const url = buildPhotoUrl(bp || '', ownerEmail);
                    if (!url) continue;
                    avatarBuffer = await loadLogoBuffer(url);
                    if (avatarBuffer) {
                      avatarStage = (bp === portalPrefix) ? 'mailbox-owner-api-portal' : 'mailbox-owner-api';
                      break;
                    }
                  }
                } catch { /* noop */ }
            ...
            if (!avatarBuffer) {
              avatarBuffer = mailboxAvatarBuffer;
              avatarStage = ownerEmail ? 'mailbox-owner-fallback-icon' : 'mailbox-private-icon';
            }
          }
        } else {
          // 0) Se o remetente é o próprio usuário autenticado, tente usar a foto já presente no ctxUser
          // (evita depender de fetch interno e costuma ser o que a UI usa para o avatar).
          try {
            const ctxEmail = String(ctxUser?.email || ctxUser?.userEmail || '').trim().toLowerCase();
            if (!avatarBuffer && ctxEmail && fromKey && String(fromKey).includes('@') && String(fromKey).trim().toLowerCase() === ctxEmail) {
              const fotoFromCtx = pickFotoValueFromUserObj(ctxUser);
              if (fotoFromCtx) {
                avatarBuffer = await loadLogoBuffer(fotoFromCtx);
                if (avatarBuffer) avatarStage = 'ctx-user';
              }
            }
          } catch { /* noop */ }

          // 1) Se existir fotoUrl explícita (raramente vem no from), tenta primeiro
          try {
            avatarBuffer = await loadAvatarBufferByFotoUrl(m?.from?.fotoUrl);
            if (avatarBuffer) avatarStage = 'fotoUrl';
          } catch { /* noop */ }

          // 2) Preferir resolver direto do banco (evita depender de fetch interno via proxy do Portal)
          if (!avatarBuffer && fromKey) {
            try {
              if (String(fromKey).includes('@')) {
                const fotoValue = await resolveAvatarFotoValueByEmail(String(fromKey).trim().toLowerCase());
                if (fotoValue) {
                  avatarBuffer = await loadLogoBuffer(fotoValue);
                  if (avatarBuffer) avatarStage = 'db-email';
                }
              } else if (mongoose.isValidObjectId(String(fromKey))) {
                const fotoValue = await resolveAvatarFotoValueById(String(fromKey));
                if (fotoValue) {
                  avatarBuffer = await loadLogoBuffer(fotoValue);
                  if (avatarBuffer) avatarStage = 'db-id';
                }
              }
            } catch { /* noop */ }
          }

          // 3) Fallback compat (igual a UI): /api/usuarios/foto?email=... ou ?id=...
          if (!avatarBuffer && fromKey) {
            try {
              for (const bp of buildPhotoUrlCandidates(fromKey)) {
                const url = buildPhotoUrl(bp || '', fromKey);
                if (!url) continue;
                avatarBuffer = await loadLogoBuffer(url);
                if (avatarBuffer) {
                  avatarStage = (bp === portalPrefix) ? 'api-portal' : 'api';
                  break;
                }
              }
            } catch { /* noop */ }
          ...
          // 4) Fallback específico: se o remetente é o próprio usuário autenticado,
          // use o endpoint canônico do Gestor (/api/usuario/foto) que aproveita a sessão.
          // Isso evita placeholder quando o DB não tem a foto (mas a sessão tem).
          if (!avatarBuffer && fromKey) {
            try {
              const ctxEmail = String(ctxUser?.email || ctxUser?.userEmail || '').trim().toLowerCase();
              if (ctxEmail && String(fromKey).includes('@') && String(fromKey).trim().toLowerCase() === ctxEmail) {
                const bp = modulePrefix || '';
                const selfUrl = `${bp}/api/usuario/foto`;
                avatarBuffer = await loadLogoBuffer(selfUrl);
                if (avatarBuffer) avatarStage = 'api-self';
              }
            } catch { /* noop */ }
          }

          if (!avatarBuffer) {
            avatarBuffer = placeholderAvatarBuffer;
            if (!avatarStage) avatarStage = 'placeholder';
          }
        }

        if (!_avatarDebugSent) {
          _avatarDebugSent = true;
          try {
            res.set('X-WDG-PDF-Avatar-Stage', avatarStage || '');
            res.set('X-WDG-PDF-Avatar-FromKey', fromKey ? (String(fromKey).includes('@') ? 'email' : (mongoose.isValidObjectId(String(fromKey)) ? 'id' : 'other')) : 'none');
            res.set('X-WDG-PDF-Avatar-HasFromEmail', fromEmailExplicit ? '1' : '0');
            res.set('X-WDG-PDF-Avatar-HasFromOwner', fromOwnerRaw ? '1' : '0');
            res.set('X-WDG-PDF-Avatar-HasCreatedBy', fromCreatedByRaw ? '1' : '0');
          } catch { /* noop */ }
        }

        if (_wantPdfDebugHeaders && !_pdfTimeDebugSent) {
          _pdfTimeDebugSent = true;
          try {
            const dt = (m?.createdAt instanceof Date) ? m.createdAt : (m?.createdAt ? new Date(m.createdAt) : null);
            res.set('X-WDG-PDF-FirstWhen', String(when || ''));
            res.set('X-WDG-PDF-FirstCreatedAt-ISO', dt && isFinite(dt.getTime()) ? dt.toISOString() : '');
            res.set('X-WDG-PDF-FirstCreatedAt-Epoch', dt && isFinite(dt.getTime()) ? String(dt.getTime()) : '');
          } catch { /* noop */ }
        }
        ...
        const avatarX = cardX + cardPadX;
        const avatarY = cardY + cardPadY;
        const textX = avatarX + avatarSize + 10;
        const textW = innerW - (avatarSize + 10);
        const rightColX = cardX + cardPadX;
        ...
        const anexosLines = anexos
          .map(a => {
            ...
            const tam = formatBytesRel(a?.tamanho);
            const right = [mime, tam].filter(Boolean).join(' - ');
            return nome ? (right ? `${nome} — ${right}` : nome) : '';
          })
          .filter(Boolean);

        // Pré-cálculo de altura para quebra de página (layout como na UI: sem linha 'Assunto' dentro do card)
        doc.font('Helvetica').fontSize(9.5);
        const paraH = Math.max(12, doc.heightOfString(`Para: ${paraValue}`, { width: textW, align: 'left' }));
        const ccH = ccValue ? Math.max(12, doc.heightOfString(`Cópia: ${ccValue}`, { width: textW, align: 'left' })) : 0;

        doc.font('Helvetica').fontSize(10);
        const bodyH = Math.max(14, doc.heightOfString(bodyTxt, { width: textW, align: 'left' }));

        let anexosH = 0;
        if (anexosLines.length) {
          anexosH += 12; // título
          doc.font('Helvetica').fontSize(9.5);
          for (const line of anexosLines) {
            anexosH += Math.max(11, doc.heightOfString(line, { width: textW, align: 'left' }));
          }
          anexosH += 6;
        }

        const headerTopH = Math.max(avatarSize, 16 + paraH + (ccH ? (ccH + 2) : 0) + 2);
        const sepGapTop = 8;
        const sepGapBottom = 10;
        const sepLineH = 1;
        const cardH = cardPadY + headerTopH + sepGapTop + sepLineH + sepGapBottom + bodyH + anexosH + cardPadY;
        ...
        try {
          doc.save();
          doc.roundedRect(cardX, startY, cardW, cardH, 8)
            .lineWidth(0.7)
            .strokeColor('#e2e8f0')
            .stroke();
          doc.restore();
        ...
        // Avatar circular
        try {
          const r = Math.max(2, Math.floor(avatarSize / 2));
          doc.save();
          doc.circle(avatarX + r, startY + cardPadY + r, r).fillColor('#e2e8f0').fill();
          doc.circle(avatarX + r, startY + cardPadY + r, r).clip();
          if (avatarBuffer) {
            const img = doc.openImage(avatarBuffer);
            const ratio = Math.max(avatarSize / img.width, avatarSize / img.height);
            const w = img.width * ratio;
            const h = img.height * ratio;
            const ix = avatarX + (avatarSize - w) / 2;
            const iy = (startY + cardPadY) + (avatarSize - h) / 2;
            doc.image(img, ix, iy, { width: w, height: h });
          }
          doc.restore();
        } catch {
        ...
        // Topo: remetente (esq) | protocolo/data (dir)
        doc.fillColor('#0f172a');
        doc.font('Helvetica-Bold').fontSize(11);
        doc.text(caixaLabel, textX, startY + cardPadY, { width: textW * 0.68, align: 'left', lineBreak: false, ellipsis: true });

        // Direita: Protocolo (topo) e Data/hora (abaixo)
        doc.font('Helvetica').fontSize(9.5).fillColor('#334155');
        doc.text(`Protocolo: ${proto || '—'}`, rightColX, startY + cardPadY + 1, { width: rightColW, align: 'right', lineBreak: false, ellipsis: true });
        doc.text(when, rightColX, startY + cardPadY + 14, { width: rightColW, align: 'right', lineBreak: false, ellipsis: true });

        // Subheader: Para / Cópia (como na página)
        let y = startY + cardPadY + 18;
        doc.font('Helvetica').fontSize(9.5).fillColor('#64748b');
        doc.text(`Para: ${paraValue}`, textX, y, { width: textW, align: 'left' });
        y = doc.y + 1;
        if (ccValue) {
          doc.text(`Cópia: ${ccValue}`, textX, y, { width: textW, align: 'left' });
          y = doc.y + 1;
        }

        if (_wantPdfDebugHeaders) {
          try {
            doc.font('Helvetica').fontSize(7).fillColor('#94a3b8');
            doc.text(`avatarStage=${String(avatarStage || '')}`, textX, y, { width: textW, align: 'left', lineBreak: false, ellipsis: true });
            y = doc.y + 1;
          } catch { /* noop */ }
        }

        // Separador dentro do card
        const sepY = y + 6;
        try {
          doc.save();
          doc.moveTo(cardX + cardPadX, sepY)
            .lineTo(cardX + cardW - cardPadX, sepY)
            .lineWidth(0.6)
            .strokeColor('#e2e8f0')
            .stroke();
          doc.restore();
        ...
        y = sepY + 10;
        doc.font('Helvetica').fontSize(10).fillColor('#0f172a');
        doc.text(bodyTxt, textX, y, { width: textW, align: 'left' });
        y = doc.y + 10;

        // Anexos (como na página)
        if (anexosLines.length) {
          doc.font('Helvetica-Bold').fontSize(10).fillColor('#0f172a');
          doc.text('Anexos', textX, y, { width: textW, align: 'left' });
          y = doc.y + 3;
          doc.font('Helvetica').fontSize(9.5).fillColor('#334155');
          for (const line of anexosLines) {
        ...
        // Garante avanço mínimo até o fim do card e separação entre cards
        doc.y = Math.max(y, startY + cardH) + 10;
      } catch (innerErr) {
        const errMsg = String(innerErr?.message || innerErr || '').trim();
        const errStack = String(innerErr?.stack || '').trim();
        const errStackHead = errStack ? errStack.split('\n').slice(0, 3).join(' | ') : '';
        const mid = String(m?.id || '');
        const mproto = String(m?.protocolo || '');
        console.error('[condominios][GET /api/msg/messages/:id/imprimir.pdf] render erro:', {
          id: mid,
          protocolo: mproto,
          message: errMsg,
          stack: errStack
        ...
        try {
          ...
          doc.font('Helvetica-Bold').fontSize(10);
          doc.text('Falha ao renderizar uma mensagem deste PDF.', left, doc.y, { width: contentW, align: 'left' });
          doc.moveDown(0.2);
          doc.fillColor('#64748b');
          doc.font('Helvetica').fontSize(8.5);
          const dbg = [
            mid ? `id=${mid}` : '',
            mproto ? `protocolo=${mproto}` : '',
            errMsg ? `erro=${errMsg}` : '',
            errStackHead ? `stack=${errStackHead}` : ''
          ].filter(Boolean).join('  ');
          if (dbg) doc.text(dbg.slice(0, 900), left, doc.y, { width: contentW, align: 'left' });
          doc.fillColor('#0f172a');
          doc.font('Helvetica').fontSize(10);
          doc.moveDown(1.0);
        } catch { /* noop */ }
    ...
    console.error('[condominios][GET /api/msg/messages/:id/imprimir.pdf] erro:', e);
    try {
      // Se já começou a enviar o PDF, apenas encerra.
      if (res.headersSent) {
        try { return res.end(); } catch { return; }
      }

      // Em vez de 500 (que faz o viewer do Chrome acusar falha), retorna um PDF mínimo com a mensagem.
      res.status(200);
      res.setHeader('Content-Type', 'application/pdf');
      res.setHeader('Content-Disposition', 'inline; filename="mensagem-erro.pdf"');
      try {
        res.setHeader('Cache-Control', 'no-store, no-cache, must-revalidate, proxy-revalidate');
        res.setHeader('Pragma', 'no-cache');
        res.setHeader('Expires', '0');
      ...
      const doc = new PDFDocument({ size: 'A4', margins: { top: 72, left: 50, right: 50, bottom: 72 } });
      doc.pipe(res);
      doc.font('Helvetica-Bold').fontSize(14).fillColor('#0f172a').text('Falha ao gerar PDF', { align: 'left' });
      doc.moveDown(0.5);
      doc.font('Helvetica').fontSize(10).fillColor('#334155')
        .text('Ocorreu um erro inesperado ao gerar o PDF desta mensagem.', { align: 'left' });
      doc.moveDown(0.5);
      doc.font('Helvetica').fontSize(9).fillColor('#64748b')
        .text('Detalhes (para suporte): ' + String(e?.message || e || '').slice(0, 800), { align: 'left' });
      doc.end();
      return;
    } catch {
      try { return res.status(500).end('Falha ao gerar PDF'); } catch { return res.end(); }
...
app.post('/api/msg/messages/:id/read', async (req, res) => {
  try {
    let ctxUser = getCtxUser(req);
    if (!ctxUser) return res.status(401).json({ error: 'Não autenticado' });
    if (mongoose.connection.readyState !== 1) {
    ...
    // Compat (Portal proxy): o body pode chegar como Buffer/string por conta de middleware raw.
    // Não dependa de express.json() aqui para evitar 400 silencioso e perda do estado "lida".
    const bodyObj = (() => {
      try {
        const b = req?.body;
        if (!b) return {};
    ...
    const markThread = (() => {
      try {
        const v = bodyObj?.thread ?? bodyObj?.markThread ?? bodyObj?.threadAll ?? req.query?.thread ?? req.query?.markThread;
        const s = String(v ?? '').trim().toLowerCase();
    ...
    // Para garantir que a mensagem aberta passe a constar como lida, marcamos todos os states
    // do mesmo usuário (mesmo e-mail base) quando aplicável.
    const baseEmail = (() => {
      try {
        const owner = String(scope?.owner || '').trim().toLowerCase();
        const fromOwner = ownerKeyBaseEmailLower(owner);
        if (isEmailish(fromOwner)) return fromOwner;
        const em = String(ctxUser?.email || ctxUser?.userEmail || ctxUser?.contato_email || ctxUser?.contatoEmail || '').trim().toLowerCase();
    ...
    let portalHabIds = [];
    try {
      // Se o request veio do Portal e não temos habitacao_id consistente no ctxUser,
      // coletamos as habitações relacionadas e marcamos como lida no escopo por habitação.
      if (fromPortal && mailboxId === 'pessoal' && isEmailish(baseEmail)) {
        const unidadeId = getUserUnidadeId(ctxUser);
        portalHabIds = await collectPortalHabitacaoIds(ctxUser, req, unidadeId);
        if (Array.isArray(portalHabIds)) portalHabIds = portalHabIds.filter(mongoose.isValidObjectId).slice(0, 10);
        else portalHabIds = [];
      }
    } catch { /* noop */ }
    ...
    const hasStateAccess = (() => {
      try {
        ...
        if (!ownerCandidatesLower.length) return false;
        const states = Array.isArray(doc?.states) ? doc.states : [];
        return states.some(s => String(s?.mailbox_id || '').trim() === 'pessoal' && ownerCandidatesLower.includes(String(s?.owner || '').trim().toLowerCase()));
      } catch {
        return false;
      }
    })();
    ...
    // Helper: aplica a marcação "lida" em um doc específico respeitando o mesmo conjunto de owners.
    const markDocRead = (docToMark) => {
      try {
        if (!docToMark) return;
        if (scope.mailboxId === 'pessoal') {
          const ownersToMarkLower = (() => {
            const out = [];
            const add = (v) => {
              const s = String(v || '').trim().toLowerCase();
              if (!s) return;
            ...
            if (isEmailish(baseEmail)) {
              ...
              add(`${baseEmail}::colab`);
              (portalHabIds || []).forEach((hid) => add(`${baseEmail}::portal::hab:${hid}`));
            }
            (ownerCandidatesLower || []).forEach(add);
            return out;
          })();

          const ownersSet = new Set(ownersToMarkLower);
          const states = Array.isArray(docToMark.states) ? docToMark.states : [];
          const shouldMarkOwner = (ownerVal) => {
            const ownerLower = String(ownerVal || '').trim().toLowerCase();
            if (!ownerLower) return false;
            if (ownersSet.has(ownerLower)) return true;
            if (isEmailish(baseEmail)) {
              const base = ownerKeyBaseEmailLower(ownerLower) || ownerLower;
              if (base === baseEmail) return true;
            }
            return false;
          };

          const targets = states.filter(s => String(s?.mailbox_id || '').trim() === 'pessoal' && shouldMarkOwner(s?.owner));
          if (targets.length) {
            targets.forEach(st => { if (st && !st.lida_em) st.lida_em = now; });
          } else {
            const primary = ensureMessageState(docToMark, scope);
            if (primary && !primary.lida_em) primary.lida_em = now;
          }
        } else {
          const st = ensureMessageState(docToMark, scope);
          if (st && !st.lida_em) st.lida_em = now;
        }
      } catch {
        /* noop */
      }
    };
    // Marca o doc alvo como lido.
    markDocRead(doc);

    // Opcional: marca toda a conversa (thread) como lida.
    if (markThread) {
      try {
        const rootIdRaw = String(doc?.thread_root_id || doc?._id || '').trim();
        if (rootIdRaw) {
          const rootObjId = mongoose.isValidObjectId(rootIdRaw) ? new mongoose.Types.ObjectId(rootIdRaw) : null;

          // Dados legados podem armazenar thread_root_id como string.
          const threadQueryOrs = [];
          if (rootObjId) threadQueryOrs.push({ _id: rootObjId }, { thread_root_id: rootObjId });
          threadQueryOrs.push({ _id: rootIdRaw }, { thread_root_id: rootIdRaw });

          const threadDocs = await CondMsgMessage.find({
            ativo: { $ne: false },
            $or: threadQueryOrs
          }).limit(250);

          const hasStateAccessForDoc = (doc2) => {
            try {
              if (scope.mailboxId !== 'pessoal') return false;
              if (!ownerCandidatesLower || !ownerCandidatesLower.length) return false;
              const states = Array.isArray(doc2?.states) ? doc2.states : [];
              return states.some(s => String(s?.mailbox_id || '').trim() === 'pessoal'
                && ownerCandidatesLower.includes(String(s?.owner || '').trim().toLowerCase()));
            } catch {
              return false;
            }
          };

          const hasNameAccessForDoc = (doc2) => {
            try {
              if (!nameFallback) return false;
              const msgUnitId = doc2?.unidade_id ? String(doc2.unidade_id) : '';
              if (!msgUnitId || msgUnitId !== nameFallback.unidadeId) return false;
              const list = [...(doc2?.to || []), ...(doc2?.cc || [])];
              return list.some(m => {
                const n = String(m?.nome || m?.name || '').trim();
                return n && nameFallback.nameRx.test(n);
              });
          ...
          for (const d of (threadDocs || [])) {
            // Segurança: só marca se o usuário tem acesso a este item.
            const allowed2 = admin
              || isScopeSender(d, scope)
              || hasScopeAsRecipient(d, scope)
              || hasStateAccessForDoc(d)
              || hasNameAccessForDoc(d);
            if (!allowed2) continue;
            markDocRead(d);
          }

          // Persiste alterações da thread.
          try {
            await Promise.all((threadDocs || []).map(d => d && typeof d.save === 'function' ? d.save().catch(() => null) : null));
          } catch { /* noop */ }
        }
      } catch {
        /* noop */
    ...
    // registra acesso (sempre) apenas no doc aberto
    const who = getUserIdentityKey(ctxUser) || String(ctxUser?.nome || ctxUser?.name || '').trim() || 'Usuário';
    const ev = { mailbox_id: scope.mailboxId, owner: scope.owner, user: who, at: now };
    const list = Array.isArray(doc.acessos) ? doc.acessos : [];
    doc.acessos = [...list, ev].slice(-200);
    await doc.save();
    return res.json({ ok: true });
  } catch (e) {
    console.error('[condominios][POST /api/msg/messages/:id/read] erro:', e);
...
function safeUploadName(name) {
  ...
  // remove caracteres problemáticos em caminho; mantém ._- e alfanum
  const cleaned = raw
    .replace(/\s+/g, '_')
...
function maybeUploadMsgAttachments(req, res, next) {
  const ct = String(req.headers['content-type'] || '').toLowerCase();
  if (!ct.includes('multipart/form-data')) return next();

  return msgUpload.array('anexos', 15)(req, res, (err) => {
    if (!err) return next();
    const msg = String(err?.message || '').toLowerCase();
    if (String(err?.code || '') === 'LIMIT_FILE_SIZE') {
      return res.status(400).json({ error: 'Cada arquivo deve ter no máximo 10MB.' });
    }
    if (msg.includes('unexpected field')) {
      return res.status(400).json({ error: 'Campo de anexo inválido.' });
    }
    return res.status(400).json({ error: 'Falha ao processar anexos.' });
  });
}
...
function sanitizeGroupMembers(members) {
  ...
  for (const m of arr) {
    ...
    if (type === 'mailbox' || m.mailboxId) {
      ...
      out.push({ type: 'mailbox', mailboxId, name });
      continue;
    }
    const email = String(m.email || '').trim().toLowerCase();
    const nome = String(m.nome || m.name || '').trim();
    if (!email && !nome) continue;
    out.push({ type: 'user', email, nome, fotoUrl: String(m.fotoUrl || m.foto || m.photo || m.avatar || '').trim() });
  }
...
async function loadMailboxOrThrow(mailboxId) {
  ...
  if (!mongoose.isValidObjectId(mailboxId)) {
    const err = new Error('mailboxId inválido');
    err.status = 400;
    throw err;
  }
  const mailbox = await CondMsgMailbox.findById(mailboxId).lean();
  if (!mailbox || mailbox.ativo === false) {
...
// API: grupos de destinatários (persistidos em banco)
app.get('/api/msg/groups', async (req, res) => {
  try {
    const ctxUser = getCtxUser(req);
    if (!ctxUser) return res.status(401).json({ error: 'Não autenticado' });
    if (mongoose.connection.readyState !== 1) {
      const ok = await ensureMongoReady().catch((err) => {
        if (isMongoOfflineError(err)) return false;
        throw err;
      });
    ...
    maybeRunCondMsgRetention('GET /api/msg/messages/:id').catch(() => { /* noop */ });
    ...
    try {
      ...
      if (!admin && fromPortal && mailboxIsPublic(mailbox) && !mailboxIsMember(mailbox, ctxUser)) {
        return res.status(403).json({ error: 'Acesso negado' });
      }
    } catch { /* noop */ }

    const docs = await CondMsgGroup.find({ mailbox_id: mailboxId, ativo: { $ne: false } })
      .sort({ name: 1, createdAt: -1 })
      .lean();
    return res.json((docs || []).map(toGroupClient).filter(Boolean));
  } catch (e) {
    if (isMongoOfflineError(e)) return respondDbOffline(res, req);
    const st = e && e.status ? Number(e.status) : 500;
    if (st !== 500) return res.status(st).json({ error: String(e.message || 'Erro') });
    console.error('[condominios][GET /api/msg/groups] erro:', e);
    return res.status(500).json({ error: 'Falha ao listar grupos' });
  }
});

app.post('/api/msg/groups', express.json({ limit: '2mb' }), async (req, res) => {
  try {
    const ctxUser = getCtxUser(req);
    if (!ctxUser) return res.status(401).json({ error: 'Não autenticado' });
    if (mongoose.connection.readyState !== 1) {
    ...
    startCondMsgRetentionInterval();
    maybeRunCondMsgRetention('POST /api/msg/messages/:id/read').catch(() => { /* noop */ });
    ...
    const createdBy = getUserIdentityKey(ctxUser) || String(ctxUser?.nome || ctxUser?.name || '').trim();
    ...
    const st = e && e.status ? Number(e.status) : 500;
    if (st !== 500) return res.status(st).json({ error: String(e.message || 'Erro') });
    console.error('[condominios][POST /api/msg/groups] erro:', e);
    return res.status(500).json({ error: 'Falha ao criar grupo' });
  }
});

app.patch('/api/msg/groups/:id', express.json({ limit: '2mb' }), async (req, res) => {
  try {
    const ctxUser = getCtxUser(req);
    if (!ctxUser) return res.status(401).json({ error: 'Não autenticado' });
    if (mongoose.connection.readyState !== 1) {
    ...
    if (!doc || doc.ativo === false) return res.status(404).json({ error: 'Grupo não encontrado' });
    ...
    const st = e && e.status ? Number(e.status) : 500;
    if (st !== 500) return res.status(st).json({ error: String(e.message || 'Erro') });
    console.error('[condominios][PATCH /api/msg/groups/:id] erro:', e);
    return res.status(500).json({ error: 'Falha ao atualizar grupo' });
  }
});

app.delete('/api/msg/groups/:id', async (req, res) => {
  try {
    const ctxUser = getCtxUser(req);
    if (!ctxUser) return res.status(401).json({ error: 'Não autenticado' });
    if (mongoose.connection.readyState !== 1) {
    ...
    const st = e && e.status ? Number(e.status) : 500;
    if (st !== 500) return res.status(st).json({ error: String(e.message || 'Erro') });
    console.error('[condominios][DELETE /api/msg/groups/:id] erro:', e);
    return res.status(500).json({ error: 'Falha ao excluir grupo' });
  }
});

// API: caixas de mensagem (persistidas em banco)
// - /api/msg/mailboxes: lista para navegação (somente caixas que o usuário participa, exceto admin)
// - /api/msg/mailboxes/recipients: lista para seleção de destinatários (inclui caixas públicas da unidade)
app.get('/api/msg/mailboxes/recipients', async (req, res) => {
  try {
    let ctxUser = getCtxUser(req);
    if (!ctxUser) return res.status(401).json({ error: 'Não autenticado' });
    const wantDebug = String(req.query?.debug || req.query?.__debug || '').trim() === '1';
    const includeHabitacoes = String(req.query?.includeHabitacoes || req.query?.include_habitacoes || req.query?.habitacoes || '').trim() === '1';
    try {
      res.set('Cache-Control', 'no-store');
      res.set('Pragma', 'no-cache');
      res.set('Expires', '0');
    ...
    } else if (fromPortal && allowedUnitIds.length) {
      filter.unidade_id = { $in: allowedUnitIds.map(id => new mongoose.Types.ObjectId(id)) };
    } else if (unidadeId) {
      if (!mongoose.isValidObjectId(unidadeId)) return res.status(400).json({ error: 'unidade_id inválido' });
      filter.unidade_id = unidadeId;
    }
    ...
    if (!admin) {
      if (fromPortal) {
        // Portal: mostrar caixas do condomínio como potenciais destinatários.
        // Habitação: apenas se pertencer ao morador/proprietário.
        visible = (visible || []).filter(d => {
          if (!d) return false;
          if (mailboxIsHabitacao(d)) {
            if (portalHabIds.length) return mailboxMatchesHabitacaoIds(d, portalHabIds);
            return mailboxMatchesPortalHabitacoes(d, ctxUser, unidadeId);
          }
          // Para envio, a regra dura (de segurança) está no POST /api/msg/messages (escopo por unidade).
          // Aqui evitamos esconder caixas existentes, que causava “sumir” no Para/Cópia.
          return true;
        });
      } else {
        // Gestor: para seleção de destinatários, exibir todas as caixas ativas da unidade.
        // Regras de leitura/permissão continuam aplicadas em outras rotas.
        visible = visible || [];
      }
    }

    // Regra histórica: caixas de habitação não aparecem no Gestor por padrão.
    // Porém, para seleção de destinatários, podemos habilitar via query (ex.: includeHabitacoes=1).
    if (!fromPortal && !includeHabitacoes) {
      visible = (visible || []).filter(d => !mailboxIsHabitacao(d));
    }

    // Para seleção de destinatários, não precisamos expor operadores/criador.
    const out = (visible || [])
      .map(d => ({
    ...
    console.error('[condominios][GET /api/msg/mailboxes/recipients] erro:', e);
...
app.get('/api/msg/recipients/perms', async (req, res) => {
  try {
    ...
    console.error('[condominios][GET /api/msg/recipients/perms] erro:', e);
...
app.get('/api/msg/mailboxes', async (req, res) => {
  try {
    ...
    // No Gestor, mesmo admin não deve listar caixas de habitação.
    const docsFiltered = !fromPortal
      ? (docs || []).filter(d => !mailboxIsHabitacao(d))
      : (docs || []);

    const visible = admin
      ? docsFiltered
      : (docsFiltered || []).filter(d => {
          if (mailboxIsMember(d, ctxUser)) return true;
          if (fromPortal) {
            if (portalHabIds.length && mailboxMatchesHabitacaoIds(d, portalHabIds)) return true;
            if (!portalHabIds.length && mailboxMatchesPortalHabitacoes(d, ctxUser, unidadeId)) return true;
            if (mailboxIsPublic(d)) return true;
          }
          return false;
        });
    ...
    const operatorDisplayByKeyLower = new Map();
    try {
      const keys = new Set();
      for (const d of (visible || [])) {
        const ops = Array.isArray(d?.operators) ? d.operators : [];
        for (const op of ops) {
          const raw = (typeof op === 'string') ? op : (op && typeof op === 'object' ? op.user : '');
      ...
      const ids = allKeys.filter(k => mongoose.isValidObjectId(k));
      const emails = allKeys.filter(k => k.includes('@'));

      const [docsById, docsByEmail] = await Promise.all([
        ids.length
          ? CondUsuario.find({ _id: { $in: ids } }).select('_id nome email').lean().catch(() => [])
          : Promise.resolve([]),
        emails.length
          ? CondUsuario.find({ email: { $in: emails } }).select('_id nome email').lean().catch(() => [])
          : Promise.resolve([])
      ]);
      ...
      const add = (doc) => {
        ...
        const display = (nome && email) ? `${nome} (${email})` : (nome || email || '');
        if (!display) return;
        if (email) operatorDisplayByKeyLower.set(email.toLowerCase(), display);
        if (id) operatorDisplayByKeyLower.set(id.toLowerCase(), display);
      };
      (Array.isArray(docsById) ? docsById : []).forEach(add);
      (Array.isArray(docsByEmail) ? docsByEmail : []).forEach(add);
    } catch {
      // silencioso: apenas melhora rótulo
    }
    ...
    const out = (visible || [])
      .map(d => {
        ...
        // Flag explícita (para UI do Portal): evita depender de ctx.userEmail no frontend.
        // - true para membro/participante (inclui habitação quando aplicável)
        // - false para caixas públicas não-membro
        let isMember = false;
        try {
          if (admin) {
            isMember = true;
          } else if (mailboxIsMember(d, ctxUser)) {
            isMember = true;
          } else if (fromPortal && mailboxIsHabitacao(d)) {
            if (portalHabIds.length) isMember = mailboxMatchesHabitacaoIds(d, portalHabIds);
            else isMember = mailboxMatchesPortalHabitacoes(d, ctxUser, unidadeId);
          }
        } catch {
          isMember = false;
        }
        ...
        // Permissão para administrar (UI): importante para ocultar editor de permissões no Portal.
        try {
          const perms = mailboxGetUserPerms(d, ctxUser);
          mb.canAdmin = !!(admin || (perms && typeof perms === 'object' && perms.administrar));

          // Permissões do usuário atual (para UI). No Portal, evita depender do ctx.userEmail no frontend.
          // Importante: retorna sempre um objeto normalizado com todas as keys conhecidas.
          try {
        ...
        // No Portal, caixas públicas podem ser exibidas para leitura/envio,
        // mas não devem expor operadores/criador a não-membros.
        if (!admin && fromPortal && mailboxIsPublic(d) && !mailboxIsMember(d, ctxUser)) {
          mb.createdBy = '';
          mb.operators = [];
        } else {
          // Enriquecer operadores com displayName (nome) quando disponível.
          try {
            ...
            mb.operators = rawOps
              .map(op => {
                if (typeof op === 'string') {
                  ...
                  if (!user) return null;
                  const keyLower = user.toLowerCase();
                  return { user, perms: {}, displayName: operatorDisplayByKeyLower.get(keyLower) || '' };
                }
                if (op && typeof op === 'object') {
                  ...
                  const keyLower = user.toLowerCase();
                  const displayName = String(op.displayName || op.nome || op.name || '').trim() || (operatorDisplayByKeyLower.get(keyLower) || '');
    ...
    if (wantDebug) {
      const meta = buildMailboxesDebugMeta(ctxUser, req, {
        unidadeId: unidadeId || '',
        portalHabIdsCount: portalHabIds.length,
        counts: {
          total: Array.isArray(docs) ? docs.length : 0,
          visible: Array.isArray(out) ? out.length : 0
        }
    ...
    console.error('[condominios][GET /api/msg/mailboxes] erro:', e);
    res.status(500).json({ error: 'Falha ao listar caixas' });
  }
});
...
app.post('/api/msg/mailboxes', express.json(), async (req, res) => {
  try {
    ...
    // Denormaliza nome da unidade para facilitar UI
    ...
    const creator = getUserIdentityKey(ctxUser) || String(ctxUser?.nome || ctxUser?.name || '').trim();
    ...
    res.status(201).json(toMailboxClient(doc));
  } catch (e) {
    console.error('[condominios][POST /api/msg/mailboxes] erro:', e);
    res.status(500).json({ error: 'Falha ao criar caixa' });
  }
});

app.patch('/api/msg/mailboxes/:id', express.json(), async (req, res) => {
  try {
    let ctxUser = getCtxUser(req);
    if (!ctxUser) return res.status(401).json({ error: 'Não autenticado' });
    if (mongoose.connection.readyState !== 1) {
    ...
    // Regra: edição exige que o usuário seja membro (exceto admin/scopeAll).
    if (!admin && !mailboxIsMember(doc, ctxUser)) {
      return res.status(403).json({ error: 'Acesso negado' });
    }

    const wantsOperatorsUpdate = !!(req.body && Object.prototype.hasOwnProperty.call(req.body, 'operators'));
    if (wantsOperatorsUpdate) {
      // Regra: apenas "Administrar a caixa" pode alterar operadores/permissões.
      if (!admin) {
        const perms = mailboxGetUserPerms(doc, ctxUser);
        if (!perms || !perms.administrar) {
          return res.status(403).json({ error: 'Somente quem tem Administrar a caixa pode alterar permissões' });
        }
      }
    } else {
      // Outras edições (ex.: nome) seguem a regra antiga de edit.
      if (!mailboxCanEdit(doc, ctxUser)) return res.status(403).json({ error: 'Acesso negado' });
    }
    ...
    await doc.save();
    res.json(toMailboxClient(doc));
  } catch (e) {
    console.error('[condominios][PATCH /api/msg/mailboxes/:id] erro:', e);
    res.status(500).json({ error: 'Falha ao atualizar caixa' });
  }
});

app.delete('/api/msg/mailboxes/:id', async (req, res) => {
  try {
    let ctxUser = getCtxUser(req);
    if (!ctxUser) return res.status(401).json({ error: 'Não autenticado' });
    if (mongoose.connection.readyState !== 1) {
    ...
    // Proteções: não permitir apagar caixas de sistema/habitação por esta rota.
    try {
      if (mailboxIsHabitacao(doc)) return res.status(400).json({ error: 'Não é permitido excluir caixa de habitação.' });
    } catch { /* noop */ }
    try {
      const t = String(doc.type || '').trim().toLowerCase();
      if (t && t !== 'grupo') return res.status(400).json({ error: 'Somente caixas de grupo podem ser excluídas.' });
    } catch { /* noop */ }

    // Regra: exclusão exige membership (exceto admin/scopeAll).
    if (!admin && !mailboxIsMember(doc, ctxUser)) {
      return res.status(403).json({ error: 'Acesso negado' });
    }
    if (!mailboxCanEdit(doc, ctxUser)) return res.status(403).json({ error: 'Acesso negado' });

    const deleted = await hardDeleteMailboxWithCleanup(id, { logTag: '[condominios][DELETE /api/msg/mailboxes/:id]' });
    res.json({ ok: true, id, deleted: { mailbox: 1, ...deleted } });
  } catch (e) {
    console.error('[condominios][DELETE /api/msg/mailboxes/:id] erro:', e);
    res.status(500).json({ error: 'Falha ao excluir caixa' });
  }
});

// Admin: excluir mailbox definitivamente (somente master/admin) — apenas caixas manuais de grupo
app.delete('/api/msg/admin/mailboxes/:id', async (req, res) => {
  try {
    const ctxUser = getCtxUser(req);
    if (!ctxUser) return res.status(401).json({ error: 'Não autenticado' });
    ...
    const linkType = String(doc.link_type || '').trim().toLowerCase();
    const linkId = doc.link_id ? String(doc.link_id) : '';
    if (linkType || linkId) {
      return res.status(400).json({ error: 'Somente caixas de grupo criadas manualmente podem ser excluídas.' });
    }
...
async function hardDeleteMailboxWithCleanup(id, opts = {}) {
  const tag = String(opts?.logTag || '[condominios][hardDeleteMailbox]');
  // Hard delete: remove do banco e limpa dados vinculados.
  // Observação: para evitar blobs órfãos, pode remover anexos quando ENABLE_DELETE_OLD_BLOB=1.
  ...
  // Mensagens: remove states dessa mailbox; se sobrar sem states, remove do banco.
  try {
    const affected = await CondMsgMessage.find({ ativo: { $ne: false }, 'states.mailbox_id': id })
      .select('_id')
      .lean();
    const ids = (affected || []).map(d => String(d?._id || '')).filter(mongoose.isValidObjectId);
    if (ids.length) {
...
app.get('/api/msg/signature', async (req, res) => {
  try {
    let ctxUser = getCtxUser(req);
    if (!ctxUser) return res.status(401).json({ error: 'Não autenticado' });
    if (mongoose.connection.readyState !== 1) {
    ...
    if (mailboxId !== 'pessoal') {
      const mailbox = await loadMailboxOrThrow(mailboxId);
      if (!mailboxCanSendMessage(mailbox, ctxUser)) return res.status(403).json({ error: 'Acesso negado' });
    }

    // Prioriza match exato e faz fallback para baseEmail (compat). Evita retorno não-determinístico
    // quando existem duplicatas antigas com chaves diferentes.
    let doc = await CondMsgSignaturePref.findOne({ owner: owner, mailbox_id: mailboxId }).lean();
    if (!doc && baseEmail && baseEmail !== owner) {
      doc = await CondMsgSignaturePref.findOne({ owner: baseEmail, mailbox_id: mailboxId }).lean();
    }
    if (!doc) return res.json({ enabled: false, text: '' });
    return res.json({ enabled: !!doc.enabled, text: String(doc.text || '') });
  } catch (e) {
    const st = e && e.status ? Number(e.status) : 500;
    if (st !== 500) return res.status(st).json({ error: String(e.message || 'Erro') });
    console.error('[condominios][GET /api/msg/signature] erro:', e);
    return res.status(500).json({ error: 'Falha ao carregar assinatura' });
  }
});

app.put('/api/msg/signature', express.json({ limit: '64kb' }), async (req, res) => {
  try {
    let ctxUser = getCtxUser(req);
    if (!ctxUser) return res.status(401).json({ error: 'Não autenticado' });
    if (mongoose.connection.readyState !== 1) {
    ...
    return res.json({ enabled: !!updated?.enabled, text: String(updated?.text || '') });
  } catch (e) {
    const st = e && e.status ? Number(e.status) : 500;
    if (st !== 500) return res.status(st).json({ error: String(e.message || 'Erro') });
    console.error('[condominios][PUT /api/msg/signature] erro:', e);
...
app.post('/api/msg/messages', maybeUploadMsgAttachments, async (req, res) => {
  try {
    ...
    const ct = String(req.headers['content-type'] || '').toLowerCase();
    const rawPayload = ct.includes('multipart/form-data') ? (req.body?.payload || '') : '';
    ...
    if (fromPortal) {
      ...
      // Inclui tanto usuários do Gestor (User) quanto usuários do condomínio (CondUsuario).
      const allowedEmails = new Set();
      try {
        if (User && unitIds.length) {
          const userEmails = await User.find({ unidade_id: { $in: unitIds } })
            .select('email')
            .lean();
      ...
      try {
        if (CondUsuario && unitIds.length) {
          const condEmails = await CondUsuario.find({ ativo: { $ne: false }, unidade_id: { $in: unitIds } })
            .select('email')
            .lean();
      ...
      // Portal: incluir moradores/proprietários (para permitir Pessoal→Pessoal dentro do Portal,
      // quando habilitado nas Configurações > Geral).
      try {
        if (CondMorador && unitIds.length) {
          const morEmails = await CondMorador.find({ ativo: { $ne: false }, unidade_id: { $in: unitIds } })
            .select('email responsavel_email')
            .lean();
          (morEmails || []).forEach(m => {
            const a = String(m?.email || '').trim().toLowerCase();
            const b = String(m?.responsavel_email || '').trim().toLowerCase();
            if (a) allowedEmails.add(a);
            if (b) allowedEmails.add(b);
            if (a && isEmailish(a)) portalPersonalEmails.add(a);
            if (b && isEmailish(b)) portalPersonalEmails.add(b);
          });
        }
      } catch { /* noop */ }

      try {
        if (CondProprietario && unitIds.length) {
          const propEmails = await CondProprietario.find({ ativo: { $ne: false }, unidade_id: { $in: unitIds } })
            .select('contato_email')
            .lean();
          (propEmails || []).forEach(p => {
            const em = String(p?.contato_email || '').trim().toLowerCase();
            if (em) allowedEmails.add(em);
      ...
      // Caso extremo: se não conseguimos determinar o escopo, bloqueia por segurança.
      if (!allowedUnitIds.size) {
        return res.status(403).json({ error: 'Não foi possível determinar a unidade do usuário.' });
      }

      async function validateRecipients(list, label) {
        const arr = Array.isArray(list) ? list : [];
        for (const m of arr) {
          const type = String(m?.type || '').trim().toLowerCase();
          if (type === 'mailbox') {
            const mailboxId = String(m?.mailboxId || '').trim();
            if (!mailboxId) return { ok: false, error: `Destinatário inválido em ${label}.` };
            try {
              const mb = await loadMailboxOrThrow(mailboxId);
              const mbUnitId = mb && mb.unidade_id ? String(mb.unidade_id).trim() : '';
              if (!mbUnitId || !allowedUnitIds.has(mbUnitId)) {
                return { ok: false, error: `Caixa de destinatário fora da sua unidade em ${label}.` };
              }
            } catch {
              return { ok: false, error: `Caixa de destinatário inválida em ${label}.` };
            }
            continue;
          }
          ...
          const email = String(m?.email || '').trim().toLowerCase();
          if (!email) {
            return { ok: false, error: `Selecione um usuário da lista em ${label} (e-mail obrigatório).` };
          }
          if (!allowedEmails.has(email)) {
            return { ok: false, error: `Usuário fora da sua unidade em ${label}.` };
          }
        }
        return { ok: true };
      }

      const vTo = await validateRecipients(to, 'Para');
      if (!vTo.ok) return res.status(400).json({ error: vTo.error });
      const vCc = await validateRecipients(cc, 'Cópia');
      if (!vCc.ok) return res.status(400).json({ error: vCc.error });
    }
    ...
    const threadRootId = threadRootIdRaw && mongoose.isValidObjectId(threadRootIdRaw) ? new mongoose.Types.ObjectId(threadRootIdRaw) : null;
    const inReplyToId = inReplyToIdRaw && mongoose.isValidObjectId(inReplyToIdRaw) ? new mongoose.Types.ObjectId(inReplyToIdRaw) : null;
    const forwardedFromId = forwardedFromIdRaw && mongoose.isValidObjectId(forwardedFromIdRaw) ? new mongoose.Types.ObjectId(forwardedFromIdRaw) : null;
    ...
    // Caixa pessoal exige chave estável (preferencialmente e-mail). Não use nome.
    if (fromMailboxId === 'pessoal' && !isEmailish(fromOwner)) {
      return res.status(400).json({ error: 'Não foi possível determinar o e-mail do remetente para enviar pela caixa pessoal.' });
    }
    ...
    if (fromMailboxId === 'pessoal') {
      ...
      const mailbox = await loadMailboxOrThrow(fromMailboxId);
      if (!(await canAccessMsgMailbox(mailbox, ctxUser, req))) return res.status(403).json({ error: 'Acesso negado' });
      if (!admin && !mailboxCanSendMessage(mailbox, ctxUser)) return res.status(403).json({ error: 'Sem permissão para enviar pela caixa' });
      unidadeId = mailbox.unidade_id || null;
      fromMailboxName = String(mailbox.name || '').trim();
    }

    // Regras por unidade (Configurações > Geral)
    try {
      const unitKey = unidadeId ? String(unidadeId).trim() : '';
      const settings = unitKey ? await getOrInitMsgSettingsForUnidade(unitKey) : null;
      const suspendedPersonalGlobal = !!(settings && settings.suspender_caixas_pessoais);
      const suspendedGroupGlobal = !!(settings && settings.suspender_caixas_grupo);
      // IMPORTANTE: pessoal→pessoal é uma regra do Portal do Morador. No Gestor, não bloqueamos.
      const allowP2P = !(settings && settings.permitir_pessoal_para_pessoal === false);
      const legacy = normalizeOwnerKeyList(settings?.pessoais_suspensas || []);
      const portalList = normalizeOwnerKeyList(settings?.pessoais_suspensas_portal || []);
      const colabList = normalizeOwnerKeyList(settings?.pessoais_suspensas_colaborador || []);
      const hasNew = (portalList.length + colabList.length) > 0;
      ...
      const fromOwnerBaseEmailLower = ownerKeyBaseEmailLower(fromOwnerLower);

      // =========================
      // Restrições por usuário (Configuração > Geral) também para Colaborador/Gestor
      // Requisito do negócio: em "Nova mensagem", quando as flags estiverem desativadas,
      // o usuário não pode escolher pessoas (Portal/Colaborador) nem caixas de habitação de terceiros.
      // Exceção: em reply (inReplyToId), permite responder apenas ao remetente original.
      // =========================
      if (fromMailboxId === 'pessoal' && !fromPortal) {
        const p = resolvePortalPerms(fromOwnerBaseEmailLower || fromOwnerLower);
        const restricted = !!(
          p
          && (p.permitir_pessoal_para_pessoal === false
            || p.permitir_pessoal_para_habitacao === false
            || p.permitir_pessoal_para_colaborador === false)
        );

        if (restricted) {
          // Helper: coletar habitações do usuário (best-effort)
          const collectMyHabIds = async () => {
            const out = [];
            const add = (v) => {
              const s = String(v || '').trim();
              if (!s || !mongoose.isValidObjectId(s)) return;
            ...
            try {
              extractPortalHabitacaoIds(ctxUser, unitKey).forEach(add);
            } catch { /* noop */ }

            // Se não veio no snapshot do usuário, tenta por e-mail.
            if (!out.length) {
              const em = String(fromOwnerBaseEmailLower || fromOwnerLower || '').trim().toLowerCase();
              if (em && isEmailish(em)) {
                try {
                  const q = { ativo: { $ne: false } };
                  if (unitKey && mongoose.isValidObjectId(unitKey)) q.unidade_id = new mongoose.Types.ObjectId(unitKey);
                  // Morador por e-mail/responsável
                  const moradores = await CondMorador.find({ ...q, $or: [{ email: em }, { responsavel_email: em }] })
                    .select('habitacao_id')
                    .limit(20)
                    .lean()
                    .catch(() => []);
                ...
                try {
                  const q2 = { ativo: { $ne: false }, contato_email: em };
                  if (unitKey && mongoose.isValidObjectId(unitKey)) q2.unidade_id = new mongoose.Types.ObjectId(unitKey);
                  const props = await CondProprietario.find(q2)
                    .select('_id')
                    .limit(10)
                    .lean()
                    .catch(() => []);
                  ...
                  if (propIds.length) {
                    const qh = { proprietario_id: { $in: propIds.map(id => new mongoose.Types.ObjectId(id)) } };
                    if (unitKey && mongoose.isValidObjectId(unitKey)) qh.unidade_id = new mongoose.Types.ObjectId(unitKey);
                    const habs = await CondHabitacao.find(qh)
                      .select('_id')
                      .limit(30)
                      .lean()
                      .catch(() => []);
          ...
          // Reply: permite apenas responder ao remetente original.
          if (inReplyToId) {
            const base = await CondMsgMessage.findById(inReplyToId)
              .select('from_mailbox_id from_owner')
              .lean()
              .catch(() => null);
            if (!base) {
              return res.status(400).json({ error: 'Mensagem de referência inválida para resposta.' });
            }

            const baseFromMailboxId = String(base?.from_mailbox_id || '').trim();
            const baseFromOwner = String(base?.from_owner || '').trim().toLowerCase();
            const baseFromOwnerEmail = ownerKeyBaseEmailLower(baseFromOwner) || baseFromOwner;

            const all = [...to, ...cc];
            const bad = all.find(m => {
              const type = String(m?.type || '').trim().toLowerCase();
              if (type === 'mailbox') {
                const mid = String(m?.mailboxId || '').trim();
                return !!(baseFromMailboxId && mid && mid !== baseFromMailboxId);
              }
              const em = String(m?.email || '').trim().toLowerCase();
              if (!isEmailish(em)) return true;
              if (em === (fromOwnerBaseEmailLower || fromOwnerLower)) return false;
              return em !== baseFromOwnerEmail;
            });
            if (bad) {
              return res.status(400).json({ error: 'Sua permissão permite responder apenas ao remetente original.' });
            }
          } else {
            // Nova mensagem: aplica as regras por flag.
            const all = [...to, ...cc];

            // Classifica e-mails do Portal (morador/proprietário/cond_usuario) para decidir Pessoal→Pessoal vs Pessoal→Colaborador.
            const portalEmails = new Set();
            try {
              const unitObj = (unitKey && mongoose.isValidObjectId(unitKey)) ? new mongoose.Types.ObjectId(unitKey) : null;

              if (CondUsuario) {
                const q = { ativo: { $ne: false } };
                if (unitObj) q.unidade_id = unitObj;
                const list = await CondUsuario.find(q).select('email').limit(5000).lean().catch(() => []);
                (list || []).forEach(u => {
                  const em = String(u?.email || '').trim().toLowerCase();
                  if (em && isEmailish(em)) portalEmails.add(em);
                });
              }
              ...
              if (CondMorador) {
                const q = { ativo: { $ne: false } };
                if (unitObj) q.unidade_id = unitObj;
                const list = await CondMorador.find(q).select('email responsavel_email').limit(5000).lean().catch(() => []);
                (list || []).forEach(u => {
                  const a = String(u?.email || '').trim().toLowerCase();
                  const b = String(u?.responsavel_email || '').trim().toLowerCase();
                  if (a && isEmailish(a)) portalEmails.add(a);
                  if (b && isEmailish(b)) portalEmails.add(b);
                });
              }
              ...
              if (CondProprietario) {
                const q = { ativo: { $ne: false } };
                if (unitObj) q.unidade_id = unitObj;
                const list = await CondProprietario.find(q).select('contato_email').limit(5000).lean().catch(() => []);
                (list || []).forEach(u => {
                  const em = String(u?.contato_email || '').trim().toLowerCase();
                  if (em && isEmailish(em)) portalEmails.add(em);
                });
              }
            } catch { /* noop */ }

            // 1) Pessoas (destinatários user)
            for (const m of all) {
              const type = String(m?.type || '').trim().toLowerCase();
              if (type === 'mailbox') continue;
              const em = String(m?.email || '').trim().toLowerCase();
              if (!isEmailish(em)) {
                return res.status(400).json({ error: 'Selecione um usuário da lista (e-mail obrigatório).' });
              }
              // Envio para si mesmo ok.
              if (em === (fromOwnerBaseEmailLower || fromOwnerLower)) continue;

              const isPortalUser = portalEmails.size ? portalEmails.has(em) : false;
              if (!p.permitir_pessoal_para_pessoal && isPortalUser) {
                return res.status(400).json({ error: 'Envio para usuários do Portal do Morador está desativado.' });
              }
              if (!p.permitir_pessoal_para_colaborador && !isPortalUser) {
                return res.status(400).json({ error: 'Envio para colaboradores está desativado.' });
              }
            }

            // 2) Caixas de habitação (destinatários mailbox)
            if (!p.permitir_pessoal_para_habitacao) {
              const myHabIds = await collectMyHabIds().catch(() => []);
              const myHabSet = new Set((myHabIds || []).map(String));
              ...
              for (const m of all) {
                ...
                if (!mid) continue;
                try {
                  const mb = await loadMailboxOrThrow(mid);
                  const lt = String(mb?.link_type || mb?.linkType || '').trim().toLowerCase();
                  if (lt !== 'habitacao') continue;
                  const hid = String(mb?.link_id || mb?.linkId || '').trim();
                  if (!hid || !myHabSet.has(hid)) {
                    return res.status(400).json({ error: 'Envio para caixa de habitação de terceiros está desativado.' });
                  }
                } catch {
                  // ignore (validação já cobre mailbox inválida)
                }
              }
            }
          }
      ...
      if (fromMailboxId === 'pessoal') {
        if (suspendedPersonalGlobal) {
          return res.status(403).json({ error: 'Caixas pessoais estão suspensas para este condomínio.' });
        }
        const suspendedSet = fromPortal ? suspendedOwnersPortal : suspendedOwnersColab;
        if (fromOwnerLower && (suspendedSet.has(fromOwnerLower) || (fromOwnerBaseEmailLower && suspendedSet.has(fromOwnerBaseEmailLower)))) {
          return res.status(403).json({ error: 'Sua caixa pessoal está suspensa para este condomínio.' });
        }

        // Regras do Portal por usuário (somente para caixa pessoal).
        if (fromPortal) {
          const p = resolvePortalPerms(fromOwnerBaseEmailLower || fromOwnerLower);
          const effectiveAllowP2P = !!(allowP2P && p.permitir_pessoal_para_pessoal);

          // Pessoal→Pessoal: restringe apenas Portal→Portal.
          if (!effectiveAllowP2P) {
            const hasPortalPersonalRecipient = [...to, ...cc].some(m => {
              const type = String(m?.type || '').trim().toLowerCase();
              if (type && type !== 'user') return false;
              const em = String(m?.email || '').trim().toLowerCase();
              if (!isEmailish(em)) return false;
              if ((fromOwnerBaseEmailLower && em === fromOwnerBaseEmailLower) || em === fromOwnerLower) return false; // envio para si mesmo ok
              if (!portalPersonalEmails || !portalPersonalEmails.size) return false; // fallback: não bloquear por falta de classificação
              return portalPersonalEmails.has(em);
            });
            if (hasPortalPersonalRecipient) {
              return res.status(400).json({ error: 'Envio de caixa pessoal para caixa pessoal está desativado.' });
            }
          }

          // Pessoal→Colaborador: bloqueia Portal→Gestor quando desativado.
          if (!p.permitir_pessoal_para_colaborador) {
            const hasColabRecipient = [...to, ...cc].some(m => {
              const type = String(m?.type || '').trim().toLowerCase();
              if (type && type !== 'user') return false;
              const em = String(m?.email || '').trim().toLowerCase();
              if (!isEmailish(em)) return false;
              if ((fromOwnerBaseEmailLower && em === fromOwnerBaseEmailLower) || em === fromOwnerLower) return false;
              if (!portalPersonalEmails || !portalPersonalEmails.size) return false;
              // Se não é portal, tratamos como colaborador.
              return !portalPersonalEmails.has(em);
            });
            if (hasColabRecipient) {
              return res.status(400).json({ error: 'Envio de caixa pessoal para colaborador está desativado.' });
            }
          }

          // Pessoal→Caixa de habitação: bloqueia quando desativado (somente mailboxes link_type=habitacao).
          if (!p.permitir_pessoal_para_habitacao) {
            const mailboxIds = [...to, ...cc]
              .filter(m => String(m?.type || '').trim().toLowerCase() === 'mailbox')
              .map(m => String(m?.mailboxId || '').trim())
              .filter(Boolean);
            ...
            for (const mid of mailboxIds) {
              try {
                ...
                const lt = String(mb?.link_type || mb?.linkType || '').trim().toLowerCase();
                if (lt === 'habitacao') {
                  return res.status(400).json({ error: 'Envio de caixa pessoal para caixa de habitação está desativado.' });
                }
              } catch {
                // ignore (validação já cobre mailbox inválida)
              }
        ...
        // Regra antiga (global): mantida por compatibilidade com mensagens antigas.
        // Se fromPortal e allowP2P estiver desativado, efetivamente já foi coberto acima.
        if (fromPortal && !allowP2P) {
          const hasPortalPersonalRecipient = [...to, ...cc].some(m => {
            const type = String(m?.type || '').trim().toLowerCase();
            if (type && type !== 'user') return false;
            const em = String(m?.email || '').trim().toLowerCase();
      ...
      // Se caixas de grupo estiverem suspensas, bloqueia envio PARA caixas (type=mailbox)
      if (suspendedGroupGlobal) {
        const hasMailboxRecipients = [...to, ...cc].some(m => {
          const type = String(m?.type || '').trim().toLowerCase();
          if (type !== 'mailbox') return false;
          return !!String(m?.mailboxId || '').trim();
        });
        if (hasMailboxRecipients) {
          return res.status(400).json({ error: 'Envio para caixas de grupo está desativado (caixas de grupo suspensas).' });
        }
      }

      // Bloqueia destinatários pessoais suspensos / suspensão global de pessoais
      const personalRecipients = [...to, ...cc]
        .filter(m => String(m?.type || '').trim().toLowerCase() !== 'mailbox')
      ...
      // best-effort: se falhar leitura de settings, não bloqueia envio
    }
    ...
    if (totalBytes > maxTotal) {
      return res.status(400).json({ error: 'Limite de 10MB em anexos (total) atingido.' });
    }

    const createdBy = String(ctxUser?.nome || ctxUser?.name || ctxUser?.username || '').trim()
      || String(ctxUser?.email || ctxUser?.userEmail || '').trim().toLowerCase()
      || String(getMsgIdentityKey(ctxUser, req) || '').trim();
    ...
    // Detecta envio para si mesmo (pessoal): se o remetente também está em To/CC,
    // não podemos marcar como lida na criação; caso contrário a inbox nunca terá "não lidas".
    const senderKey = `${String(fromMailboxId || '').trim()}::${fromOwnerKey}`;
    const recipientKeys = new Set();
    [...to, ...cc].forEach(m => {
      const s = memberToScopeKeys(m);
      if (s) {
    ...
    for (let i = 0; i < 7; i++) {
      ...
      try {
        ...
        // Se vier reply/forward mas sem root explícito, usa o próprio alvo como root.
        ...
        doc = await CondMsgMessage.create({
          ...
          client_nonce: clientNonce ? clientNonce : null,
          to,
          cc,
          assunto,
          body_html: bodyHtml,
    ...
    if (!doc) return res.status(500).json({ error: 'Falha ao gerar protocolo' });

    // Persistência dos anexos: somente Vercel Blob (evita dependência de filesystem/local)
    const blobToken = process.env.BLOB_READ_WRITE_TOKEN
      || process.env.WDGESTOR_DB_DADOS_READ_WRITE_TOKEN
      || process.env.VERCEL_BLOB_RW_TOKEN
      || '';
    ...
    if (files.length && !(inVercel || blobToken)) {
    ...
    try {
      for (const f of files.slice(0, 15)) {
        ...
        // `caminho`: chave/pathname do blob (útil para limpeza posterior)
        const caminho = String(uploaded?.pathname || uploaded?.path || key || '').trim();
        saved.push({ nome: original, mime, tamanho: size, url: uploaded.url, caminho });
      }
    } catch (e) {
    ...
    if (saved.length) {
      try {
        ...
        console.warn('[condominios][POST /api/msg/messages] falha ao salvar anexos no doc:', e?.message);
    ...
    // Push (Portal do Morador): notifica destinatários de perfil morador/proprietário.
    // Best-effort: nunca bloquear o envio por falha de push.
    try {
      if (isPortalPushConfigured && isPortalPushConfigured()) {
        const msgId = String(doc._id || '').trim();
        const protocoloMsg = String(doc.protocolo || '').trim();
        const unitKey = unidadeId ? String(unidadeId).trim() : '';

        const senderEmail = (() => {
          const base = ownerKeyBaseEmailLower(fromOwnerKey);
          if (base && isEmailish(base)) return base;
          if (fromOwnerKey && isEmailish(fromOwnerKey)) return fromOwnerKey;
          return '';
        })();

        const collectRecipientEmails = () => {
          const out = new Set();
          const all = [...(Array.isArray(to) ? to : []), ...(Array.isArray(cc) ? cc : [])];
          all.forEach(m => {
            const type = String(m?.type || '').trim().toLowerCase();
            if (type === 'mailbox') return;
            const em = String(m?.email || '').trim().toLowerCase();
            if (!isEmailish(em)) return;
            if (senderEmail && em === senderEmail) return;
            out.add(em);
          });
          return out;
        };

        const collectHabitacaoEmailsFromMailboxIds = async (mailboxIds) => {
          const out = new Set();
          const ids = Array.from(new Set((mailboxIds || []).map(v => String(v || '').trim()).filter(Boolean))).slice(0, 20);
          for (const mid of ids) {
            try {
              ...
              const habObj = new mongoose.Types.ObjectId(hid);
              try {
                const moradores = await CondMorador.find({ ativo: { $ne: false }, habitacao_id: habObj })
                  .select('email responsavel_email')
                  .limit(60)
                  .lean()
                  .catch(() => []);
                (moradores || []).forEach(m => {
                  const a = String(m?.email || '').trim().toLowerCase();
                  const b = String(m?.responsavel_email || '').trim().toLowerCase();
                  if (a && isEmailish(a)) out.add(a);
                  if (b && isEmailish(b)) out.add(b);
                });
              } catch { /* noop */ }

              try {
                const hab = await CondHabitacao.findById(habObj).select('proprietario_id unidade_id').lean().catch(() => null);
                const pid = hab?.proprietario_id ? String(hab.proprietario_id) : '';
                if (pid && mongoose.isValidObjectId(pid)) {
                  const prop = await CondProprietario.findById(pid).select('contato_email').lean().catch(() => null);
                  const em = String(prop?.contato_email || '').trim().toLowerCase();
                  if (em && isEmailish(em)) out.add(em);
                }
              } catch { /* noop */ }
            } catch {
              // mailbox inválida/sem acesso: ignora
            }
          }
          return out;
        };
        ...
        const bodyPreview = (String(assunto || '').trim() || 'Você recebeu uma nova mensagem.').slice(0, 120);
        const tag = msgId ? `pm-msg-${msgId}` : 'pm-msg';
        const data = {
          tipo: 'mensagem',
          msgId,
          msgProtocolo: protocoloMsg,
          assunto: String(assunto || '').trim(),
          unidadeId: unitKey,
          url: '/portal-morador/mensagens/entrada'
        };

        const run = async () => {
          const directEmails = collectRecipientEmails();

          const mailboxIds = [...(Array.isArray(to) ? to : []), ...(Array.isArray(cc) ? cc : [])]
            .filter(m => String(m?.type || '').trim().toLowerCase() === 'mailbox')
            .map(m => String(m?.mailboxId || '').trim())
            .filter(Boolean);

          const habEmails = mailboxIds.length ? await collectHabitacaoEmailsFromMailboxIds(mailboxIds) : new Set();

          const allEmails = new Set([...directEmails, ...habEmails]);
          if (senderEmail) allEmails.delete(senderEmail);

          // Heurística: evita spam/bursts em listas grandes.
          const targets = Array.from(allEmails).slice(0, 40);
          if (!targets.length) return;

          const wdIcon = '/images/logoWDGestor.png';
          ...
          try {
            if (unitKey && mongoose.isValidObjectId(unitKey)) {
              ...
              if (raw) {
                ...
                else if (raw.startsWith('/')) unitIcon = raw;
                else unitIcon = '/' + raw.replace(/^\/+/, '');
              }
            }
          } catch { /* noop */ }
          if (!unitIcon) unitIcon = wdIcon;

          // Master/admin (gestor): ícone WD Gestor.
          const masterAdminEmails = new Set();
          try {
            const rows = await User.find({ email: { $in: targets } }).select('email role').lean().catch(() => []);
            (rows || []).forEach(u => {
              const em = String(u?.email || '').trim().toLowerCase();
              const role = String(u?.role || '').trim().toLowerCase();
              if (!em) return;
              if (role === 'master' || role === 'admin') masterAdminEmails.add(em);
            });
          } catch { /* noop */ }

          await Promise.allSettled(
            targets.map(email => {
              const icon = masterAdminEmails.has(String(email || '').toLowerCase()) ? wdIcon : unitIcon;
              return sendPortalPush({ email, title, body: bodyPreview, tag, data, icon, badge: icon });
            })
          );
        };

        // Não aguarda: mantém latência do POST baixa.
        run().catch(() => { /* noop */ });
      }
    } catch { /* noop */ }

    return res.status(201).json({ ok: true, id: String(doc._id), protocolo: doc.protocolo });
  } catch (e) {
    const st = e && e.status ? Number(e.status) : 500;
    if (st !== 500) return res.status(st).json({ error: String(e.message || 'Erro') });
    console.error('[condominios][POST /api/msg/messages] erro:', e);
...
app.put('/api/msg/admin/settings', express.json({ limit: '200kb' }), async (req, res) => {
  try {
    const ctxUser = getCtxUser(req);
    if (!ctxUser) return res.status(401).json({ error: 'Não autenticado' });
    if (mongoose.connection.readyState !== 1) {
    ...
    const $set = {
      suspender_caixas_pessoais: !!patch.suspender_caixas_pessoais,
      suspender_caixas_grupo: !!patch.suspender_caixas_grupo,
      pessoais_suspensas: patch.pessoais_suspensas,
      pessoais_suspensas_portal: (patch.pessoais_suspensas_portal && patch.pessoais_suspensas_portal.length)
        ? patch.pessoais_suspensas_portal
        : patch.pessoais_suspensas,
      pessoais_suspensas_colaborador: (patch.pessoais_suspensas_colaborador && patch.pessoais_suspensas_colaborador.length)
        ? patch.pessoais_suspensas_colaborador
        : patch.pessoais_suspensas,
      portal_user_perms: Array.isArray(patch.portal_user_perms) ? patch.portal_user_perms : []
    };
    if (patch.permitir_pessoal_para_pessoal === undefined || patch.permitir_pessoal_para_pessoal === null) {
      // mantém valor atual
    } else {
      $set.permitir_pessoal_para_pessoal = !!patch.permitir_pessoal_para_pessoal;
    }

    const updated = await CondMsgSettings.findOneAndUpdate(
...
// Admin: listar mailboxes (inclui inativas)
app.get('/api/msg/admin/mailboxes', async (req, res) => {
  try {
    const ctxUser = getCtxUser(req);
    if (!ctxUser) return res.status(401).json({ error: 'Não autenticado' });
    if (mongoose.connection.readyState !== 1) {
    ...
    const out = (docs || []).map(d => ({
      ...
      ativo: d.ativo !== false,
      link_type: String(d.link_type || ''),
      link_id: d.link_id ? String(d.link_id) : '',
      public: mailboxIsPublic(d)
    })).filter(x => x.id);
...
// Admin: suspender/reativar mailbox (ativo)
app.patch('/api/msg/admin/mailboxes/:id/status', express.json({ limit: '20kb' }), async (req, res) => {
  try {
    const ctxUser = getCtxUser(req);
    if (!ctxUser) return res.status(401).json({ error: 'Não autenticado' });
    if (mongoose.connection.readyState !== 1) {
...
// Admin: listar usuários por unidade (email/nome) para suspensão individual
app.get('/api/msg/admin/users', async (req, res) => {
  try {
    const ctxUser = getCtxUser(req);
    if (!ctxUser) return res.status(401).json({ error: 'Não autenticado' });
    if (mongoose.connection.readyState !== 1) {
    ...
    const warnings = [];
    const addWarn = (code, e) => {
      try {
        const c = String(code || '').trim();
        if (c) warnings.push(c);
        console.warn('[condominios][GET /api/msg/admin/users] warn:', c || 'unknown', String(e?.message || e || ''));
      } catch { /* noop */ }
    };

    const colab = new Map(); // email -> {email,nome,origem}
    // Portal: uma entrada por contexto (ex.: por habitação)
    const portal = new Map(); // key -> {email,nome,origem,habitacao,habitacao_id}

    const portalKey = (email, habitacaoId) => {
      const em = String(email || '').trim().toLowerCase();
      const hid = String(habitacaoId || '').trim();
      return `${em}::${hid}`;
    };
    ...
    try {
      if (User) {
        // Regra alinhada com /api/usuarios/busca.v2: colaborador = possui vínculo de funcionário
        const users = await User.find({
          unidade_id: unitObjectId,
          ativo: { $ne: false },
          funcionario_id: { $ne: null }
        })
          .select('email nome')
          .lean();
        (users || []).forEach(u => {
          const em = String(u?.email || '').trim().toLowerCase();
          if (!isEmailish(em)) return;
          if (!colab.has(em)) colab.set(em, { email: em, nome: String(u?.nome || '').trim(), origem: 'colaborador' });
        });
      }
    } catch (e) { addWarn('users.gestor.colaborador', e); }

    // Usuários do Gestor sem vínculo de funcionário devem ser tratados como Portal (não como colaborador)
    try {
      if (User) {
        const usersPortal = await User.find({
          unidade_id: unitObjectId,
          ativo: { $ne: false },
          $or: [{ funcionario_id: null }, { funcionario_id: { $exists: false } }]
        })
          .select('email nome')
          .lean();
        (usersPortal || []).forEach(u => {
          const em = String(u?.email || '').trim().toLowerCase();
          if (!isEmailish(em)) return;
          const key = portalKey(em, '');
          if (!portal.has(key)) {
            portal.set(key, { email: em, nome: String(u?.nome || '').trim(), origem: 'portal', habitacao: '', habitacao_id: '' });
            return;
          }
          const cur = portal.get(key);
          if (cur && !String(cur.nome || '').trim()) {
            cur.nome = String(u?.nome || '').trim();
          }
        });
      }
    } catch (e) { addWarn('users.gestor.portal', e); }

    // Habitações do condomínio: base para montar labels e mapear moradores/proprietários.
    const habLabelById = new Map();
    const habLabelsByProprietarioId = new Map();
    const habIdsByProprietarioId = new Map();
    const habIds = [];

    try {
      if (CondHabitacao) {
        const habs = await CondHabitacao.find({ unidade_id: unitObjectId, ativa: { $ne: false } })
          .select('_id bloco_id andar_id numero descricao proprietario_id')
          .lean();
        const rows = Array.isArray(habs) ? habs : [];

        const blocoIds = [];
        const andarIds = [];
        for (const h of rows) {
          const id = h?._id ? String(h._id).trim() : '';
          if (!id) continue;
          habIds.push(new mongoose.Types.ObjectId(id));
          const bid = h?.bloco_id ? String(h.bloco_id).trim() : '';
          const aid = h?.andar_id ? String(h.andar_id).trim() : '';
          if (bid && mongoose.isValidObjectId(bid)) blocoIds.push(new mongoose.Types.ObjectId(bid));
          if (aid && mongoose.isValidObjectId(aid)) andarIds.push(new mongoose.Types.ObjectId(aid));
        }

        const [blocos, andares] = await Promise.all([
          (CondBloco && blocoIds.length) ? CondBloco.find({ _id: { $in: blocoIds } }).select('nome').lean() : [],
          (CondAndar && andarIds.length) ? CondAndar.find({ _id: { $in: andarIds } }).select('nome').lean() : []
        ]);
        const blocoNomeById = new Map((Array.isArray(blocos) ? blocos : []).map(b => [String(b?._id), String(b?.nome || '').trim()]));
        const andarNomeById = new Map((Array.isArray(andares) ? andares : []).map(a => [String(a?._id), String(a?.nome || '').trim()]));

        const buildHabLabel = (h) => {
          const parts = [];
          const blocoNome = h?.bloco_id ? (blocoNomeById.get(String(h.bloco_id)) || '') : '';
          const andarNome = h?.andar_id ? (andarNomeById.get(String(h.andar_id)) || '') : '';
          const numero = String(h?.numero || '').trim();
          const desc = String(h?.descricao || '').trim();
          if (blocoNome) parts.push(blocoNome);
          if (andarNome) parts.push(andarNome);
          if (numero) parts.push(numero);
          if (!parts.length && desc) parts.push(desc);
          return parts.join(' · ');
        };

        for (const h of rows) {
          const hid = h?._id ? String(h._id).trim() : '';
          if (!hid) continue;
          const label = buildHabLabel(h) || hid;
          habLabelById.set(hid, label);
          const pid = h?.proprietario_id ? String(h.proprietario_id).trim() : '';
          if (pid) {
            const arr = habLabelsByProprietarioId.get(pid) || [];
            arr.push(label);
            habLabelsByProprietarioId.set(pid, arr);

            const arrIds = habIdsByProprietarioId.get(pid) || [];
            arrIds.push(hid);
            habIdsByProprietarioId.set(pid, arrIds);
          }
        }
      }
    } catch (e) { addWarn('users.cond.habitacoes', e); }

    // Portal: usuários do módulo Condomínios (credenciais/convites do Portal)
    try {
      if (CondUsuario) {
        const condUsers = await CondUsuario.find({ unidade_id: unitObjectId, ativo: { $ne: false }, portal_acesso_ativo: { $ne: false } })
          .select('email nome favoritos.habitacoes')
          .lean();
        (condUsers || []).forEach(u => {
          const em = String(u?.email || '').trim().toLowerCase();
          if (!isEmailish(em)) return;
          const favs = u?.favoritos?.habitacoes;
          const favIds = Array.isArray(favs) ? favs.map(x => String(x || '').trim()).filter(Boolean) : [];

          // Preferência: uma entrada por habitação (quando existir vínculo)
          const favLabels = favIds
            .map(id => ({ id, label: habLabelById.get(id) || '' }))
            .filter(x => x.id && x.label);

          if (favLabels.length) {
            favLabels.slice(0, 40).forEach(x => {
              const key = portalKey(em, x.id);
              if (!portal.has(key)) {
                portal.set(key, { email: em, nome: String(u?.nome || '').trim(), origem: 'portal', habitacao: x.label, habitacao_id: x.id });
                return;
              }
              const cur = portal.get(key);
              if (cur) {
                if (!String(cur.nome || '').trim()) cur.nome = String(u?.nome || '').trim();
                if (!String(cur.habitacao || '').trim()) cur.habitacao = x.label;
                if (!String(cur.habitacao_id || '').trim()) cur.habitacao_id = x.id;
              }
            });
            return;
          }

          // Sem habitação: mantém entrada genérica do Portal para o e-mail
          const key = portalKey(em, '');
          if (!portal.has(key)) portal.set(key, { email: em, nome: String(u?.nome || '').trim(), origem: 'portal', habitacao: '', habitacao_id: '' });
          else {
            const cur = portal.get(key);
            if (cur && !String(cur.nome || '').trim()) cur.nome = String(u?.nome || '').trim();
          }
        });
      }
    } catch (e) { addWarn('users.cond.usuarios', e); }

    // Portal do Morador: moradores (mapeados via habitações, sem depender de morador.unidade_id)
    try {
      if (CondMorador && habIds.length) {
        const moradores = await CondMorador.find({ habitacao_id: { $in: habIds }, ativo: { $ne: false } })
          .select('nome email responsavel_email habitacao_id')
          .lean();
        (moradores || []).forEach(m => {
          const nome = String(m?.nome || '').trim();
          const a = String(m?.email || '').trim().toLowerCase();
          const b = String(m?.responsavel_email || '').trim().toLowerCase();
          const hid = m?.habitacao_id ? String(m.habitacao_id).trim() : '';
          const habLabel = hid ? (habLabelById.get(hid) || hid) : '';

          for (const em of [a, b]) {
            if (!isEmailish(em)) continue;

            if (hid) {
              const key = portalKey(em, hid);
              if (!portal.has(key)) portal.set(key, { email: em, nome, origem: 'portal', habitacao: habLabel, habitacao_id: hid });
              const cur = portal.get(key);
              if (cur && !cur.nome && nome) cur.nome = nome;
              if (cur && habLabel && !String(cur.habitacao || '').trim()) cur.habitacao = habLabel;
              if (cur && hid && !String(cur.habitacao_id || '').trim()) cur.habitacao_id = hid;
            } else {
              const key = portalKey(em, '');
              if (!portal.has(key)) portal.set(key, { email: em, nome, origem: 'portal', habitacao: '', habitacao_id: '' });
              const cur = portal.get(key);
              if (cur && !cur.nome && nome) cur.nome = nome;
            }
          }
    ...
    } catch (e) { addWarn('users.portal.moradores', e); }

    // Portal do Morador: proprietários + vínculo por habitações (proprietario_id)
    try {
      if (CondProprietario) {
        const proprietarios = await CondProprietario.find({ unidade_id: unitObjectId, ativo: { $ne: false } })
          .select('_id nome contato_email')
          .lean();
        (proprietarios || []).forEach(p => {
          const nome = String(p?.nome || '').trim();
          const em = String(p?.contato_email || '').trim().toLowerCase();
          if (!isEmailish(em)) return;
          const pid = p?._id ? String(p._id).trim() : '';

          const habs = pid
            ? (habIdsByProprietarioId.get(pid) || []).map(hid => ({ id: String(hid || '').trim(), label: habLabelById.get(String(hid || '').trim()) || '' }))
            : [];

          const rows = habs.filter(x => x.id && x.label);
          if (rows.length) {
            rows.slice(0, 40).forEach(x => {
              const key = portalKey(em, x.id);
              if (!portal.has(key)) {
                portal.set(key, { email: em, nome, origem: 'portal', habitacao: x.label, habitacao_id: x.id });
                return;
              }
              const cur = portal.get(key);
              if (cur) {
                if (!String(cur.nome || '').trim() && nome) cur.nome = nome;
                if (!String(cur.habitacao || '').trim()) cur.habitacao = x.label;
                if (!String(cur.habitacao_id || '').trim()) cur.habitacao_id = x.id;
              }
            });
          ...
          if (!portal.has(key)) portal.set(key, { email: em, nome, origem: 'portal', habitacao: '', habitacao_id: '' });
            ...
            const cur = portal.get(key);
            if (cur && !String(cur.nome || '').trim() && nome) cur.nome = nome;
          }
        });
      }
    } catch (e) { addWarn('users.portal.proprietarios', e); }

    // Pós-processamento: se um e-mail do Portal possui pelo menos um vínculo por habitação,
    // não deve existir também a linha genérica (habitacao_id vazio), para evitar duplicidade.
    try {
      const portalHasHabByEmail = new Set();
      for (const v of portal.values()) {
        const em = String(v?.email || '').trim().toLowerCase();
        const hid = String(v?.habitacao_id || '').trim();
        if (isEmailish(em) && hid) portalHasHabByEmail.add(em);
      }
      for (const em of portalHasHabByEmail.values()) {
        const genericKey = portalKey(em, '');
        if (portal.has(genericKey)) portal.delete(genericKey);
      }
    } catch (e) { addWarn('users.postprocess.portalDedup', e); }

    const portalRows = Array.from(portal.values());
    const colabRows = Array.from(colab.values());

    const out = [...portalRows, ...colabRows]
      .map(u => ({
        ...
        habitacao_id: String(u?.habitacao_id || '').trim()
      }))
      .filter(u => isEmailish(u.email))
      .sort((a, b) => {
        const an = String(a.nome || a.email);
        const bn = String(b.nome || b.email);
        const c = an.localeCompare(bn);
        if (c !== 0) return c;
        const ao = a.origem === 'portal' ? 0 : 1;
        const bo = b.origem === 'portal' ? 0 : 1;
...
function buildMsgBytesAddFields() {
  return {
    bodyBytes: {
      $add: [
        { $strLenBytes: { $ifNull: ['$body_text', ''] } },
        { $strLenBytes: { $ifNull: ['$body_html', ''] } }
      ]
    },
    anexosBytes: {
      $sum: {
...
function buildDailyKeyExpr() {
  // Mantém bucket diário consistente para a UI.
  // Obs: timezone é importante para evitar “cortar” dias ao redor de meia-noite.
  return {
    $dateToString: {
      format: '%Y-%m-%d',
      date: '$createdAt',
...
async function aggregateMsgSeriesUsers(unidadeId, from, to) {
  const match = {
    ativo: { $ne: false },
    unidade_id: new mongoose.Types.ObjectId(unidadeId),
    createdAt: { $gte: from, $lte: to }
  };
  const addFields = buildMsgBytesAddFields();
  const dayKey = buildDailyKeyExpr();
  ...
  (recvAgg || []).forEach(r => {
    const d = String(r?._id || '').trim();
    if (!d) return;
    const cur = byDay.get(d) || { date: d, sentCount: 0, sentBytes: 0, receivedCount: 0, receivedBytes: 0 };
...
async function aggregateMsgSeriesMailboxes(unidadeId, from, to) {
  const match = {
    ativo: { $ne: false },
    unidade_id: new mongoose.Types.ObjectId(unidadeId),
    createdAt: { $gte: from, $lte: to }
...
// Admin: métricas por usuário
app.get('/api/msg/admin/metrics/users', async (req, res) => {
  try {
    const ctxUser = getCtxUser(req);
    if (!ctxUser) return res.status(401).json({ error: 'Não autenticado' });
    if (mongoose.connection.readyState !== 1) {
    ...
    (sentAgg || []).forEach(r => {
      ...
      if (!k) return;
      byEmail.set(k, { email: k, sentCount: Number(r?.sentCount) || 0, sentBytes: Number(r?.sentBytes) || 0, receivedCount: 0, receivedBytes: 0 });
    });
    (recvAgg || []).forEach(r => {
      const k = String(r?._id || '').trim().toLowerCase();
      if (!k) return;
      const cur = byEmail.get(k) || { email: k, sentCount: 0, sentBytes: 0, receivedCount: 0, receivedBytes: 0 };
      cur.receivedCount = Number(r?.receivedCount) || 0;
      cur.receivedBytes = Number(r?.receivedBytes) || 0;
      byEmail.set(k, cur);
    });
    ...
    const data = Array.from(byEmail.values())
      .map(r => ({
        ...r,
        sentBytesHuman: bytesToHuman(r.sentBytes),
        receivedBytesHuman: bytesToHuman(r.receivedBytes)
      }))
      .sort((a, b) => (b.sentCount + b.receivedCount) - (a.sentCount + a.receivedCount));

    const totals = data.reduce((acc, r) => {
      acc.sentCount += r.sentCount;
      acc.receivedCount += r.receivedCount;
      acc.sentBytes += r.sentBytes;
      acc.receivedBytes += r.receivedBytes;
      return acc;
    }, { sentCount: 0, receivedCount: 0, sentBytes: 0, receivedBytes: 0 });
    ...
    return res.json({
      ...
      from,
      to,
      totals: { ...totals, sentBytesHuman: bytesToHuman(totals.sentBytes), receivedBytesHuman: bytesToHuman(totals.receivedBytes) },
      data
    });
  } catch (e) {
...
// Admin: série temporal (totais) por usuário
app.get('/api/msg/admin/metrics/timeseries/users', async (req, res) => {
  try {
    const ctxUser = getCtxUser(req);
    if (!ctxUser) return res.status(401).json({ error: 'Não autenticado' });
    if (mongoose.connection.readyState !== 1) {
...
// Admin: métricas por mailbox
app.get('/api/msg/admin/metrics/mailboxes', async (req, res) => {
  try {
    const ctxUser = getCtxUser(req);
    if (!ctxUser) return res.status(401).json({ error: 'Não autenticado' });
    if (mongoose.connection.readyState !== 1) {
    ...
    const ids = new Set();
    (sentAgg || []).forEach(r => ids.add(String(r?._id || '').trim()));
    (recvAgg || []).forEach(r => ids.add(String(r?._id || '').trim()));

    const realIds = Array.from(ids).filter(id => id && id !== 'pessoal' && mongoose.isValidObjectId(id));
    const mailboxDocs = realIds.length
      ? await CondMsgMailbox.find({ _id: { $in: realIds.map(id => new mongoose.Types.ObjectId(id)) } })
        .select('_id name type ativo link_type link_id public')
        .lean()
      : [];
    const mailboxMap = new Map((mailboxDocs || []).map(d => [String(d._id), d]));

    const byId = new Map();
    (sentAgg || []).forEach(r => {
      const k = String(r?._id || '').trim();
      if (!k) return;
      byId.set(k, { mailboxId: k, sentCount: Number(r?.sentCount) || 0, sentBytes: Number(r?.sentBytes) || 0, receivedCount: 0, receivedBytes: 0 });
    });
    (recvAgg || []).forEach(r => {
      const k = String(r?._id || '').trim();
      if (!k) return;
      const cur = byId.get(k) || { mailboxId: k, sentCount: 0, sentBytes: 0, receivedCount: 0, receivedBytes: 0 };
      cur.receivedCount = Number(r?.receivedCount) || 0;
      cur.receivedBytes = Number(r?.receivedBytes) || 0;
      byId.set(k, cur);
    });

    const data = Array.from(byId.values()).map(r => {
      if (r.mailboxId === 'pessoal') {
        return {
          ...r,
          mailboxName: 'Pessoal',
          mailboxType: 'pessoal',
      ...
      const mb = mailboxMap.get(r.mailboxId) || null;
      return {
        ...r,
        mailboxName: mb ? String(mb.name || '') : 'Caixa',
        mailboxType: mb ? String(mb.type || 'grupo') : 'grupo',
        ativo: mb ? (mb.ativo !== false) : true,
        link_type: mb ? String(mb.link_type || '') : '',
        link_id: mb && mb.link_id ? String(mb.link_id) : '',
        public: mb ? mailboxIsPublic(mb) : false,
        sentBytesHuman: bytesToHuman(r.sentBytes),
        receivedBytesHuman: bytesToHuman(r.receivedBytes)
      };
    }).sort((a, b) => (b.sentCount + b.receivedCount) - (a.sentCount + a.receivedCount));
...
// Admin: série temporal (totais) por mailbox
app.get('/api/msg/admin/metrics/timeseries/mailboxes', async (req, res) => {
  try {
    const ctxUser = getCtxUser(req);
    if (!ctxUser) return res.status(401).json({ error: 'Não autenticado' });
    if (mongoose.connection.readyState !== 1) {
...
// API: listar marcadores
app.get('/api/msg/markers', async (req, res) => {
  try {
    let ctxUser = getCtxUser(req);
    if (!ctxUser) return res.status(401).json({ error: 'Não autenticado' });
    if (mongoose.connection.readyState !== 1) {
    ...
    if (scope.mailboxId !== 'pessoal') {
      ...
      // Caixa pública no Portal: leitura/envio podem ser permitidos, mas marcadores são classificação interna.
      // Portanto, não lista marcadores para não-membros.
      try {
        const ref = String(req.headers?.referer || req.headers?.Referer || '').toLowerCase();
        const fromPortal = String(req.headers['x-wdg-portal'] || '').trim() === '1' || ref.includes('/portal-morador');
        if (!admin && fromPortal && mailboxIsPublic(mailbox) && !mailboxIsMember(mailbox, ctxUser)) {
    ...
    // Dedup por nome (evita duplicados por owner legado vs owner atual)
    const out = [];
    const seen = new Set();
    for (const d of (docs || [])) {
    ...
    console.error('[condominios][GET /api/msg/markers] erro:', e);
    return res.status(500).json({ error: 'Falha ao listar marcadores' });
  }
});

// API: criar marcador
app.post('/api/msg/markers', express.json(), async (req, res) => {
  try {
    let ctxUser = getCtxUser(req);
    if (!ctxUser) return res.status(401).json({ error: 'Não autenticado' });
    if (mongoose.connection.readyState !== 1) {
    ...
    try {
      ...
      if (existing) {
        return res.json({ id: String(existing._id || ''), nome: existing.nome || '', cor: String(existing.cor || '').trim() });
      }
    } catch { /* noop */ }

    const createdBy = getUserIdentityKey(ctxUser) || String(ctxUser?.nome || ctxUser?.name || '').trim();

    // Marcador pessoal deve ser único por usuário (não fragmentar por origem/habitação).
    ...
    try {
      ...
      return res.status(201).json({ id: String(doc._id || ''), nome: doc.nome || '', cor: String(doc.cor || '').trim() });
    } catch (err) {
      ...
      if (/E11000/.test(msg)) {
        ...
        if (existing) return res.json({ id: String(existing._id || ''), nome: existing.nome || '', cor: String(existing.cor || '').trim() });
      }
      throw err;
    }
  } catch (e) {
    console.error('[condominios][POST /api/msg/markers] erro:', e);
    return res.status(500).json({ error: 'Falha ao criar marcador' });
  }
});

// API: remover marcador (soft delete) + remove o marcador das mensagens do escopo
app.delete('/api/msg/markers/:id', async (req, res) => {
  try {
    let ctxUser = getCtxUser(req);
    if (!ctxUser) return res.status(401).json({ error: 'Não autenticado' });
    if (mongoose.connection.readyState !== 1) {
    ...
    const doc = await CondMsgMarker.findOne({ _id: id, mailbox_id: scope.mailboxId, owner: ownerQuery, ativo: { $ne: false } });
    if (!doc) return res.status(404).json({ error: 'Marcador não encontrado' });

    const marker = normalizeMarkerName(doc.nome);

    // Soft-delete em todos os owners compatíveis com o mesmo nome (evita ressurgir duplicado)
    ...
    if (marker) {
      ...
      await CondMsgMessage.updateMany(
        ...
        { arrayFilters: [{ 'st.mailbox_id': scope.mailboxId, 'st.owner': ownersToPull.length > 1 ? { $in: ownersToPull } : ownersToPull[0] }] }
      );
    }

    return res.json({ ok: true });
  } catch (e) {
    console.error('[condominios][DELETE /api/msg/markers/:id] erro:', e);
    return res.status(500).json({ error: 'Falha ao remover marcador' });
  }
});

// API: listar mensagens (Entrada/Saída/Arquivo/Lixeira)
app.get('/api/msg/messages', async (req, res) => {
  try {
    let ctxUser = getCtxUser(req);
    if (!ctxUser) return res.status(401).json({ error: 'Não autenticado' });
    if (mongoose.connection.readyState !== 1) {
    ...
    startCondMsgRetentionInterval();
    maybeRunCondMsgRetention('GET /api/msg/messages').catch(() => { /* noop */ });
    ...
    const baseEmailForOwnerMatch = (() => {
      try {
        if (scope.mailboxId !== 'pessoal') return '';
        // Preferir o base e-mail do owner canônico; fallback para ctxUser.
    ...
    const isOwnerMatch = (ownerVal) => {
      ...
      // Compat: quando o ownerKey do request vem com outro sufixo (portal/colab),
      // ainda queremos achar o state correto pelo mesmo e-mail base.
      return baseMatches(ownerLower);
    };

    // Em caixa pessoal, usamos vários candidatos para *acesso* (compat com legados),
    // mas o estado de leitura (lida_em) deve preferir o owner canônico do usuário atual.
    // Caso contrário, um state de outro candidato (ex.: responsável) pode marcar como "lida"
    // e a UI perde a diferença entre lidas/não lidas.
    const canonicalOwnerLower = (() => {
      try {
        if (scope.mailboxId !== 'pessoal') return String(scope.owner || '').trim().toLowerCase();

        // Preferir o ownerKey do scope quando ele é estável (email/id) e inclui contexto
        // (Portal/Habitação/Colab). Isso garante que `lida_em` seja lido do mesmo state
        // que o POST /read salva no banco.
        const ownerExact = String(scope.owner || '').trim().toLowerCase();
        if (ownerKeyBaseEmailLower(ownerExact) || mongoose.isValidObjectId(ownerExact) || isEmailish(ownerExact)) {
          return ownerExact;
        }
        ...
        add(ctxUser?.contatoEmail);
        (ownerCandidatesLower || []).forEach(add);
        // Prioriza chaves com contexto, depois email.
        const portalHabPick = picks.find((s) => s.includes('::portal::hab:'));
        if (portalHabPick) return portalHabPick;
        const portalPick = picks.find((s) => s.includes('::portal'));
        if (portalPick) return portalPick;
        const colabPick = picks.find((s) => s.includes('::colab'));
        if (colabPick) return colabPick;
        const emailPick = picks.find((s) => isEmailish(s));
        if (emailPick) return emailPick;
        return picks[0] || String(scope.owner || '').trim().toLowerCase();
      } catch {
        return (ownerCandidatesLower && ownerCandidatesLower[0]) ? ownerCandidatesLower[0] : String(scope.owner || '').trim().toLowerCase();
      }
    })();

    // Portal + caixa pública (não-membro): ações/pastas devem ser por usuário.
    // Só ativamos isso para caixas de grupo quando detectarmos allowPublicPortal adiante.
    let allowPublicPortalForMailbox = false;
    let publicPortalOwnerLower = '';

    const findScopeState = (doc) => {
      const states = Array.isArray(doc?.states) ? doc.states : [];
      const mailboxOk = (s) => String(s?.mailbox_id || '').trim() === scope.mailboxId;
      const notDeleted = (s) => !s?.excluida_em;

      // Caixas de grupo: normalmente o state é por mailbox_id (owner pode ser vazio/legado).
      // Exceção: Portal em caixa pública (não-membro) usa state por usuário.
      if (scope.mailboxId !== 'pessoal') {
        if (allowPublicPortalForMailbox && publicPortalOwnerLower) {
          const exact = states.find(s => mailboxOk(s)
            && notDeleted(s)
            && String(s?.owner || '').trim().toLowerCase() === publicPortalOwnerLower);
          if (exact) return exact;
        }
        const firstActive = states.find(s => mailboxOk(s) && notDeleted(s));
        if (firstActive) return firstActive;
        return states.find(mailboxOk);
      }

      if (scope.mailboxId === 'pessoal' && canonicalOwnerLower) {
        const exact = states.find(s => mailboxOk(s) && String(s?.owner || '').trim().toLowerCase() === canonicalOwnerLower && notDeleted(s));
        if (exact) return exact;
      }

      // Se houver múltiplos states do mesmo usuário (ex.: portal::hab + portal + colab),
      // prefira um com lida_em para não regressar para “não lida” após refresh.
      if (scope.mailboxId === 'pessoal' && baseEmailForOwnerMatch) {
        const byBase = states.filter(s => mailboxOk(s) && baseMatches(s?.owner));
        const readOne = byBase.find(s => !!s?.lida_em && notDeleted(s));
        if (readOne) return readOne;
        const firstActive = byBase.find(notDeleted);
        if (firstActive) return firstActive;
      }

      const direct = states.find(s => mailboxOk(s) && isOwnerMatch(s?.owner) && notDeleted(s));
      if (direct) return direct;
      return states.find(s => mailboxOk(s) && isOwnerMatch(s?.owner));
    };

    const hasOnlyDeletedStateForScope = (doc) => {
      const states = Array.isArray(doc?.states) ? doc.states : [];
      const scoped = (scope.mailboxId !== 'pessoal')
        ? (allowPublicPortalForMailbox && publicPortalOwnerLower
          ? states.filter(s => String(s?.mailbox_id || '').trim() === scope.mailboxId
            && String(s?.owner || '').trim().toLowerCase() === publicPortalOwnerLower)
          : states.filter(s => String(s?.mailbox_id || '').trim() === scope.mailboxId))
        : states.filter(s => String(s?.mailbox_id || '').trim() === scope.mailboxId && isOwnerMatch(s?.owner));
      return scoped.length > 0 && scoped.every(s => !!s?.excluida_em);
    };

    // Evita falha silenciosa quando a identidade não está disponível (Portal/cookie reduzido).
    try {
      const fromPortal = String(req.headers['x-wdg-portal'] || '').trim() === '1';
      if (fromPortal && scope.mailboxId === 'pessoal' && ownerCandidatesLower.length === 0 && !nameFallback) {
        return res.status(400).json({ error: 'Portal: não foi possível determinar o identificador do usuário para carregar a caixa pessoal.' });
      }
    } catch { /* noop */ }

    let mailboxDoc = null;
    if (scope.mailboxId !== 'pessoal') {
      mailboxDoc = await loadMailboxOrThrow(scope.mailboxId);
      if (!(await canAccessMsgMailbox(mailboxDoc, ctxUser, req))) return res.status(403).json({ error: 'Acesso negado' });
      ...
      if (mailboxIsGroup(mailboxDoc)) {
        ...
        if (allowPublicPortalForMailbox) {
          try {
            publicPortalOwnerLower = String(getMsgIdentityKey(ctxUser, req) || '').trim().toLowerCase();
          } catch { /* noop */ }
        }
    ...
    // Paginação (UI): sempre exibir e suportar 10/25/50/100 por página.
    const pageSizeRaw = Number(req.query.pageSize || req.query.page_size || req.query.perPage || req.query.per_page || req.query.limit || 25) || 25;
    ...
    // Scan limit: por compatibilidade, este endpoint aplicava vários filtros em memória.
    // Para paginação por página, garantimos um teto alto mas seguro.
    const scanLimit = Math.min(5000, Math.max(pageReq * pageSize, 250));

    const filter = { ativo: { $ne: false } };

    // Pasta: saida = by from; demais = por destinatário
    if (folder === 'saida') {
      filter.from_mailbox_id = scope.mailboxId;
      if (scope.mailboxId === 'pessoal') {
        // Compat: mensagens antigas podem ter salvo from_owner de forma não-canônica.
        // Para evitar "sumir" da Saída, aceitamos match por:
        // - from_owner (candidatos)
        // - states (owner candidatos)
        // - createdBy (candidatos) quando era salvo como email/id
        const ownerExact = String(scope.owner || '').trim().toLowerCase();
        const isPortalHabScoped = !!(fromPortal && ownerExact.includes('::portal::hab:'));
        const baseEmail = ownerKeyBaseEmailLower(ownerExact);
        const basePrefixRx = (isEmailish(baseEmail)) ? new RegExp('^' + escapeRegExp(baseEmail) + '::') : null;
        const cands = (() => {
          // Portal: não use candidatos amplos (podem incluir outros moradores da mesma habitação)
          // para evitar listar envios de terceiros.
          if (fromPortal) {
            const out = [];
            const add = (v) => {
              const s = String(v || '').trim().toLowerCase();
          ...
          if (isPortalHabScoped) {
            return Array.from(new Set([
              ownerExact,
              (baseEmail ? `${baseEmail}::portal` : ''),
              baseEmail
            ].filter(Boolean)));
          }
          return (ownerCandidatesLower && ownerCandidatesLower.length)
            ? ownerCandidatesLower
            : [ownerExact].filter(Boolean);
        })();

        if (fromPortal) {
          // Portal: saída deve listar apenas o que foi ENVIADO pelo usuário atual.
          // Não usar states/createdBy para evitar mensagens recebidas aparecerem na saída.
          filter.$or = [
            { from_owner: cands.length > 1 ? { $in: cands } : cands[0] },
            ...(basePrefixRx ? [{ from_owner: basePrefixRx }] : [])
          ];
        } else {
          // Saída deve listar apenas mensagens ENVIADAS pelo usuário atual.
          // Não use `states` aqui: ele representa o estado do destinatário e pode fazer
          // mensagens recebidas aparecerem na saída.
          filter.$or = [
            { from_owner: cands.length > 1 ? { $in: cands } : cands[0] },
            ...(basePrefixRx ? [{ from_owner: basePrefixRx }] : []),
            { createdBy: { $in: cands } }
      ...
      if (scope.mailboxId === 'pessoal') {
        ...
        if (isPortalHabScoped) {
          const ors = [];
          const baseEmail = ownerKeyBaseEmailLower(ownerExact);
          const portalOwnerCompat = Array.from(new Set([
            ownerExact,
            (baseEmail ? `${baseEmail}::portal` : ''),
            baseEmail
          ].filter(Boolean)));

          // Para Portal por habitação, a separação real vem do ownerKey nos states.
          // Compat: mensagens legadas (antes do hab-scope) podem ter salvo owner como
          // email/email::portal e/ou destinatário apenas em to.email/cc.email.
          ors.push({
            states: {
              $elemMatch: {
                mailbox_id: 'pessoal',
                owner: portalOwnerCompat.length > 1 ? { $in: portalOwnerCompat } : portalOwnerCompat[0]
              }
            }
          });
          if (baseEmail) {
            const rx = new RegExp('^\\s*' + escapeRegExp(baseEmail) + '\\s*$', 'i');
            ors.push({ 'to.email': rx }, { 'cc.email': rx });
          }
          // Legado Portal (sem email): mantém fallback por nome+unidade, se existir.
          if (nameFallback) {
            ors.push({
              unidade_id: nameFallback.unidadeId,
              $or: [
                { 'to.nome': nameFallback.nameRx },
                { 'cc.nome': nameFallback.nameRx },
                { 'to.name': nameFallback.nameRx },
                { 'cc.name': nameFallback.nameRx }
              ]
            });
          }
          if (includeSent) {
            ors.push({ from_mailbox_id: scope.mailboxId, from_owner: portalOwnerCompat.length > 1 ? { $in: portalOwnerCompat } : ownerExact });
          }
          filter.$or = ors;
        } else {
        // Compat: mensagens legadas podem ter espaços no identificador (email ou id) e variações de case.
        ...
        const addOwnerCandidate = (v) => {
          const s = String(v || '').trim().toLowerCase();
          if (!s) return;
          if (!ownerCandidatesLower.includes(s)) ownerCandidatesLower.push(s);
        };
          ...
          const baseEmail = (() => {
            try {
              const fromOwner = ownerKeyBaseEmailLower(String(scope?.owner || '').trim().toLowerCase());
              if (isEmailish(fromOwner)) return fromOwner;
              const em = String(ctxUserForScope?.email || ctxUserForScope?.userEmail || ctxUserForScope?.contato_email || ctxUserForScope?.contatoEmail || '').trim().toLowerCase();
              return isEmailish(em) ? em : '';
            } catch {
              return '';
            }
          ...
          // Compat: se a chave canônica alternou (ex.: portal com/sem habitação),
          // garanta que ações (marcador/arquivar/fixar) atinjam o state existente,
          // sem criar um novo state com lida_em=null.
          if (isEmailish(baseEmail)) {
            addRecipientCandidate(baseEmail);
            addRecipientCandidate(`${baseEmail}::portal`);
            addRecipientCandidate(`${baseEmail}::colab`);
            addOwnerCandidate(baseEmail);
            addOwnerCandidate(`${baseEmail}::portal`);
            addOwnerCandidate(`${baseEmail}::colab`);
            try {
              const fromPortal2 = String(req.headers['x-wdg-portal'] || '').trim() === '1';
              if (fromPortal2) {
                const habIdRaw = String(ctxUserForScope?.habitacao_id || ctxUserForScope?.habitacaoId || '').trim();
                if (habIdRaw && mongoose.isValidObjectId(habIdRaw)) {
                  addRecipientCandidate(`${baseEmail}::portal::hab:${habIdRaw}`);
                  addOwnerCandidate(`${baseEmail}::portal::hab:${habIdRaw}`);
                }
              }
            } catch { /* noop */ }
          }

        const recipientKeyRxs = recipientKeyCandidatesLower.map(k => new RegExp('^\\s*' + escapeRegExp(k) + '\\s*$', 'i'));
        const ors = [];
        if (recipientKeyRxs.length) ors.push({ 'to.email': { $in: recipientKeyRxs } }, { 'cc.email': { $in: recipientKeyRxs } });
        // Robustez: se existir state criado para este usuário, use-o como critério de acesso.
        // (evita inbox vazia quando o destinatário foi salvo de forma não-canônica).
        if (ownerCandidatesLower.length) {
          ors.push({ states: { $elemMatch: { mailbox_id: 'pessoal', owner: { $in: ownerCandidatesLower } } } });
        }
        if (basePrefixRx) {
          ors.push({ states: { $elemMatch: { mailbox_id: 'pessoal', owner: basePrefixRx } } });
        }

        // Legado Portal: match por nome+unidade quando destinatário não tem email.
        if (nameFallback) {
          ors.push({
            unidade_id: nameFallback.unidadeId,
            $or: [
        ...
        if (includeSent) {
          ors.push({ from_mailbox_id: scope.mailboxId, from_owner: ownerCandidatesLower.length > 1 ? { $in: ownerCandidatesLower } : String(email || '').trim().toLowerCase() });
          if (basePrefixRx) ors.push({ from_mailbox_id: scope.mailboxId, from_owner: basePrefixRx });
        }
        filter.$or = ors;
        }
      } else {
        const ors = [{ 'to.mailboxId': scope.mailboxId }, { 'cc.mailboxId': scope.mailboxId }];
        if (includeSent) ors.push({ from_mailbox_id: scope.mailboxId });
        filter.$or = ors;
      }
    }

    // Data (inputs tipo YYYY-MM)
    const dateFilter = {};
    if (qIni) {
    ...
    // Remove mensagens excluídas definitivamente pelo usuário/escopo.
    ...
    docs = (docs || []).map(d => {
      ...
      if (!st) {
        // Marca para que filtros de status (lidas/não lidas) não contem "fantasmas".
        // O state sintetizado é útil para renderização (pasta/ações), mas não é confiável
        // para contagem de não lidas no badge.
        fake.__wdg_state_injected = true;

        const canonicalOwner = (scope.mailboxId === 'pessoal')
          ? (canonicalOwnerLower || ownerCandidatesLower[0] || scope.owner)
          : (allowPublicPortalForMailbox && publicPortalOwnerLower
            ? publicPortalOwnerLower
            : String(scope.owner || '').trim().toLowerCase());
        fake.states = [...fake.states, { mailbox_id: scope.mailboxId, owner: canonicalOwner, lida_em: null, arquivada_em: null, arquivada_de: '', lixeira_em: null, lixeira_de: '', excluida_em: null, fixada_em: null, marcadores: [] }];
      }
      return fake;
    });
    ...
    // Saída (caixa pessoal): garantir que nada "vaze" por compat/estado.
    if (folder === 'saida' && scope.mailboxId === 'pessoal') {
      docs = (docs || []).filter(d => {
        try {
          const mb = String(scope?.mailboxId || '').trim() || 'pessoal';
          const fromMailboxId = String(d?.from_mailbox_id || '').trim();
          if (fromMailboxId !== mb) return false;
          ...
          return !!(createdByLower && Array.isArray(ownerCandidatesLower) && ownerCandidatesLower.includes(createdByLower));
        } catch {
          return false;
        }
      });
    ...
    // Entrada: não deve exibir mensagens enviadas pelo próprio usuário (exceto envio para si mesmo)
    if (folder === 'entrada') {
      docs = docs.filter(d => {
        try {
          const mb = String(scope?.mailboxId || '').trim() || 'pessoal';
          const fromMailboxId = String(d?.from_mailbox_id || '').trim();
          const sentFromThisScope = (fromMailboxId === mb)
            && (mb !== 'pessoal' || isOwnerMatch(String(d?.from_owner || '').trim().toLowerCase()));
    ...
    const debugUnread = String(req.query.debugUnread || req.query.debug_unread || '').trim() === '1';
    let debugUnreadSample = null;

    // status lidas/não lidas
    if (qStatus === 'lidas' || qStatus === 'nao_lidas' || qStatus === 'não_lidas') {
      const wantRead = qStatus === 'lidas';

      const hasReadAccessForScope = (doc) => {
        try {
          const access = Array.isArray(doc?.acessos) ? doc.acessos : [];
          if (!access.length) return false;
          const mb = String(scope?.mailboxId || '').trim();
          if (!mb) return false;

          // Para caixa pessoal, owner pode variar em legados (id/email). Usamos isOwnerMatch/baseMatches.
          // Para caixas de grupo, normalmente owner é vazio; então isso não altera o comportamento.
          return access.some((a) => {
            const amb = String(a?.mailbox_id || a?.mailboxId || '').trim();
            if (amb !== mb) return false;
            const aOwner = String(a?.owner || '').trim().toLowerCase();
            if (!aOwner) return false;
            if (isOwnerMatch(aOwner)) return true;
            return false;
          });
        } catch {
          return false;
      ...
      const isReadForScope = (doc) => {
        try {
          ...
          // Compat: em caixa pessoal, podem existir múltiplos states do MESMO usuário
          // (ex.: email, email::portal, email::portal::hab:*). Se qualquer um deles
          // estiver marcado como lido, considere como lido para efeito de contagem.
          if (scope.mailboxId === 'pessoal' && baseEmailForOwnerMatch) {
            const states = Array.isArray(doc?.states) ? doc.states : [];
            const mailboxOk = (s) => String(s?.mailbox_id || '').trim() === 'pessoal';
            const active = (s) => mailboxOk(s) && !s?.excluida_em && baseMatches(s?.owner);
            const byBase = states.filter(active);
            if (byBase.some(s => !!s?.lida_em)) return true;
          }

          // Último fallback: se já há evento de acesso para este escopo, trate como lida.
          return hasReadAccessForScope(doc);
        } catch {
          return false;
        }
      };
      ...
      docs = docs.filter(d => {
        // Se o state foi sintetizado, não use para contagem/consulta por status.
        // Caso contrário, qualquer mensagem legada sem state vira "não lida".
        if (d && d.__wdg_state_injected) return false;
        const read = isReadForScope(d);
        return wantRead ? read : !read;
      });

      // Debug aprofundado: captura uma amostra dos itens que estão sendo contados.
      if (debugUnread && !wantRead) {
        try {
          const sample = (Array.isArray(docs) ? docs : []).slice(0, 25);
          debugUnreadSample = sample.map((d) => {
            const st = findScopeState(d);
            const states = Array.isArray(d?.states) ? d.states : [];
            const mailboxId = String(scope.mailboxId || '').trim();
            const mailboxOk = (s) => String(s?.mailbox_id || '').trim() === mailboxId;
            const notDeleted = (s) => !s?.excluida_em;
            const scoped = states.filter(s => mailboxOk(s));
            const scopedActive = scoped.filter(notDeleted);

            const anyReadByBase = (() => {
              try {
                if (scope.mailboxId !== 'pessoal' || !baseEmailForOwnerMatch) return false;
                const mailboxOkPessoal = (s) => String(s?.mailbox_id || '').trim() === 'pessoal';
                const active = (s) => mailboxOkPessoal(s) && !s?.excluida_em && baseMatches(s?.owner);
                const byBase = states.filter(active);
                return byBase.some(s => !!s?.lida_em);
              } catch {
                return false;
              }
            })();

            const readByState = !!st?.lida_em;
            const readByAccess = hasReadAccessForScope(d);
            const hasAnyScopedState = scoped.length > 0;
            const hasAnyActiveScopedState = scopedActive.length > 0;
            const hasAnyBaseState = (() => {
              try {
                if (scope.mailboxId !== 'pessoal' || !baseEmailForOwnerMatch) return false;
                return states.some(s => String(s?.mailbox_id || '').trim() === 'pessoal' && baseMatches(s?.owner));
              } catch {
                return false;
              }
            })();
            ...
            const reasons = [];
            if (!st) reasons.push('no_state_for_scope');
            if (st && !readByState) reasons.push('state_not_marked_read');
            if (scope.mailboxId === 'pessoal' && baseEmailForOwnerMatch && !anyReadByBase) reasons.push('no_read_state_by_base');
            if (!readByAccess) reasons.push('no_access_read_event');
            if (!hasAnyScopedState) reasons.push('no_state_for_mailbox');
            if (hasAnyScopedState && !hasAnyActiveScopedState) reasons.push('only_deleted_states_for_mailbox');
            if (scope.mailboxId === 'pessoal' && baseEmailForOwnerMatch && !hasAnyBaseState) reasons.push('no_state_for_base_email');

            const owners = states
              .filter(s => String(s?.mailbox_id || '').trim() === String(scope.mailboxId || '').trim())
              .slice(0, 8)
              .map(s => ({
                owner: String(s?.owner || '').trim().toLowerCase(),
                lida: !!s?.lida_em,
                lidaEm: s?.lida_em || null,
                arquivada: !!s?.arquivada_em,
                lixeira: !!s?.lixeira_em,
                excluida: !!s?.excluida_em
              }));

            return {
              id: String(d?._id || ''),
              threadRootId: String(d?.thread_root_id || d?._id || ''),
              createdAt: (inferDocCreatedAt(d) || null),
              fromMailboxId: String(d?.from_mailbox_id || ''),
              fromOwner: String(d?.from_owner || ''),
              resolvedState: st ? {
                owner: String(st?.owner || '').trim().toLowerCase(),
                lida: !!st?.lida_em,
                lidaEm: st?.lida_em || null,
                arquivada: !!st?.arquivada_em,
                lixeira: !!st?.lixeira_em,
                excluida: !!st?.excluida_em
              } : null,
              anyReadByBase,
              readByState,
              readByAccess,
              hasAnyScopedState,
              hasAnyActiveScopedState,
              hasAnyBaseState,
              reasons,
              stateOwners: owners,
              injected: !!d?.__wdg_state_injected
            };
          });
        } catch {
          debugUnreadSample = null;
        }
      }
    }

    if (qDe) {
      const q = qDe.toLowerCase();
      docs = docs.filter(d => String(d.from_mailbox_name || '').toLowerCase().includes(q) || String(d.createdBy || '').toLowerCase().includes(q));
    }
    if (qPara) {
      const q = qPara.toLowerCase();
      docs = docs.filter(d => {
        const members = [...(d.to || []), ...(d.cc || [])];
        return members.some(m => String(m?.name || m?.nome || m?.email || '').toLowerCase().includes(q));
      });
    }

    if (qMarker) {
      const key = String(qMarker || '').trim().toLowerCase();
      docs = docs.filter(d => {
        const st = findScopeState(d);
        const tags = Array.isArray(st?.marcadores) ? st.marcadores : [];
        return tags.map(t => String(t || '').trim().toLowerCase()).includes(key);
      });
    } else if (qSemMarcador === '1' || qSemMarcador === 'true') {
      docs = docs.filter(d => {
        const st = findScopeState(d);
        const tags = Array.isArray(st?.marcadores) ? st.marcadores : [];
    ...
    // Ordena fixadas no topo
    docs.sort((a, b) => {
      const sa = findScopeState(a);
      const sb = findScopeState(b);
      const pa = sa?.fixada_em ? new Date(sa.fixada_em).getTime() : 0;
      const pb = sb?.fixada_em ? new Date(sb.fixada_em).getTime() : 0;
      if (pa && pb) return pb - pa;
      if (pa && !pb) return -1;
      if (!pa && pb) return 1;
      const ta = inferDocCreatedAt(a)?.getTime?.() || 0;
      const tb = inferDocCreatedAt(b)?.getTime?.() || 0;
      return tb - ta;
    });

    // =========================================================
    // Agrupamento por conversa (thread)
    // =========================================================
    // A UI da Caixa de Mensagem é orientada a conversa: respostas/encaminhamentos
    // devem aparecer dentro do mesmo item (com contador), não como linhas separadas.
    // Para isso, colapsamos a listagem para 1 doc por `thread_root_id` (ou `_id`).
    // Mantemos a ordem já calculada (fixadas + cronológica), escolhendo o primeiro
    // item visto em cada thread como representante (normalmente o mais recente).
    try {
      const list = Array.isArray(docs) ? docs : [];
      const seen = new Set();
      const grouped = [];
      for (const d of list) {
        const rootKey = String(d?.thread_root_id || d?._id || '').trim();
        if (!rootKey) continue;
      ...
      // best-effort: se falhar, mantém listagem original
    }

    // Paginação após filtros/ordenação (para refletir fixadas, regras de pasta e agrupamento por conversa)
    ...
    try {
      const list = Array.isArray(docs) ? docs : [];
      const rootIds = Array.from(new Set(list.map(d => String(d?.thread_root_id || d?._id || '')).filter(Boolean)));
      if (rootIds.length) {
        const rootObjIds = rootIds.filter(mongoose.isValidObjectId).map(id => new mongoose.Types.ObjectId(id));

        // Filtro de acesso (sem pasta): recebidos (to/cc) OU enviados (from) para este scope.
        const accessFilter = { ativo: { $ne: false } };
        if (scope.mailboxId === 'pessoal') {
          const email = scope.owner;
          const ownerExact = String(scope.owner || '').trim().toLowerCase();
          const isPortalHabScoped = !!(fromPortal && ownerExact.includes('::portal::hab:'));
          ...
          accessFilter.$or = [
            { 'to.mailboxId': scope.mailboxId },
            { 'cc.mailboxId': scope.mailboxId },
        ...
        const related = await CondMsgMessage.find({
          ...
          .select('_id thread_root_id in_reply_to forwarded_from_id')
          .lean();

        const statsByRoot = new Map();
        (related || []).forEach(m => {
          const rootId = String(m?.thread_root_id || m?._id || '').trim();
          if (!rootId) return;
          const cur = statsByRoot.get(rootId) || { count: 0, hasReply: false, hasForward: false };
          cur.count += 1;
          if (m?.in_reply_to) cur.hasReply = true;
          if (m?.forwarded_from_id) cur.hasForward = true;
          statsByRoot.set(rootId, cur);
        });

        docs = list.map(d => {
          const rootId = String(d?.thread_root_id || d?._id || '').trim();
          const st = statsByRoot.get(rootId) || null;
          const isRoot = !d?.thread_root_id; // raiz não tem thread_root_id
          return {
            ...d,
            __thread_count: st?.count || 0,
            __thread_has_reply: !!st?.hasReply,
            __thread_has_forward: !!st?.hasForward,
            __thread_is_root: !!isRoot
          };
        });
      }
    } catch (e) {
      console.warn('[condominios][GET /api/msg/messages] falha ao enriquecer thread:', e?.message || e);
    }

    // Padroniza remetente para caixa pessoal: "Nome (Colaborador)" ou "Nome (Bloco ... - Andar ... - Apartamento ...)"
    // Best-effort: nunca deve quebrar a listagem.
    try {
      const list = Array.isArray(docs) ? docs : [];
      const docById = new Map(list.map(d => [String(d?._id || ''), d]));

      const senderEntries = [];
      const unitIds = new Set();
      const emails = new Set();
      const condUserIds = new Set();

      list.forEach(d => {
        const fromMailboxId = String(d?.from_mailbox_id || '').trim();
        if (fromMailboxId !== 'pessoal') return;
        const unidadeId = d?.unidade_id ? String(d.unidade_id) : '';
        if (!unidadeId || !mongoose.isValidObjectId(unidadeId)) return;

        const fromOwnerRaw = String(d?.from_owner || '').trim().toLowerCase();
        const baseFromOwner = ownerKeyBaseEmailLower(fromOwnerRaw) || '';
        const createdBy = String(d?.createdBy || '').trim().toLowerCase();
        const email = isEmailish(baseFromOwner)
          ? baseFromOwner
          : (isEmailish(fromOwnerRaw) ? fromOwnerRaw : (isEmailish(createdBy) ? createdBy : ''));
        const condUserId = mongoose.isValidObjectId(fromOwnerRaw) ? fromOwnerRaw : '';
        if (!isEmailish(email)) return;

        const msgId = String(d?._id || '');
        if (!msgId) return;

        unitIds.add(unidadeId);
        emails.add(email);
        if (condUserId) condUserIds.add(condUserId);
        senderEntries.push({ msgId, unidadeId, email, condUserId });
      });

      if (senderEntries.length) {
        const unitObjIds = Array.from(unitIds).filter(mongoose.isValidObjectId).map(id => new mongoose.Types.ObjectId(id));
        const emailList = Array.from(emails);

        const [moradores, proprietarios, moradoresByCondUser, proprietariosByCondUser, users] = await Promise.all([
          CondMorador.find({
            unidade_id: { $in: unitObjIds },
            ativo: { $ne: false },
            $or: [
              { email: { $in: emailList } },
              { responsavel_email: { $in: emailList } }
            ]
          })
            .select('unidade_id email responsavel_email nome habitacao_id')
            .lean(),
          CondProprietario.find({ unidade_id: { $in: unitObjIds }, contato_email: { $in: emailList }, ativo: { $ne: false } })
            .select('unidade_id contato_email nome')
            .lean(),
          condUserIds.size
            ? CondMorador.find({ unidade_id: { $in: unitObjIds }, cond_usuario_id: { $in: Array.from(condUserIds).map(id => new mongoose.Types.ObjectId(id)) }, ativo: { $ne: false } })
              .select('unidade_id cond_usuario_id nome habitacao_id')
              .lean()
            : Promise.resolve([]),
          condUserIds.size
            ? CondProprietario.find({ unidade_id: { $in: unitObjIds }, cond_usuario_id: { $in: Array.from(condUserIds).map(id => new mongoose.Types.ObjectId(id)) }, ativo: { $ne: false } })
              .select('unidade_id cond_usuario_id nome')
              .lean()
            : Promise.resolve([]),
          (User ? User.find({ email: { $in: emailList }, ativo: { $ne: false } }).select('email nome name username').lean() : Promise.resolve([]))
        ]);

        const profileByUnitEmail = new Map();
        const profileByUnitCondUser = new Map();
        const userByEmail = new Map();
        const habitacaoIds = new Set();

        (moradores || []).forEach(m => {
          const uid = m?.unidade_id ? String(m.unidade_id) : '';
          const email1 = String(m?.email || '').trim().toLowerCase();
          const email2 = String(m?.responsavel_email || '').trim().toLowerCase();
          if (!uid || (!email1 && !email2)) return;
          const payload = {
            tipo: 'MORADOR',
            nome: String(m?.nome || '').trim(),
            habitacao_id: m?.habitacao_id ? String(m.habitacao_id) : ''
          };
          if (email1) profileByUnitEmail.set(`${uid}::${email1}`, payload);
          if (email2) profileByUnitEmail.set(`${uid}::${email2}`, payload);
          if (m?.habitacao_id) habitacaoIds.add(String(m.habitacao_id));
        });

        (moradoresByCondUser || []).forEach(m => {
          const uid = m?.unidade_id ? String(m.unidade_id) : '';
          const cid = m?.cond_usuario_id ? String(m.cond_usuario_id) : '';
          if (!uid || !cid) return;
          profileByUnitCondUser.set(`${uid}::${cid}`, {
            tipo: 'MORADOR',
            nome: String(m?.nome || '').trim(),
            habitacao_id: m?.habitacao_id ? String(m.habitacao_id) : ''
          });
        ...
        if (propIds.length) {
          ...
          (habs || []).forEach(h => {
            const pid = h?.proprietario_id ? String(h.proprietario_id) : '';
            const uid = h?.unidade_id ? String(h.unidade_id) : '';
            if (!pid || !uid) return;
            const key = `${pid}::${uid}`;
            if (!habByPropUnit.has(key)) habByPropUnit.set(key, h);
          });
        }

        (proprietarios || []).forEach(p => {
          const uid = p?.unidade_id ? String(p.unidade_id) : '';
          const email = String(p?.contato_email || '').trim().toLowerCase();
          if (!uid || !email) return;

          const key = `${uid}::${email}`;
          if (profileByUnitEmail.has(key)) return;

          const pid = p?._id ? String(p._id) : '';
          const hab = pid ? (habByPropUnit.get(`${pid}::${uid}`) || null) : null;

          profileByUnitEmail.set(key, {
            tipo: 'PROPRIETARIO',
            nome: String(p?.nome || '').trim(),
            habitacao_id: hab && hab._id ? String(hab._id) : ''
          });
          if (hab && hab._id) habitacaoIds.add(String(hab._id));
        });

        (proprietariosByCondUser || []).forEach(p => {
          const uid = p?.unidade_id ? String(p.unidade_id) : '';
          const cid = p?.cond_usuario_id ? String(p.cond_usuario_id) : '';
          if (!uid || !cid) return;

          const key = `${uid}::${cid}`;
          if (profileByUnitCondUser.has(key)) return;

          profileByUnitCondUser.set(key, {
            tipo: 'PROPRIETARIO',
            nome: String(p?.nome || '').trim(),
            habitacao_id: ''
          });
        ...
        // Carrega habitação/bloco/andar para montar rótulo
        const habIdList = Array.from(habitacaoIds).filter(mongoose.isValidObjectId);
        const habMap = new Map();
        const blocoIds = new Set();
        const andarIds = new Set();
        ...
        // Injeta display por mensagem
        senderEntries.forEach(ent => {
          const d = docById.get(ent.msgId);
          if (!d) return;

          const prof = profileByUnitEmail.get(`${ent.unidadeId}::${ent.email}`)
            || (ent.condUserId ? (profileByUnitCondUser.get(`${ent.unidadeId}::${ent.condUserId}`) || null) : null);
          const baseNome = (prof && prof.nome)
            ? prof.nome
            : (stripMailboxPrefix(d.from_mailbox_name) || '').trim()
              || (!isEmailish(d.createdBy) ? String(d.createdBy || '').trim() : '')
              || ent.email;

          let suffix = '';
          if (prof && (prof.tipo === 'MORADOR' || prof.tipo === 'PROPRIETARIO')) {
            const hid = prof.habitacao_id ? String(prof.habitacao_id) : '';
            const hab = hid ? (habMap.get(hid) || null) : null;
            const bloco = hab?.bloco_id ? (blocoMap.get(String(hab.bloco_id)) || null) : null;
            const andar = hab?.andar_id ? (andarMap.get(String(hab.andar_id)) || null) : null;
            const habLabel = buildSenderHabitacaoLabel(hab, bloco, andar);
            suffix = habLabel ? ` (${habLabel})` : ' (Habitação)';
          } else if (userByEmail.has(ent.email)) {
            suffix = ' (Colaborador)';
          }

          d.__remetente_display = `${baseNome}${suffix}`;
        });
      }
    } catch (e) {
      console.warn('[condominios][GET /api/msg/messages] falha ao enriquecer remetente:', e?.message || e);
    }

    // Padroniza destinatário quando a listagem estiver exibindo mensagens ENVIADAS
    // (Saída e equivalentes em Arquivo/Lixeira), para exibir nome ao invés de e-mail,
    // seguindo o mesmo padrão da Entrada.
    // Best-effort: nunca deve quebrar a listagem.
    const shouldEnrichDestinatarioDisplay = (d) => {
      try {
        if (!d || typeof d !== 'object') return false;
        if (folder === 'saida') return true;
        if (folder !== 'arquivo' && folder !== 'lixeira') return false;

        const mb = String(scope?.mailboxId || '').trim() || 'pessoal';
        const owner = String(scope?.owner || '').trim().toLowerCase();

        const sentFromThisScope = (String(d.from_mailbox_id || '').trim() === mb)
          && (mb !== 'pessoal' || String(d.from_owner || '').trim().toLowerCase() === owner);

        // Caso especial: usuário enviou para si mesmo (pessoal -> pessoal com o próprio email em To/CC).
        // Para Arquivo/Lixeira, deve ser tratado como recebido (não como enviado).
        const sentToSelf = (() => {
          try {
            if (mb !== 'pessoal') return false;
            if (!sentFromThisScope) return false;
            const listTo = Array.isArray(d.to) ? d.to : [];
            const listCc = Array.isArray(d.cc) ? d.cc : [];
            const isMe = (m) => String(m?.email || '').trim().toLowerCase() === owner;
            return listTo.some(isMe) || listCc.some(isMe);
          } catch {
            return false;
          }
        ...
        let treatAsSent = sentFromThisScope;
        if (sentToSelf) treatAsSent = false;
        return treatAsSent;
      } catch {
        return false;
      }
    ...
    if (shouldEnrichDestinatarioDisplay({})) {
      // noop: apenas garante que a função existe; evita lint/perf regressiva em ambientes que stripping.
    }

    if (shouldEnrichDestinatarioDisplay && (folder === 'saida' || folder === 'arquivo' || folder === 'lixeira')) {
      try {
        const list = Array.isArray(docs) ? docs : [];
        const docById = new Map(list.map(d => [String(d?._id || ''), d]));
        ...
        const firstRecipientEmail = (d) => {
          try {
            const all = [...(Array.isArray(d?.to) ? d.to : []), ...(Array.isArray(d?.cc) ? d.cc : [])].filter(Boolean);
            const first = all[0];
            const type = String(first?.type || '').trim().toLowerCase();
            if (type === 'mailbox' || first?.mailboxId) return '';
            const em = String(first?.email || '').trim().toLowerCase();
            return isEmailish(em) ? em : '';
          } catch {
            return '';
          }
        ...
        list.forEach(d => {
          if (!shouldEnrichDestinatarioDisplay(d)) return;
          const msgId = String(d?._id || '');
          if (!msgId) return;
          const unidadeId = d?.unidade_id ? String(d.unidade_id) : '';
          const email = firstRecipientEmail(d);
          if (!email) return;
          emails.add(email);
          if (unidadeId && mongoose.isValidObjectId(unidadeId)) unitIds.add(unidadeId);
          recipientEntries.push({ msgId, unidadeId, email });
        });
        ...
        if (recipientEntries.length) {
          ...
          const [moradores, proprietarios, condUsuarios, gestorUsers] = await Promise.all([
            unitObjIds.length
              ? CondMorador.find({ unidade_id: { $in: unitObjIds }, email: { $in: emailList }, ativo: { $ne: false } })
                  .select('unidade_id email nome habitacao_id')
                  .lean()
              : CondMorador.find({ email: { $in: emailList }, ativo: { $ne: false } })
                  .select('unidade_id email nome habitacao_id')
                  .lean(),
            unitObjIds.length
              ? CondProprietario.find({ unidade_id: { $in: unitObjIds }, contato_email: { $in: emailList }, ativo: { $ne: false } })
                  .select('unidade_id contato_email nome')
                  .lean()
              : CondProprietario.find({ contato_email: { $in: emailList }, ativo: { $ne: false } })
                  .select('unidade_id contato_email nome')
                  .lean(),
            CondUsuario
              ? CondUsuario.find({ email: { $in: emailList }, ativo: { $ne: false } })
                  .select('unidade_id email nome')
                  .lean()
                  .catch(() => [])
              : Promise.resolve([]),
            User
          ...
          (moradores || []).forEach(m => {
            ...
            const email = String(m?.email || '').trim().toLowerCase();
            if (!uid || !email) return;
            profileByUnitEmail.set(`${uid}::${email}`, {
              tipo: 'MORADOR',
              nome: String(m?.nome || '').trim(),
              habitacao_id: m?.habitacao_id ? String(m.habitacao_id) : ''
            });
          ...
          // Para proprietário, tenta relacionar a uma habitação (best-effort)
          const propIds = (proprietarios || []).map(p => (p && p._id ? String(p._id) : null)).filter(Boolean);
          const habByPropUnit = new Map();
          if (propIds.length && unitObjIds.length) {
            const habs = await CondHabitacao.find({
              unidade_id: { $in: unitObjIds },
              proprietario_id: { $in: propIds.filter(mongoose.isValidObjectId).map(id => new mongoose.Types.ObjectId(id)) },
              ativa: { $ne: false }
          ...
          (proprietarios || []).forEach(p => {
            ...
            const pid = p?._id ? String(p._id) : '';
            const hab = (pid && uid) ? (habByPropUnit.get(`${pid}::${uid}`) || null) : null;
          ...
          // Colaborador (Gestor/Condomínio)
          (condUsuarios || []).forEach(u => {
            const email = String(u?.email || '').trim().toLowerCase();
            const nome = String(u?.nome || '').trim();
            const uid = u?.unidade_id ? String(u.unidade_id) : '';
            if (!email || !nome) return;
            const key = uid && mongoose.isValidObjectId(uid) ? `${uid}::${email}` : `::${email}`;
            if (!profileByUnitEmail.has(key)) {
              profileByUnitEmail.set(key, { tipo: 'COLAB', nome, habitacao_id: '' });
            }
          });
          ...
          recipientEntries.forEach(ent => {
            const d = docById.get(ent.msgId);
            if (!d) return;

            const uid = (ent.unidadeId && mongoose.isValidObjectId(ent.unidadeId)) ? String(ent.unidadeId) : '';
            const keyUnit = uid ? `${uid}::${ent.email}` : '';
            const prof = (keyUnit && profileByUnitEmail.has(keyUnit))
              ? profileByUnitEmail.get(keyUnit)
              : (profileByUnitEmail.get(`::${ent.email}`) || null);

            const baseNome = (prof && prof.nome)
              ? String(prof.nome).trim()
              : ent.email;
            ...
            if (prof && (prof.tipo === 'MORADOR' || prof.tipo === 'PROPRIETARIO')) {
              ...
              const habLabel = buildSenderHabitacaoLabel(hab, bloco, andar);
              suffix = habLabel ? ` (${habLabel})` : ' (Habitação)';
            } else if (prof && prof.tipo === 'COLAB') {
              suffix = ' (Colaborador)';
            }

            // Só override se realmente melhorou (evita trocar "nome" vazio por email).
            const display = `${baseNome}${suffix}`.trim();
            if (display && display !== ent.email) {
              d.__destinatario_display = display;
            } else if (prof && prof.tipo && baseNome && baseNome !== ent.email) {
              d.__destinatario_display = baseNome;
            }
          });
        }
      } catch (e) {
        console.warn('[condominios][GET /api/msg/messages] falha ao enriquecer destinatario:', e?.message || e);
      }
    }

    const items = (Array.isArray(docs) ? docs : []).map(d => toMessageListItem(d, scope, folder)).filter(Boolean);

    // Debug (sem dados sensíveis): ajuda a diagnosticar inbox vazio por identidade/filtro.
    const debug = String(req.query.debug || '').trim() === '1' || debugUnread;
    if (debug) {
      const fromPortal = String(req.headers['x-wdg-portal'] || '').trim() === '1';
      const injectedCount = (Array.isArray(docs) ? docs : []).filter(d => !!d?.__wdg_state_injected).length;

      const redactEmailish = (val) => {
        try {
          ...
          const domainHead = domain.split('.').slice(-2).join('.');
      ...
      return res.json({
        ...
        pageSize,
        pages,
        ...(debugUnread ? { debugUnreadSample: (debugUnreadSample || []) } : {}),
        meta: {
          msgApiBuild: '2026-01-25_debugUnread_v2',
          fromPortal,
          folder,
          mailboxId: scope.mailboxId,
          qStatus,
          hasEmailishOwner: isEmailish(scope.owner),
          ownerRedacted: redactEmailish(scope.owner),
          ownerCandidatesRedacted: ownerCandidatesLower.slice(0, 6).map(redactEmailish),
          emailCandidatesRedacted: portalEmailCandidatesLower.slice(0, 6).map(redactEmailish),
          ownerCandidatesCount: ownerCandidatesLower.length,
          emailCandidatesCount: portalEmailCandidatesLower.length,
          nameFallbackEnabled: !!nameFallback,
          injectedStateCount: injectedCount,
          mongoReadyState: mongoose.connection.readyState,
          effectiveSkipDb: !!getEffectiveSkipDb(req),
          scanLimit
        }
    ...
    return res.json({ ok: true, items, total, page, pageSize, pages });
  } catch (e) {
    ...
    try {
      ...
      console.error('[condominios][GET /api/msg/messages] erro detalhado:', errMsg, '\nRequest info:', JSON.stringify(reqInfo, null, 2));
    } catch (logErr) {
      console.error('[condominios][GET /api/msg/messages] erro ao logar erro:', logErr);
    }
    if (isMongoOfflineError(e)) return respondDbOffline(res, req);
    // Retorna mensagem de erro detalhada para facilitar debug (remover em produção se necessário)
    return res.status(500).json({ error: 'Falha ao listar mensagens', details: (e && e.message) || String(e) });
  }
});

// API: ações em lote (arquivar/marcar/fixar/excluir)
app.post('/api/msg/messages/actions', express.json(), async (req, res) => {
  try {
    const ctxUser = getCtxUser(req);
    if (!ctxUser) return res.status(401).json({ error: 'Não autenticado' });
    if (mongoose.connection.readyState !== 1) {
    ...
    const fromFolderRaw = String(req.body?.fromFolder || req.body?.from_folder || req.body?.folder || '').trim().toLowerCase();
    const fromFolder = (fromFolderRaw === 'entrada' || fromFolderRaw === 'saida' || fromFolderRaw === 'arquivo') ? fromFolderRaw : '';
    ...
    if (scope.mailboxId !== 'pessoal') {
      ...
      if (mailboxIsGroup(mailboxDoc)) {
        ...
        if (allowPublicPortalForMailbox) {
          try {
            publicPortalOwnerLower = String(getMsgIdentityKey(ctxUserForScope, req) || '').trim().toLowerCase();
          } catch { /* noop */ }
        }
        const perms = mailboxGetUserPerms(mailboxDoc, ctxUser);
        // Caixa pública no Portal: permitir apenas ações que afetem SOMENTE o state do usuário.
        // Isso dá ao morador a capacidade de arquivar/mover para lixeira/fixar para si mesmo,
        // sem impactar o restante da caixa pública.
        if (allowPublicPortal) {
          const allowed = new Set(['archive', 'trash', 'restore', 'unarchive', 'pin']);
          if (!allowed.has(action)) return res.status(403).json({ error: 'Acesso negado' });
        }

        const canAdminMailbox = admin || mailboxHasPerm(perms, 'administrar');

        if ((action === 'trash' || action === 'delete') && !(canAdminMailbox || mailboxHasPerm(perms, 'excluirMensagem'))) {
          return res.status(403).json({ error: 'Sem permissão para excluir mensagens nesta caixa.' });
        }

        if (action === 'marker' && !(canAdminMailbox || mailboxHasPerm(perms, 'gerenciarMarcador'))) {
          return res.status(403).json({ error: 'Sem permissão para gerenciar marcadores nesta caixa.' });
        }
      }
    }
    ...
    // aplicamos a ação a TODOS os states daquela mailbox_id.
    // Exceção: Portal em caixa pública (não-membro) -> aplicar SOMENTE ao state do usuário.
    const groupTargetStates = (doc) => {
      try {
        if (!doc || scope.mailboxId === 'pessoal') return [];
        const states = Array.isArray(doc?.states) ? doc.states : [];
        // Portal em caixa pública (não-membro): state por usuário.
        if (allowPublicPortalForMailbox && publicPortalOwnerLower) {
          const targets = states.filter(s => String(s?.mailbox_id || '').trim() === String(scope.mailboxId || '').trim()
            && !s?.excluida_em
            && String(s?.owner || '').trim().toLowerCase() === publicPortalOwnerLower);
          if (targets.length) return targets;
          return [ensureMessageState(doc, { mailboxId: scope.mailboxId, owner: publicPortalOwnerLower })].filter(Boolean);
        }

        const targets = states.filter(s => String(s?.mailbox_id || '').trim() === String(scope.mailboxId || '').trim() && !s?.excluida_em);
        if (targets.length) return targets;
        // Se não existe state, cria um padrão para a mailbox.
        return [ensureMessageState(doc, { mailboxId: scope.mailboxId, owner: '' })].filter(Boolean);
      } catch {
        return [ensureMessageState(doc, { mailboxId: scope.mailboxId, owner: (allowPublicPortalForMailbox && publicPortalOwnerLower) ? publicPortalOwnerLower : '' })].filter(Boolean);
      }
    };

    // Caixa pessoal pode ter múltiplos states por compatibilidade (email vs id, portal/colab, etc).
    // Se a action atualizar um state diferente daquele que a listagem (GET /api/msg/messages) prioriza,
    // o usuário vê “não saiu da pasta”. Aqui, alinhamos a seleção do state com a mesma heurística da listagem.
    const personalStateResolver = (() => {
      try {
        if (scope.mailboxId !== 'pessoal') return null;
        ...
        addPick(scope.owner);
        addPick(ctxUserForScope?.email);
        addPick(ctxUserForScope?.userEmail);
        addPick(ctxUserForScope?.contato_email);
        addPick(ctxUserForScope?.contatoEmail);
        addPick(ctxUserForScope?.cond_usuario_id);
        addPick(ctxUserForScope?.condUsuarioId);
        addPick(ctxUserForScope?._id);
        ...
        const baseEmailForOwnerMatch = (() => {
          try {
            const ownerExact = String(scope.owner || '').trim().toLowerCase();
            const baseFromOwner = ownerKeyBaseEmailLower(ownerExact) || normalizeEmailKey(ownerExact);
            if (isEmailish(baseFromOwner)) return baseFromOwner;
            const em = String(ctxUserForScope?.email || ctxUserForScope?.userEmail || ctxUserForScope?.contato_email || ctxUserForScope?.contatoEmail || '').trim().toLowerCase();
            return isEmailish(em) ? em : '';
          } catch {
            return '';
        ...
        const isOwnerMatch = (ownerVal) => {
          const ownerLower = String(ownerVal || '').trim().toLowerCase();
          if (!ownerLower) return false;
          if (ownerSet.has(ownerLower)) return true;
          return baseMatches(ownerLower);
        };
        ...
        const canonicalOwnerLower = (() => {
          try {
            ...
            const emailPick = picks.find((s) => isEmailish(s));
            if (emailPick) return emailPick;
            return picks[0] || ownerExact;
          } catch {
            return (ownerCandidatesLower && ownerCandidatesLower[0]) ? ownerCandidatesLower[0] : String(scope.owner || '').trim().toLowerCase();
          }
        })();

        const canonicalScopeForState = { ...scope, owner: canonicalOwnerLower };

        const getTargetStates = (doc) => {
          const states = Array.isArray(doc?.states) ? doc.states : [];
          const matches = states.filter(s => String(s?.mailbox_id || '').trim() === 'pessoal' && isOwnerMatch(s?.owner) && !s?.excluida_em);
          if (matches.length) return matches;
          // Se não existir state compatível, crie exatamente no owner canônico usado pela listagem.
          return [ensureMessageState(doc, canonicalScopeForState)].filter(Boolean);
        };

        return { isOwnerMatch, canonicalOwnerLower, canonicalScopeForState, getTargetStates };
      } catch {
        return null;
      }
    })();
    ...
    // =========================================================
    // Ações em lote na UI são por conversa (thread)
    // =========================================================
    // O GET /api/msg/messages colapsa a listagem para 1 item por thread_root_id.
    // Se aplicarmos ações apenas no "representante" selecionado, a conversa pode
    // continuar aparecendo (o representante muda para outra mensagem do mesmo thread).
    // Para manter o comportamento "selecionou e aplicou", expandimos o alvo para
    // todos os docs do thread nas ações que alteram pasta/estado.
    const actionsByThread = new Set(['trash', 'delete', 'archive', 'unarchive', 'restore', 'marker', 'pin']);
    let docIdsToAct = validIds;
    let selectedThreadRootIdsLower = []; // strings
    if (actionsByThread.has(action)) {
      try {
        const seed = await CondMsgMessage.find({ _id: { $in: validIds }, ativo: { $ne: false } })
          .select('_id thread_root_id')
          .lean();

        const rootIds = Array.from(new Set((seed || []).map(d => String(d?.thread_root_id || d?._id || '').trim()).filter(Boolean)));
        selectedThreadRootIdsLower = rootIds.map(s => String(s || '').trim()).filter(Boolean);
        const rootObjIds = rootIds.filter(mongoose.isValidObjectId).map(id => new mongoose.Types.ObjectId(id));
        ...
        if (rootObjIds.length) {
          ...
          const expandedIds = Array.from(new Set((related || []).map(d => String(d?._id || '').trim()).filter(mongoose.isValidObjectId)));
          if (expandedIds.length) docIdsToAct = expandedIds;
        }
      } catch {
        // best-effort: se falhar, mantém somente os ids enviados pelo cliente
      }
    }
    ...
    const hasStateAccess = (doc) => {
      try {
        if (scope.mailboxId !== 'pessoal') return false;
        if (!ownerCandidatesLower.length) return false;
        const states = Array.isArray(doc?.states) ? doc.states : [];
        return states.some(s => String(s?.mailbox_id || '').trim() === 'pessoal' && ownerCandidatesLower.includes(String(s?.owner || '').trim().toLowerCase()) && !s?.excluida_em);
      } catch {
        return false;
      }
    };

    let allowedDocs = (docs || []).filter(d => {
      try {
        if (admin) return true;
        if (isScopeSender(d, scope)) return true;
        if (hasScopeAsRecipient(d, scope)) return true;
        return hasStateAccess(d);
      } catch {
        return false;
      }
    });

    // Exclusão definitiva (na lixeira): ao expandir threads, podemos pegar mensagens
    // do mesmo thread que NÃO estão na lixeira deste usuário/escopo. Nesse caso,
    // não deve dar erro; apenas ignore-as.
    if (action === 'delete') {
      try {
        const isInTrashForScope = (d) => {
          const targets = (scope.mailboxId !== 'pessoal')
            ? groupTargetStates(d)
            : ((scope.mailboxId === 'pessoal' && personalStateResolver)
              ? personalStateResolver.getTargetStates(d)
              : [ensureMessageState(d, scope)].filter(Boolean));
          return targets.some(st => st && st.lixeira_em);
        };
        allowedDocs = (allowedDocs || []).filter(isInTrashForScope);
      } catch { /* noop */ }
    }

    if (!allowedDocs.length) return res.status(403).json({ error: 'Acesso negado' });

    const rootKeyOf = (d) => {
      try {
        return String(d?.thread_root_id || d?._id || '').trim();
      } catch {
        return '';
      }
    };

    const getTargetsForDoc = (d) => {
      return (scope.mailboxId !== 'pessoal')
        ? groupTargetStates(d)
        : ((scope.mailboxId === 'pessoal' && personalStateResolver)
          ? personalStateResolver.getTargetStates(d)
          : [ensureMessageState(d, scope)].filter(Boolean));
    ...
    // Limite de fixadas: máximo 10 por conversa (thread) por mailbox/owner (não conta itens na lixeira)
    async function canPinMore() {
      // calcula quantas novas conversas seriam fixadas
      const effectiveOwnerLower = (scope.mailboxId !== 'pessoal' && allowPublicPortalForMailbox && publicPortalOwnerLower)
        ? publicPortalOwnerLower
        : String(scope.owner || '').trim().toLowerCase();

      const ownerMatch = (scope.mailboxId === 'pessoal' && ownerCandidatesLower.length)
        ? { $in: ownerCandidatesLower }
        : effectiveOwnerLower;
      ...
      const alreadyPinned = await CondMsgMessage.find({
        ativo: { $ne: false },
        'states.mailbox_id': scope.mailboxId,
        'states.owner': ownerMatch,
        'states.fixada_em': { $type: 'date' }
      }).select('_id thread_root_id states').lean();

      const pinnedThreadKeys = new Set();
      (alreadyPinned || []).forEach(d => {
        const rootKey = String(d?.thread_root_id || d?._id || '').trim();
        if (!rootKey) return;
        const st = (Array.isArray(d.states) ? d.states : []).find(s => {
          const mb = String(s?.mailbox_id || '').trim();
          const ow = String(s?.owner || '').trim().toLowerCase();
          if (mb !== String(scope.mailboxId || '').trim()) return false;
          if (scope.mailboxId === 'pessoal' && ownerCandidatesLower.length) return ownerCandidatesLower.includes(ow);
          return ow === effectiveOwnerLower;
        }) || null;
        if (st?.fixada_em && !st?.lixeira_em) pinnedThreadKeys.add(rootKey);
      });

      // Considera apenas as threads selecionadas na UI (representantes)
      const selectedSet = new Set((selectedThreadRootIdsLower || []).map(s => String(s || '').trim()).filter(Boolean));
      const byRoot = new Map();
      for (const d of allowedDocs) {
        const rk = rootKeyOf(d);
        if (!rk) continue;
        if (selectedSet.size && !selectedSet.has(rk)) continue;
        const arr = byRoot.get(rk) || [];
        arr.push(d);
        byRoot.set(rk, arr);
      }
      ...
      const isPinnedDoc = (doc) => {
        try {
          const targets = getTargetsForDoc(doc);
          return targets.some(st => st && st.fixada_em && !st.lixeira_em);
        } catch {
          return false;
        }
      };

      let wouldPinThreads = 0;
      let wouldUnpinThreads = 0;
      for (const [rk, arr] of byRoot.entries()) {
        const anyPinned = arr.some(isPinnedDoc);
        const allPinned = arr.length > 0 && arr.every(isPinnedDoc);
        if (allPinned) {
          if (anyPinned) wouldUnpinThreads++;
          continue;
        }
        // Se não está totalmente fixada, a ação "pin" vai fixar a conversa.
        if (!anyPinned) wouldPinThreads++;
      }

      const currentPinned = pinnedThreadKeys.size;
      const finalPinned = Math.max(0, currentPinned - wouldUnpinThreads) + wouldPinThreads;
      return finalPinned <= 10;
    }

    if (action === 'pin') {
      const ok = await canPinMore();
      if (!ok) return res.status(400).json({ error: 'Limite de 10 mensagens fixadas atingido.' });
    }

    // Valida limite de marcadores antes de gravar (para evitar updates parciais)
    if (action === 'marker' && marker) {
      for (const d of allowedDocs) {
        const targets = getTargetsForDoc(d);
        for (const st of targets) {
          if (!st) continue;
          const tags = Array.isArray(st.marcadores) ? st.marcadores : [];
          const has = tags.includes(marker);
          if (!has && tags.length >= 3) {
            return res.status(400).json({ error: 'Limite de 3 marcadores por mensagem.' });
          }
        }
      }
    }

    // Para "pin" queremos um toggle consistente por conversa: se a conversa inteira
    // estiver fixada, desafixa; caso contrário, fixa tudo (exceto itens na lixeira).
    let pinModeByRoot = null; // Map<rootKey, 'pin'|'unpin'>
    if (action === 'pin') {
      try {
        const selectedSet = new Set((selectedThreadRootIdsLower || []).map(s => String(s || '').trim()).filter(Boolean));
        const byRoot = new Map();
        ...
        pinModeByRoot = new Map();
        for (const [rk, arr] of byRoot.entries()) {
          const allPinned = arr.length > 0 && arr.every(isPinnedDoc);
          pinModeByRoot.set(rk, allPinned ? 'unpin' : 'pin');
        }
      } catch {
        pinModeByRoot = null;
      }
    ...
    // Exclusão definitiva: só permitido se já estiver na lixeira (evita remoção acidental)
    if (action === 'delete') {
      for (const d of allowedDocs) {
        const targets = (scope.mailboxId !== 'pessoal')
          ? groupTargetStates(d)
          : ((scope.mailboxId === 'pessoal' && personalStateResolver)
            ? personalStateResolver.getTargetStates(d)
            : [ensureMessageState(d, scope)].filter(Boolean));
        // Compat: caixa pessoal pode ter múltiplos states (email vs id, portal/colab etc).
        // Basta que UM dos states do escopo esteja na lixeira para permitir a exclusão.
        if (targets.every(st => !st || !st.lixeira_em)) {
          return res.status(400).json({ error: 'A mensagem precisa estar na lixeira para exclusão definitiva.' });
        }
      }
    }

    // Restaurar: só permitido se estiver na lixeira
    if (action === 'restore') {
      for (const d of allowedDocs) {
        const targets = (scope.mailboxId !== 'pessoal')
          ? groupTargetStates(d)
          : ((scope.mailboxId === 'pessoal' && personalStateResolver)
            ...
            : [ensureMessageState(d, scope)].filter(Boolean));
        if (targets.every(st => !st || !st.lixeira_em)) {
          return res.status(400).json({ error: 'A mensagem não está na lixeira.' });
        }
      }
    }

    // Desarquivar: só permitido se estiver no arquivo e não na lixeira
    if (action === 'unarchive') {
      for (const d of allowedDocs) {
        const targets = (scope.mailboxId !== 'pessoal')
          ? groupTargetStates(d)
          : ((scope.mailboxId === 'pessoal' && personalStateResolver)
            ? personalStateResolver.getTargetStates(d)
            : [ensureMessageState(d, scope)].filter(Boolean));
        // Basta que UM state esteja arquivado (e não na lixeira) para permitir.
        if (targets.every(st => !st || !st.arquivada_em || st.lixeira_em)) {
          return res.status(400).json({ error: 'A mensagem não está no arquivo.' });
        }
      }
    }
    ...
    for (const d of allowedDocs) {
      const rootKey = rootKeyOf(d);
      const targetList = (scope.mailboxId !== 'pessoal')
        ? groupTargetStates(d)
        : ((scope.mailboxId === 'pessoal' && personalStateResolver)
          ? personalStateResolver.getTargetStates(d)
          : [ensureMessageState(d, scope)].filter(Boolean));

      for (const st of targetList) {
        if (!st) continue;

        if (action === 'archive') {
          if (fromFolder) st.arquivada_de = fromFolder;
          st.arquivada_em = now;
          st.lixeira_em = null;
          st.lixeira_de = '';
        } else if (action === 'trash') {
          if (fromFolder) st.lixeira_de = fromFolder;
          st.lixeira_em = now;
          st.arquivada_em = null;
          st.arquivada_de = '';
          st.fixada_em = null;
        } else if (action === 'restore') {
          const src = String(st.lixeira_de || '').trim().toLowerCase();
          st.lixeira_em = null;
          st.lixeira_de = '';
          st.fixada_em = null;
          st.arquivada_em = (src === 'arquivo') ? now : null;
          st.arquivada_de = '';
          ...
          const mode = (pinModeByRoot && rootKey) ? (pinModeByRoot.get(rootKey) || 'pin') : 'pin';
          if (st.lixeira_em) {
            st.fixada_em = null;
          } else {
            st.fixada_em = (mode === 'unpin') ? null : now;
          }
        } else if (action === 'marker') {
          if (!marker) continue;
          const tags = Array.isArray(st.marcadores) ? st.marcadores : [];
          if (tags.includes(marker)) {
            st.marcadores = tags.filter(t => t !== marker);
          } else {
            st.marcadores = [...tags, marker];
          }
        } else if (action === 'delete') {
          // Exclusão definitiva: marca tombstone no state para não reaparecer.
          st.excluida_em = now;
          st.lixeira_em = st.lixeira_em || now;
          st.lixeira_de = st.lixeira_de || String(fromFolder || st.lixeira_de || '').trim().toLowerCase();
          st.fixada_em = null;
        } else {
      ...
      // Não remover states no delete: evita reaparecer na caixa de entrada.

      // Garantia de persistência: mudanças em arrays/subdocs podem não ser detectadas
      // em alguns cenários (especialmente quando states é criado/mesclado on-the-fly).
      try { if (typeof d.markModified === 'function') d.markModified('states'); } catch { /* noop */ }

      await d.save();

      // Exclusão definitiva: se a mensagem ficou sem states e foi desativada,
      // remova anexos do Blob (best-effort) para não deixar URLs públicas vivas.
      if (action === 'delete' && d.ativo === false) {
        await maybeDeleteMsgBlobs(d);
      }
    }

    return res.json({ ok: true });
  } catch (e) {
    console.error('[condominios][POST /api/msg/messages/actions] erro:', e);
    return res.status(500).json({ error: 'Falha ao aplicar ação' });
  }
});
