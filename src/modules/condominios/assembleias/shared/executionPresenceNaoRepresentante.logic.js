export async function executionPresenceNaoRepresentanteLogic({ req, res, shadow = false, deps }) {
  const {
    PRESENCE_ROLE,
  ...
  if (!router || typeof router.handle !== 'function') {
    return {
      status: 500,
      body: { ok: false, error: 'Falha ao registrar não representante' }
    };
  }
  ...
  return router.handle(
    ...
    res,
    () => res.status(500).json({ ok: false, error: 'Falha ao registrar não representante' })
  );
}