...
export async function unidadesCluster(req, res) {
	try {
		...
		if (/^[0-9a-fA-F]{24}$/.test(unidade_id)) unidadeBase = await Unidade.findById(unidade_id).lean();
		if (!unidadeBase) unidadeBase = await Unidade.findOne({ codigo: unidade_id }).lean();
		...
		if (!isPrivileged) {
			...
			const baseDentroEscopo = await Unidade.exists(filtroBase);
			if (!baseDentroEscopo) return res.json({ ok: true, total: 0, unidades: [] });
		}

		// Monta cluster: base + filiais diretas
		const subunidades = await Unidade.find({ unidade_principal_id: unidadeBase._id }).lean();
		let todas = [unidadeBase, ...subunidades];
		// Caso não privilegiado, filtra pelo escopo
		if (condAcessiveis) {
			const acessiveis = await Unidade.find(condAcessiveis).select('_id').lean();
