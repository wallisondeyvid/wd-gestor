...
function runDiagnosticoBiometriaCore(){ const devices = listarDispositivos(); const resultados = devices.map(d=>{ const info={ vendorId:d.vendorId, productId:d.productId, path:d.path, product:d.product||'', manufacturer:d.manufacturer||'', usagePage:d.usagePage, usage:d.usage }; const lower=(info.product+' '+info.manufacturer).toLowerCase(); if(/hidi2c|intel|synaptics|goodix|validity|windows hello/.test(lower)) info.possivelIntegrado=true; else info.possivelIntegrado=false; try { const dev=new HID.HID(d.path); dev.close(); info.opened=true; info.error=null; } catch(e){ info.opened=false; info.error=e.message; if(/access|denied|permission/i.test(e.message)) info.sugestao='Executar servidor como administrador ou dispositivo bloqueado pelo Windows Biometric Framework (integrado).'; else if(/cannot open device with path/i.test(e.message)) info.sugestao='Dispositivo monopolizado por outro driver (ex: Windows Hello).'; } return info; }); return { sistema:process.platform, qtd:devices.length, resultados }; }
