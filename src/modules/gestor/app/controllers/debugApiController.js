...
export async function userByEmail(req,res){ try { const { email } = req.params; if(!email) return badRequest(res,'Email obrigatório'); const user = await User.findOne({ email: email.toLowerCase() }).lean(); if(!user) return notFound(res,'Usuário não encontrado'); return ok(res, { _id:user._id, email:user.email, role:user.role, ativo:user.ativo, unidade_id:user.unidade_id }); } catch(e){ return serverError(res,e); } }
export async function userByCpf(req,res){ try { const { cpf } = req.params; if(!cpf) return badRequest(res,'CPF obrigatório'); const clean = cpf.replace(/\D/g,''); const user = await User.findOne({ cpf: clean }).lean(); if(!user) return notFound(res,'Usuário não encontrado'); return ok(res, { _id:user._id, email:user.email, role:user.role, ativo:user.ativo, unidade_id:user.unidade_id }); } catch(e){ return serverError(res,e); } }
export async function testUnidades(req,res){ try { const unidades = await Unidade.find({}).lean(); const unidadePrincipal = await Unidade.findOne({ is_principal: true }).lean(); return ok(res, { total_unidades: unidades.length, unidade_principal: unidadePrincipal ? { id:unidadePrincipal._id, nome:unidadePrincipal.nome, is_principal:unidadePrincipal.is_principal } : null, unidades: unidades.map(u=>({ id:u._id, nome:u.nome, is_principal:u.is_principal })) }); } catch(e){ return serverError(res,e); } }
export async function removeWrongMaster(req,res){ try { if(!req.user?.isMaster) return badRequest(res,'Acesso negado'); const wrongEmail = 'wallisondeyvdi13@gmail.com'; const errado = await User.findOne({ email: wrongEmail }); if(!errado) return ok(res, { removed:false, message:'Usuário incorreto não encontrado' }); await User.deleteOne({ _id: errado._id }); return ok(res, { removed:true, message:'Usuário incorreto removido' }); } catch(e){ return serverError(res,e); } }
