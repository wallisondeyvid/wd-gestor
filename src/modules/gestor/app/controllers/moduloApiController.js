...
export async function listarModulos(req,res){ try { const result = await listModulosOwnerService({ userRole: req.user?.role, activeUnitId: resolveActiveUnitId(req), authContext: req?.session?.gestorAuthContext || null, requestUser: req.user || null }); return ok(res, result.modulos); } catch(e){ console.error('[API MODULOS][listar] Erro:', e); return serverError(res, e); } }
