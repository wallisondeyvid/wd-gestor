...
import { getModuloByIdRead, listModulosRead } from '#modules/gestor/app/services/ModuloReadService.js';
...
export async function listarModulos(req,res){ try { const modulos = await listModulosRead({ user: req.user, activeUnitId: resolveActiveUnitId(req) }); return ok(res, modulos); } catch(e){ console.error('[API MODULOS][listar] Erro:', e); return serverError(res, e); } }
export async function obterModulo(req,res){ try { const modulo = await getModuloByIdRead(req.params.id); if(!modulo) return notFound(res,'Módulo não encontrado'); return ok(res, modulo); } catch(e){ console.error('[API MODULOS][get] Erro:', e); return serverError(res, e); } }
