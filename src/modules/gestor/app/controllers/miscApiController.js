...
function matchesRequestedUnit({ requestedUnit, requestedUnitRaw, clusterUnit } = {}) { const requestedRaw = normalizeUnitId(requestedUnitRaw).toLowerCase(); const requestedId = normalizeUnitId(requestedUnit?._id).toLowerCase(); const requestedCodigo = normalizeUnitId(requestedUnit?.codigo).toLowerCase(); const clusterId = normalizeUnitId(clusterUnit?._id || clusterUnit?.id).toLowerCase(); const clusterCodigo = normalizeUnitId(clusterUnit?.codigo).toLowerCase(); if(!clusterId && !clusterCodigo) return false; return (requestedId && requestedId === clusterId) || (requestedCodigo && requestedCodigo === clusterCodigo) || (requestedRaw && (requestedRaw === clusterId || requestedRaw === clusterCodigo)); }
...
export async function obterClusterUnidades(req,res){ try { const { unidade_id } = req.query; if(!unidade_id) return ok(res,{ unidades: [] }); const requestedUnitRaw = normalizeUnitId(unidade_id); const isPrivileged = isPrivilegedGestorUser(req.user); let clusterAnchor = ''; if(!isPrivileged){ const scopedUnitId = getScopedUnitId(req); if(!scopedUnitId) return ok(res,{ unidades: [] }); const scopedCluster = await resolveClusterAnchor(scopedUnitId); if(!scopedCluster.base || !scopedCluster.anchor) return ok(res,{ unidades: [] }); clusterAnchor = scopedCluster.anchor; } const requestedCluster = await resolveClusterAnchor(requestedUnitRaw); if(!requestedCluster.base) return ok(res,{ unidades: [] }); if(isPrivileged){ clusterAnchor = requestedCluster.anchor; if(!clusterAnchor) return ok(res,{ unidades: [] }); } const unidades = await findClusterUnidadesByAnchorLean(clusterAnchor); if(!isPrivileged){ const requestedUnitAllowed = (unidades || []).some((clusterUnit) => matchesRequestedUnit({ requestedUnit: requestedCluster.base, requestedUnitRaw, clusterUnit })); if(!requestedUnitAllowed) return ok(res,{ unidades: [] }); } return ok(res,{ unidades }); } catch(e){ return serverError(res,e); } }
