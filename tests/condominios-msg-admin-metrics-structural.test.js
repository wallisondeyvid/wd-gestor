...
test('GETs administrativos de metrics usam helper canônico mínimo de unidade e autorização', async () => {
  const source = await readSource();
  assert.match(source, /function prepareMsgAdminSettingsContext\(/);

  for (const routeSignature of routes) {
    const routeBlock = extractRouteBlock(source, routeSignature);
    assert.match(
      routeBlock,
    ...
    assert.match(routeBlock, /const st = e && e\.status \? Number\(e\.status\) : 500;/);
    assert.match(routeBlock, /if \(st !== 500\) return res\.status\(st\)\.json\(\{ error: String\(e\.message \|\| 'Erro'\) \}\);/);
  }
});