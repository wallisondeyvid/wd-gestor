...
const TEST_SESSION_SEED_ENDPOINT = '/__tests__/seed-feedback-my-detail-session';
  ...
  app.get(TEST_SESSION_SEED_ENDPOINT, async (req, res) => {
    try {
      ...
      req.session.user = {
        ...
        nome: user.nome || 'Feedback My Detail Contract User',
...
async function createTestUser({ role, marker }) {
  ...
  const email = `${marker}.${role}.${unique}@feedback.my-detail.contract.test`;
  ...
  return User.create({
    ...
    senha,
    cpf,
    nome: `Feedback My Detail Contract ${role}`,
    role,
    ativo: true,
...
async function createFeedbackDoc({ unidadeId, creatorUser, mensagem, omitUnit = false }) {
  const payload = {
    ...
    origem: {
      ...
      userAgent: 'feedback-my-detail-runtime-contract',
...
test('GET /gestor/api/feedback/meus/:feedbackId sem sessão retorna 401 JSON', async () => {
...
test('GET canônico do widget detail retorna 200 para o criador em escopo', async () => {
  await withHarness(async ({ app }) => {
    const marker = `feedback_my_detail_${Date.now()}_${nextCounter()}`;
    ...
    const feedbackId = await createFeedbackDoc({
      ...
      creatorUser,
      mensagem: 'feedback próprio da unidade A para widget detail',
    });
...
test('GET canônico do widget detail traduz feedback da mesma pessoa fora da unidade ativa para 404', async () => {
  await withHarness(async ({ app }) => {
    const marker = `feedback_my_detail_${Date.now()}_${nextCounter()}`;
    const creatorUser = await createTestUser({ role: 'user', marker });
    const creatorAgent = await seedAuthenticatedAgent(app, creatorUser, { unidadeId: FEEDBACK_UNIT_A });
    const feedbackId = await createFeedbackDoc({
      ...
      creatorUser,
      mensagem: 'feedback próprio da unidade B fora do escopo A',
    });
...
test('GET canônico do widget detail traduz feedback inexistente para 404', async () => {
  await withHarness(async ({ app }) => {
    const marker = `feedback_my_detail_${Date.now()}_${nextCounter()}`;
    const creatorUser = await createTestUser({ role: 'user', marker });
    const creatorAgent = await seedAuthenticatedAgent(app, creatorUser, { unidadeId: FEEDBACK_UNIT_A });
...
test('GET canônico do widget detail preserva a validação 400 para id malformado', async () => {
  await withHarness(async ({ app }) => {
    const marker = `feedback_my_detail_${Date.now()}_${nextCounter()}`;
    const creatorUser = await createTestUser({ role: 'user', marker });
    const creatorAgent = await seedAuthenticatedAgent(app, creatorUser, { unidadeId: FEEDBACK_UNIT_A });
