...
test('GET/POST/PATCH/DELETE /condominios/api/msg/groups fora do pessoal sem sessão preservam 401 json', async () => {
  const { app, close } = await createServer({ skipDb: true });
  const teardownGuard = installTeardownSuppression();

  try {
    const endpoints = [
      { method: 'get', url: '/condominios/api/msg/groups?mailboxId=507f1f77bcf86cd799439050' },
      { method: 'post', url: '/condominios/api/msg/groups', body: { mailboxId: '507f1f77bcf86cd799439050', name: 'Novo Grupo' } },
      { method: 'patch', url: '/condominios/api/msg/groups/507f1f77bcf86cd799439051', body: { name: 'Editado' } },
      { method: 'delete', url: '/condominios/api/msg/groups/507f1f77bcf86cd799439051' },
    ];
...
test('groups fora do pessoal permitem GET e write-side para membro com gerenciarGrupos e bloqueiam membro sem permissão de groups', async () => {
  const teardownGuard = installTeardownSuppression();
  ...
  await withEnv({ MONGO_MEMORY: '1', WDG_FLAG_GESTOR_AUTH_CONTEXT_RESOLVER: undefined }, async () => {
    ...
    try {
      ...
      const getRes = await agent
        .get(`/condominios/api/msg/groups?mailboxId=${manageableMailbox._id}`)
        .set('Accept', 'application/json')
        .set('Connection', 'close');
      ...
      const patchRes = await agent
        .patch(`/condominios/api/msg/groups/${editableGroup._id}`)
        .set('Accept', 'application/json')
        .set('Connection', 'close')
      ...
      const deleteRes = await agent
        .delete(`/condominios/api/msg/groups/${editableGroup._id}`)
        .set('Accept', 'application/json')
        .set('Connection', 'close');
      ...
      assert.equal(forbiddenPostRes.status, 403, JSON.stringify(forbiddenPostRes.body));
      assert.equal(forbiddenPostRes.body?.error, 'Acesso negado');

      const forbiddenPatchRes = await agent
        .patch(`/condominios/api/msg/groups/${forbiddenGroup._id}`)
        .set('Accept', 'application/json')
        .set('Connection', 'close')
        .send({ name: 'Nao Pode Editar' });

      assert.equal(forbiddenPatchRes.status, 403, JSON.stringify(forbiddenPatchRes.body));
      assert.equal(forbiddenPatchRes.body?.error, 'Acesso negado');

      const forbiddenDeleteRes = await agent
        .delete(`/condominios/api/msg/groups/${forbiddenGroup._id}`)
        .set('Accept', 'application/json')
        .set('Connection', 'close');

      assert.equal(forbiddenDeleteRes.status, 403, JSON.stringify(forbiddenDeleteRes.body));
      assert.equal(forbiddenDeleteRes.body?.error, 'Acesso negado');
      assert.equal((await CondMsgGroup.findById(forbiddenGroup._id).lean())?.ativo, true);
    } finally {
      try {
        await closeWithTeardownGuard(close, teardownGuard);
      } finally {
...
test('groups fora do pessoal permitem bypass administrativo global em mailbox de outra unidade sem membership explícita', async () => {
  const teardownGuard = installTeardownSuppression();
  ...
  await withEnv({ MONGO_MEMORY: '1', WDG_FLAG_GESTOR_AUTH_CONTEXT_RESOLVER: undefined }, async () => {
    ...
    try {
      ...
      const getRes = await agent
        .get(`/condominios/api/msg/groups?mailboxId=${remoteMailbox._id}`)
        .set('Accept', 'application/json')
        .set('Connection', 'close');
      ...
      const patchRes = await agent
        .patch(`/condominios/api/msg/groups/${remoteGroup._id}`)
        .set('Accept', 'application/json')
        .set('Connection', 'close')
      ...
      const deleteRes = await agent
        .delete(`/condominios/api/msg/groups/${remoteGroup._id}`)
        .set('Accept', 'application/json')
        .set('Connection', 'close');
