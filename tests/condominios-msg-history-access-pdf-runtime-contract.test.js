...
function buildPortalCookieHeader({
  secret = 'portal-history-access-pdf-runtime-test-secret',
...
function messageDoc({
  ...
  return {
    ...
    cc: [],
    assunto: 'Teste PDF historico',
    body_html: '<p>teste pdf historico</p>',
    body_text: 'teste pdf historico',
    ...
    acessos: [{ mailbox_id: 'pessoal', owner: stateOwner, user: 'portal@example.com', at: new Date('2026-04-21T12:00:00.000Z') }],
...
test('GET /condominios/api/msg/messages/:id/historico-acessos.pdf sem sessão preserva 401 texto simples', async () => {
  const { app, close } = await createServer({ skipDb: true });
  const teardownGuard = installTeardownSuppression();

  try {
    const res = await request(app)
      .get('/condominios/api/msg/messages/507f1f77bcf86cd799439099/historico-acessos.pdf?mailboxId=pessoal')
      .set('Connection', 'close')
      .set('x-wdg-portal', '1');

    assert.equal(res.status, 401);
    assert.match(String(res.text || ''), /Não autenticado/);
  } finally {
    try {
...
test('GET /condominios/api/msg/messages/:id/historico-acessos.pdf no Portal pessoal aceita state legado por habitação com email resolvido por backfill', async () => {
  const teardownGuard = installTeardownSuppression();
  ...
  await withEnv({ PORTAL_COOKIE_SECRET: 'portal-history-access-pdf-runtime-test-secret', MONGO_MEMORY: '1' }, async () => {
    ...
    try {
      ...
      const message = await CondMsgMessage.create(messageDoc({
        protocolo: `HIST-PDF-${Date.now()}`,
        stateOwner: legacyOwner,
      }));

      const res = await request(app)
        .get(`/condominios/api/msg/messages/${message._id}/historico-acessos.pdf?mailboxId=pessoal`)
        .set('Connection', 'close')
        .set('x-wdg-portal', '1')
        .set('Cookie', buildPortalCookieHeader({
          userId: portalUserId,
          session: { email: undefined, userEmail: undefined, habitacao_id: habitacaoId },
        }));

      assert.equal(res.status, 200, String(res.text || ''));
      assert.match(String(res.headers['content-type'] || ''), /^application\/pdf\b/i);
      assert.ok(Buffer.isBuffer(res.body), 'esperava body binário do PDF');
      assert.ok(res.body.length > 0, 'esperava PDF não vazio');
    } finally {
      try {
        await closeWithTeardownGuard(close, teardownGuard);
      } finally {
