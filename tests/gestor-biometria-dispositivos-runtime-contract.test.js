...
const BIOMETRIA_DISPOSITIVOS_ENDPOINT = '/gestor/api/biometria/dispositivos';
const TEST_SESSION_SEED_ENDPOINT = '/__tests__/seed-gestor-biometria-session';
...
function installSessionSeedRoute(app) {
  app.get(TEST_SESSION_SEED_ENDPOINT, (req, res) => {
    req.session.user = {
      id: 'gestor-biometria-session-user',
      _id: 'gestor-biometria-session-user',
      email: 'gestor-biometria@example.com',
      nome: 'Gestor Biometria Session',
      role: 'admin',
      global_role: 'admin',
      isMaster: false,
    };
...
test('GET /gestor/api/biometria/dispositivos sem sessão preserva o contrato real atual de autenticação', async () => {
  ...
  try {
    const res = await request(app)
      .get(BIOMETRIA_DISPOSITIVOS_ENDPOINT)
...
test('GET /gestor/api/biometria/dispositivos autenticado preserva o contrato atual de sucesso com array', async () => {
  const teardownGuard = installTeardownSuppression();
  const { app, close, registerErrorHandlers } = await createServer({ skipDb: true, deferErrorHandlers: true });
...
test('GET /gestor/api/biometria/dispositivos em ambiente de teste preserva biometria desabilitada com array vazio', async () => {
  const teardownGuard = installTeardownSuppression();
  const { app, close, registerErrorHandlers } = await createServer({ skipDb: true, deferErrorHandlers: true });
