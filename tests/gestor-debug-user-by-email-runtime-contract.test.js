...
const ENDPOINT_BASE = '/gestor/debug/user-by-email';
const TEST_SESSION_SEED_ENDPOINT = '/__tests__/seed-debug-user-by-email-session';
...
function buildUniqueEmail(prefix = 'debug-user-by-email') {
...
test('GET /gestor/debug/user-by-email/:email sem sessao cai no fluxo real de login', async () => {
  const res = await request(app)
    .get(`${ENDPOINT_BASE}/${encodeURIComponent('missing-session@example.com')}`)
    .redirects(0)
    .set('Connection', 'close');
...
test('GET /gestor/debug/user-by-email sem parametro nao atinge o owner e permanece 404 na rota real', async () => {
...
test('GET /gestor/debug/user-by-email/:email retorna 404 quando lookup nao encontra usuario', async () => {
  const user = await createUser({
  ...
  const res = await agent
    .get(`${ENDPOINT_BASE}/${encodeURIComponent('nao-existe@example.com')}`)
    .set('Accept', 'application/json')
    .set('Connection', 'close');
...
test('GET /gestor/debug/user-by-email/:email retorna 200 no caminho feliz com lowercase efetivo e payload projetado', async () => {
  ...
  const targetUser = await createUser({
    email: 'mixed.user.runtime@example.com',
  ...
  const res = await agent
    .get(`${ENDPOINT_BASE}/${encodeURIComponent('Mixed.User.Runtime@Example.COM')}`)
    .set('Accept', 'application/json')
    .set('Connection', 'close');
  ...
  assert.equal(res.body?.data?.email, 'mixed.user.runtime@example.com');
...
test('GET /gestor/debug/user-by-email/:email retorna 500 quando o lookup interno falha', async () => {
  const authUser = await createUser({
    email: buildUniqueEmail('debug-auth'),
    role: 'admin',
    globalRole: 'admin',
  ...
  try {
    User.findOne = (cond, ...args) => {
      if (String(cond?.email || '').trim().toLowerCase() === 'boom@example.com') {
        throw new Error('FORCED_DEBUG_USER_BY_EMAIL_FAILURE');
      }
      return originalFindOne.call(User, cond, ...args);
    };

    const res = await agent
      .get(`${ENDPOINT_BASE}/${encodeURIComponent('boom@example.com')}`)
      .set('Accept', 'application/json')
      .set('Connection', 'close');
    ...
    assert.deepEqual(res.body, {
      success: false,
      code: 'SERVER_ERROR',
      message: 'FORCED_DEBUG_USER_BY_EMAIL_FAILURE',
    });
  } finally {
