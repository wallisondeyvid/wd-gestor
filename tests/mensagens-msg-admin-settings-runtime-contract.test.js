...
test('GET /mensagens/api/msg/admin/settings está migrado e exige autenticação admin', async () => {
...
test('PUT /mensagens/api/msg/admin/settings está migrado e exige autenticação admin', async () => {
  const { app, close } = await createServer({ skipDb: true });
  const teardownGuard = installTeardownSuppression();
