...
test('POST /mensagens/api/msg/messages no Portal permite mailbox destinataria fora da unidade efetiva', async () => {
  ...
  await withEnv({ PORTAL_COOKIE_SECRET: 'portal-send-recipient-gates-runtime-test-secret', MONGO_MEMORY: '1' }, async () => {
    ...
    try {
      ...
      const res = await request(app)
        ...
        .set('x-wdg-portal', '1')
        .set('Cookie', buildPortalCookieHeader({ session: { unidade_id: UNIDADE_ID } }))
        .field('payload', JSON.stringify(buildPayload({
          to: [{ type: 'mailbox', mailboxId: '507f1f77bcf86cd799439050', name: 'Caixa Outra Unidade' }],
        })));
      ...
      assert.equal(
        String(recipient?.mailboxId || recipient?.mailbox_id || ''),
...
test('POST /mensagens/api/msg/messages no Portal permite pessoal para pessoal mesmo com gate restritivo', async () => {
  const teardownGuard = installTeardownSuppression();
  ...
  await withEnv({ PORTAL_COOKIE_SECRET: 'portal-send-recipient-gates-runtime-test-secret', MONGO_MEMORY: '1' }, async () => {
    ...
    try {
      ...
      const res = await request(app)
        ...
        .set('Cookie', buildPortalCookieHeader({ session: { unidade_id: UNIDADE_ID } }))
        .field('payload', JSON.stringify(buildPayload({
          to: [{ type: 'user', email: 'destino@example.com', name: 'Outro Morador Portal' }],
        })));
      ...
      assert.equal(String(saved?.from_mailbox_id || ''), 'pessoal', JSON.stringify(saved));
      assert.equal(String(recipient?.type || ''), 'user', JSON.stringify(saved));
      assert.equal(String(recipient?.email || ''), 'destino@example.com', JSON.stringify(saved));
    } finally {
      try {
        await closeWithTeardownGuard(close, teardownGuard);
      } finally {
...
test('POST /mensagens/api/msg/messages no Portal permite destinatario pessoal suspenso', async () => {
  const teardownGuard = installTeardownSuppression();
  ...
  await withEnv({ PORTAL_COOKIE_SECRET: 'portal-send-recipient-gates-runtime-test-secret', MONGO_MEMORY: '1' }, async () => {
    ...
    try {
      ...
      const res = await request(app)
        ...
        .set('Cookie', buildPortalCookieHeader({ session: { unidade_id: UNIDADE_ID } }))
        .field('payload', JSON.stringify(buildPayload({
          to: [{ type: 'user', email: 'suspenso@example.com', name: 'Destinatario Suspenso' }],
        })));
      ...
      assert.equal(String(saved?.from_mailbox_id || ''), 'pessoal', JSON.stringify(saved));
      assert.equal(String(recipient?.type || ''), 'user', JSON.stringify(saved));
      assert.equal(String(recipient?.email || ''), 'suspenso@example.com', JSON.stringify(saved));
    } finally {
      try {
        await closeWithTeardownGuard(close, teardownGuard);
      } finally {
