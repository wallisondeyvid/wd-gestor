...
const BIOMETRIA_CAPTURAR_ENDPOINT = '/gestor/api/biometria/capturar';
const TEST_SESSION_SEED_ENDPOINT = '/__tests__/seed-gestor-biometria-capturar-session';
...
function installSessionSeedRoute(app) {
  app.get(TEST_SESSION_SEED_ENDPOINT, (req, res) => {
    req.session.user = {
      id: 'gestor-biometria-capturar-session-user',
      _id: 'gestor-biometria-capturar-session-user',
      email: 'gestor-biometria-capturar@example.com',
      nome: 'Gestor Biometria Capturar Session',
      role: 'admin',
      global_role: 'admin',
      isMaster: false,
    };
...
test('POST /gestor/api/biometria/capturar sem sessão preserva o contrato real atual de autenticação', async () => {
  ...
  try {
    const res = await request(app)
      .post(BIOMETRIA_CAPTURAR_ENDPOINT)
...
test('POST /gestor/api/biometria/capturar autenticado preserva o contrato atual no ambiente de teste', async () => {
  const teardownGuard = installTeardownSuppression();
  const { app, close, registerErrorHandlers } = await createServer({ skipDb: true, deferErrorHandlers: true });
...
test('POST /gestor/api/biometria/capturar em ambiente de teste preserva o ramo disabled com payload real atual', async () => {
  const teardownGuard = installTeardownSuppression();
  const { app, close, registerErrorHandlers } = await createServer({ skipDb: true, deferErrorHandlers: true });
  ...
  try {
    ...
    const res = await agent
      ...
      .set('Accept', 'application/json')
      .set('Connection', 'close')
      .send({ vendorId: '0x1234', productId: '0x5678' });
