...
test('face/upload per-capture/blob: o helper real atual preserva parsing e sharp, e delega blob', () => {
...
test('face/upload per-capture/blob: o helper delegado deixa de falar diretamente com uuid, key, putOptions e put', () => {
  ...
  assert.match(delegatedSource, /const m = .*exec\(dataUrl\);/);
  assert.match(delegatedSource, /const buf = Buffer\.from\(b64, 'base64'\);/);
  assert.match(delegatedSource, /const image = sharp\(buf\);/);
