  ...
  load(url, context, nextLoad) {
    ...
    if (url === dataFacadeMockModuleUrl) {
      const lines = [
        ...
        'const getMocks = () => globalThis.__GESTOR_RECURSOS_LIST_DATA_FACADE_MOCKS__ || {};',
      ];
...
function setDataFacadeMocks(overrides = {}) {
  globalThis.__GESTOR_RECURSOS_LIST_DATA_FACADE_MOCKS__ = { ...overrides };
}

function clearDataFacadeMocks() {
  globalThis.__GESTOR_RECURSOS_LIST_DATA_FACADE_MOCKS__ = {};
}
...
test('listarRecursosApi aplica filtro parcial de placa com normalizacao case-insensitive', async () => {
  setDataFacadeMocks({
    findRecursosByFiltroComUnidadeLeanData: async () => ([
      {
        _id: 'r-hit',
        placa: 'ABC-1D34',
        tipo: 'carro',
