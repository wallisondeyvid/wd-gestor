...
test('GET /condominios/api/msg/messages/:id/imprimir.pdf sem sessão preserva 401 texto simples', async () => {
  const { app, close } = await createServer({ skipDb: true });
  const teardownGuard = installTeardownSuppression();

  try {
    const res = await request(app)
      .get('/condominios/api/msg/messages/507f1f77bcf86cd799439099/imprimir.pdf?mailboxId=pessoal')
...
test('GET /condominios/api/msg/messages/:id/imprimir.pdf no Portal pessoal aceita state legado por habitação com email resolvido por backfill', async () => {
  const teardownGuard = installTeardownSuppression();
  ...
  await withEnv({ PORTAL_COOKIE_SECRET: 'portal-print-pdf-runtime-test-secret', MONGO_MEMORY: '1' }, async () => {
    ...
    try {
      ...
      const res = await request(app)
        .get(`/condominios/api/msg/messages/${message._id}/imprimir.pdf?mailboxId=pessoal&order=normal`)
        .set('Connection', 'close')
        .set('x-wdg-portal', '1')
