...
test('GET /mensagens/api/msg/messages/:id/historico-acessos.pdf sem sesṣo preserva 401 texto simples', async () => {
  const { app, close } = await createServer({ skipDb: true });
  const teardownGuard = installTeardownSuppression();

  try {
    const res = await request(app)
      .get('/mensagens/api/msg/messages/507f1f77bcf86cd799439099/historico-acessos.pdf?mailboxId=pessoal')
...
test('GET /mensagens/api/msg/messages/:id/historico-acessos.pdf no Portal pessoal aceita state legado por habita̤̣o com email resolvido por backfill', async () => {
  const teardownGuard = installTeardownSuppression();
  ...
  await withEnv({ PORTAL_COOKIE_SECRET: 'portal-history-access-pdf-runtime-test-secret', MONGO_MEMORY: '1' }, async () => {
    ...
    try {
      ...
      const res = await request(app)
        .get(`/mensagens/api/msg/messages/${message._id}/historico-acessos.pdf?mailboxId=pessoal`)
        .set('Connection', 'close')
        .set('x-wdg-portal', '1')
        .set('Cookie', buildPortalCookieHeader({
          userId: portalUserId,
          session: { habitacao_id: habitacaoId },
        }));
