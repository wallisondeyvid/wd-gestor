...
test('GET /mensagens/api/msg/markers sem sessao Portal preserva 401 json', async () => {
  const { app, close } = await createServer({ skipDb: true });
  const teardownGuard = installTeardownSuppression();

  try {
    const res = await request(app)
      .get('/mensagens/api/msg/markers?mailboxId=pessoal')
...
test('GET /mensagens/api/msg/markers no Portal pessoal deduplica owners compativeis', async () => {
  const teardownGuard = installTeardownSuppression();
...
test('POST /mensagens/api/msg/markers no Portal pessoal reaproveita marcador em owner legado compativel', async () => {
  const teardownGuard = installTeardownSuppression();
  const habitacaoId = '507f1f77bcf86cd799439012';
  ...
  await withEnv({ PORTAL_COOKIE_SECRET: 'portal-markers-runtime-test-secret', MONGO_MEMORY: '1' }, async () => {
    ...
    try {
      ...
      const res = await request(app)
        .post('/mensagens/api/msg/markers')
        .set('Accept', 'application/json')
        .set('Connection', 'close')
        .set('x-wdg-portal', '1')
        .set('Cookie', buildPortalCookieHeader({ session: { habitacao_id: habitacaoId } }))
...
test('DELETE /mensagens/api/msg/markers/:id no Portal pessoal desativa owners compativeis e limpa states', async () => {
  const teardownGuard = installTeardownSuppression();
  const habitacaoId = '507f1f77bcf86cd799439012';
  ...
  await withEnv({ PORTAL_COOKIE_SECRET: 'portal-markers-runtime-test-secret', MONGO_MEMORY: '1' }, async () => {
    ...
    try {
      ...
      const res = await request(app)
        .delete(`/mensagens/api/msg/markers/${baseMarker._id}?mailboxId=pessoal`)
        .set('Accept', 'application/json')
        .set('Connection', 'close')
        .set('x-wdg-portal', '1')
        .set('Cookie', buildPortalCookieHeader({ session: { habitacao_id: habitacaoId } }));
