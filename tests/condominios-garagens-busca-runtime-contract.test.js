...
import CondVagaGaragem from '../src/core/models/cond_vaga_garagem.js';
...
function buildUniqueEmail(prefix = 'garagens-busca-runtime') {
...
async function createUser({
  email = buildUniqueEmail('diretor-garagens-busca'),
  nome = 'Diretor Teste',
  role = 'diretor',
  unidadeId = null,
} = {}) {
...
async function createDiretorAgent(app, { unidadeId } = {}) {
  ...
  await UserMembership.create({
    ...
    status: 'active',
    origem: 'condominios-garagens-busca-runtime-contract-test',
  });
...
async function createVaga({ unidadeId, nome = uniqueLabel('Vaga Runtime'), linkType = '', obs = '', foto = '' }) {
  return CondVagaGaragem.create({
    unidade_id: unidadeId,
    nome,
    link_type: linkType,
    obs,
...
function listGaragens(agent, query = '') {
  return agent
    .get(`/condominios/api/garagens/busca${query}`)
    .set('Accept', 'application/json')
    .set('Connection', 'close');
}

test('GET /condominios/api/garagens/busca retorna 403 quando unidade explicita esta fora do escopo do diretor', async () => {
  const { app } = await getRuntimeContext();
  const { agent } = await createDiretorAgent(app, { unidadeId: '507f1f77bcf86cd799439710' });

  const res = await listGaragens(agent, '?unidade=507f1f77bcf86cd799439711');
...
test('GET /condominios/api/garagens/busca retorna apenas vagas permitidas e respeita filtro de nome', async () => {
  const { app } = await getRuntimeContext();
  const { agent, unidade } = await createDiretorAgent(app, { unidadeId: '507f1f77bcf86cd799439720' });
  const outraUnidade = await createEnabledUnit(uniqueLabel('Outra Unidade'), '507f1f77bcf86cd799439721');

  const permitida = await createVaga({
    unidadeId: unidade._id,
    nome: 'Garagem Premium Runtime',
    linkType: 'hab',
    obs: 'Uso interno',
    foto: 'garagem-runtime.jpg'
  });
  await createVaga({
    unidadeId: unidade._id,
    nome: 'Garagem Descoberta Runtime',
    linkType: '',
    obs: 'Visitantes',
    foto: ''
  });
  await createVaga({
    ...
    nome: 'Garagem Coberta Externa',
    linkType: 'area',
    obs: 'Outra unidade',
    foto: 'externa.jpg'
  });

  const res = await listGaragens(agent, '?nome=Premium');
  ...
  assert.equal(String(res.body[0]?._id), String(permitida._id), JSON.stringify(res.body));
  assert.equal(String(res.body[0]?.unidade?._id), String(unidade._id), JSON.stringify(res.body));
  assert.equal(res.body[0]?.nome, 'Garagem Premium Runtime', JSON.stringify(res.body));
  assert.equal(res.body[0]?.link_type, 'hab', JSON.stringify(res.body));
  assert.equal(res.body[0]?.obs, 'Uso interno', JSON.stringify(res.body));
  assert.equal(res.body[0]?.foto, 'garagem-runtime.jpg', JSON.stringify(res.body));
});