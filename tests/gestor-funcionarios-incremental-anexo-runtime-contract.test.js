...
async function createFuncionarioInTenant(unidadeId, { nome, email, cpf, anexos = [] }) {
...
async function authenticateContextualAgent(app, { unidadeId, prefix }) {
  ...
  const user = await User.create({
    ...
    nome: `Incremental Anexo Runtime ${nextCounter()}`,
  });
  ...
  await UserMembership.create({
    ...
    status: 'active',
    origem: 'gestor-funcionarios-incremental-anexo-runtime-contract-test',
  });
...
async function withHarness(run) {
  ...
  try {
    ...
    const contextualAuth = await authenticateContextualAgent(app, {
      ...
      prefix: 'funcionario-incremental-anexo-runtime-context',
    });
    const outsiderAuth = await authenticateContextualAgent(app, {
      unidadeId: unidadeC._id,
      prefix: 'funcionario-incremental-anexo-runtime-outsider',
    });
...
function textBuffer() {
  return Buffer.from('contrato-runtime-anexo-incremental', 'utf8');
}

test('PUT /gestor/api/funcionarios/:id/incremental anexo sem sessao retorna unauthorized em JSON', async () => {
  await withHarness(async ({ app, unidadeB }) => {
    const funcionario = await createFuncionarioInTenant(unidadeB._id, {
      ...
      email: uniqueEmail('func-incremental-anexo-sem-sessao'),
      cpf: uniqueCpf(),
    });
    ...
    const res = await request(app)
      ...
      .attach('anexos', textBuffer(), { filename: 'sem-sessao.txt', contentType: 'text/plain' });
...
test('PUT /gestor/api/funcionarios/:id/incremental anexo fora do escopo contextual retorna not found', async () => {
  await withHarness(async ({ unidadeB, outsiderAgent }) => {
    const funcionario = await createFuncionarioInTenant(unidadeB._id, {
      nome: `Funcionario Fora Escopo ${Date.now()}-${nextCounter()}`,
      email: uniqueEmail('func-incremental-anexo-fora-escopo'),
      cpf: uniqueCpf(),
    });
    ...
    const res = await outsiderAgent
      ...
      .set('Accept', 'application/json')
      .set('Connection', 'close')
      .attach('anexos', textBuffer(), { filename: 'fora-escopo.txt', contentType: 'text/plain' });
...
test('PUT /gestor/api/funcionarios/:id/incremental anexo adiciona novo item e preserva anexos existentes', async () => {
  await withHarness(async ({ unidadeB, contextualAgent }) => {
    const funcionario = await createFuncionarioInTenant(unidadeB._id, {
      nome: `Funcionario Sucesso Anexo ${Date.now()}-${nextCounter()}`,
      email: uniqueEmail('func-incremental-anexo-success'),
      cpf: uniqueCpf(),
      anexos: [{
        nome: 'existente.pdf',
        mime: 'application/pdf',
        tamanho: 321,
        caminho: 'uploads/existente.pdf',
    ...
    const res = await contextualAgent
      ...
      .set('Accept', 'application/json')
      .set('Connection', 'close')
      .attach('anexos', textBuffer(), { filename: 'novo-contrato.txt', contentType: 'text/plain' });
    ...
    assert.ok(updated);
    assert.ok(Array.isArray(updated.anexos), JSON.stringify(updated));
    assert.equal(updated.anexos.length, 2, JSON.stringify(updated));
    assert.equal(updated.anexos[0]?.nome, 'existente.pdf', JSON.stringify(updated));
    assert.equal(updated.anexos[1]?.nome, 'novo-contrato.txt', JSON.stringify(updated));
    assert.equal(updated.anexos[1]?.mime, 'text/plain', JSON.stringify(updated));
    assert.equal(updated.anexos[1]?.tamanho, textBuffer().length, JSON.stringify(updated));
    assert.match(String(updated.anexos[1]?.caminho || ''), /^uploads\//, JSON.stringify(updated));
  });
});

test('PUT /gestor/api/funcionarios/:id/incremental anexo com funcionario inexistente retorna not found', async () => {
  await withHarness(async ({ contextualAgent }) => {
    const res = await contextualAgent
      .put('/gestor/api/funcionarios/64f111111111111111111111/incremental')
      .set('Accept', 'application/json')
      .set('Connection', 'close')
      .attach('anexos', textBuffer(), { filename: 'inexistente.txt', contentType: 'text/plain' });
...
test('PUT /gestor/api/funcionarios/:id/incremental anexo propaga falha interna do update persistido', async () => {
  await withHarness(async ({ unidadeB, contextualAgent }) => {
    const funcionario = await createFuncionarioInTenant(unidadeB._id, {
      nome: `Funcionario Falha Anexo ${Date.now()}-${nextCounter()}`,
      email: uniqueEmail('func-incremental-anexo-failure'),
      cpf: uniqueCpf(),
    });
    ...
    FuncionarioModel.findOneAndUpdate = function patchedFindOneAndUpdate() {
      throw new Error('forced incremental anexo failure');
    };
    ...
    try {
      const res = await contextualAgent
        ...
        .set('Accept', 'application/json')
        .set('Connection', 'close')
        .attach('anexos', textBuffer(), { filename: 'falha-anexo.txt', contentType: 'text/plain' });
