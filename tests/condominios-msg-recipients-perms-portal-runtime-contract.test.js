...
test('GET /condominios/api/msg/recipients/perms sem sessao Portal preserva 401 json', async () => {
  const { app, close } = await createServer({ skipDb: true });
  const teardownGuard = installTeardownSuppression();

  try {
    const res = await request(app)
      .get('/condominios/api/msg/recipients/perms')
...
test('GET /condominios/api/msg/recipients/perms no Portal resolve email, unidade e regras por usuário', async () => {
  const teardownGuard = installTeardownSuppression();
  ...
  await withEnv({ PORTAL_COOKIE_SECRET: 'portal-recipients-perms-runtime-test-secret', MONGO_MEMORY: '1' }, async () => {
    ...
    try {
      ...
      assert.equal(String(res.body?.email || ''), 'portal@example.com');
      ...
      assert.equal(res.body?.effectiveAllowP2P, false);
      assert.equal(res.body?.restrictNewMessageRecipients, true);
    } finally {
      try {
        await closeWithTeardownGuard(close, teardownGuard);
      } finally {
...
test('GET /condominios/api/msg/recipients/perms falha com 403 quando a unidade efetiva nao pode ser determinada', async () => {
  const teardownGuard = installTeardownSuppression();
  const portalUserId = '507f1f77bcf86cd799439011';
  ...
  await withEnv({ PORTAL_COOKIE_SECRET: 'portal-recipients-perms-runtime-test-secret', MONGO_MEMORY: '1' }, async () => {
    ...
    try {
      ...
      assert.equal(res.body?.error, 'Não foi possível determinar a unidade do usuário.');
    } finally {
      try {
        await closeWithTeardownGuard(close, teardownGuard);
      } finally {
