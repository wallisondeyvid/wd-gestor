...
test('POST /condominios/api/msg/messages/actions sem sessão Portal preserva 401 json', async () => {
  const { app, close } = await createServer({ skipDb: true });
  const teardownGuard = installTeardownSuppression();

  try {
    const res = await request(app)
      .post('/condominios/api/msg/messages/actions')
...
test('POST /condominios/api/msg/messages/actions no Portal pessoal arquiva a thread e preserva owner variant por habitação', async () => {
  const teardownGuard = installTeardownSuppression();
