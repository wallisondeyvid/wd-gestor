...
function buildReplyBaseDoc({
  protocolo,
  mailboxId = TARGET_MAILBOX_ID,
  mailboxName = 'Caixa Original',
} = {}) {
  return {
    protocolo,
    ano: 2026,
...
function buildPayload(overrides = {}) {
  return {
    ...
    to: [{ type: 'mailbox', mailboxId: TARGET_MAILBOX_ID, name: 'Caixa Original' }],
    cc: [],
...
async function createRestrictedSettingsFor(user) {
  return CondMsgSettings.create({
    unidade_id: UNIDADE_ID,
    portal_user_perms: [{
      email: user.email,
...
test('POST /mensagens/api/msg/messages no ramo nao-Portal permite inReplyToId inexistente valido', async () => {
  ...
  await withEnv({ MONGO_MEMORY: '1', WDG_FLAG_GESTOR_AUTH_CONTEXT_RESOLVER: undefined }, async () => {
    ...
    try {
      ...
      await CondMsgMailbox.create(mailboxDoc({
        _id: TARGET_MAILBOX_ID,
        name: 'Caixa Original',
      }));

      await createRestrictedSettingsFor(user);

      const inReplyToId = '507f1f77bcf86cd799439099';
      ...
      const saved = await CondMsgMessage.findById(res.body?.id).lean();

      assert.equal(String(saved?.in_reply_to || ''), inReplyToId, JSON.stringify(saved));
...
test('POST /mensagens/api/msg/messages no ramo nao-Portal permite reply para mailbox alvo diferente', async () => {
  const teardownGuard = installTeardownSuppression();
    ...
    try {
      ...
      const res = await agent
        ...
        .field('payload', JSON.stringify(buildPayload({
          ...
          to: [{ type: 'mailbox', mailboxId: OTHER_MAILBOX_ID, name: 'Caixa Divergente' }],
        })));
      ...
      assert.equal(String(saved?.in_reply_to || ''), String(base._id), JSON.stringify(saved));
      assert.equal(String(saved?.from_mailbox_id || ''), 'pessoal', JSON.stringify(saved));

      const recipient = Array.isArray(saved?.to) ? saved.to[0] : null;
      assert.equal(String(recipient?.type || ''), 'mailbox', JSON.stringify(saved));
      assert.equal(String(recipient?.mailboxId || recipient?.mailbox_id || ''), OTHER_MAILBOX_ID, JSON.stringify(saved));
    } finally {
      try {
        await closeWithTeardownGuard(close, teardownGuard);
      } finally {
...
test('POST /mensagens/api/msg/messages no ramo nao-Portal permite reply minimo para o mailbox remetente original', async () => {
  const teardownGuard = installTeardownSuppression();
