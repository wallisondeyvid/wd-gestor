...
async function createFuncionarioInTenant(unidadeId, overrides = {}) {
  ...
  return FuncionarioModel.create({
    ...
    email: overrides.email || uniqueEmail('funcionario-update-full-foto'),
...
async function authenticateContextualAgent(app, { unidadeId, papelContextual = 'gestor' }) {
  const email = uniqueEmail('funcionario-update-full-foto-context');
  ...
  await UserMembership.create({
    ...
    status: 'active',
    origem: 'gestor-funcionarios-update-full-foto-runtime-contract-test',
  });
...
test('PUT full foto: funcionario inexistente retorna 404', async () => {
...
test('PUT full foto: sucesso com excluir_foto true', async () => {
  await withHarness(async ({ unidadeB, contextualAgent }) => {
    const funcionario = await createFuncionarioInTenant(unidadeB._id, {
      foto: 'https://example.com/foto-excluir-full.webp',
    });
    ...
    assert.equal(Object.prototype.hasOwnProperty.call(persisted, 'foto'), false);
  });
});

test('PUT full foto: upload ou resposta real do ambiente atual', async () => {
  await withHarness(async ({ unidadeB, contextualAgent }) => {
    const funcionario = await createFuncionarioInTenant(unidadeB._id, {
      foto: 'https://example.com/foto-antiga-full.webp',
    });
    ...
    const res = await contextualAgent
      ...
      .set('Accept', 'application/json')
      .set('Connection', 'close')
      .attach('foto', tinyPngBuffer(), 'foto-full.png');
    ...
    if (hasBlobRuntimeEnabled()) {
      ...
      const persisted = await getTenantModel(Funcionario, unidadeB._id).findById(funcionario._id).lean();
      assert.equal(typeof persisted.foto, 'string');
      assert.match(String(persisted.foto || ''), /^https?:\/\//);
      assert.notEqual(String(persisted.foto || ''), 'https://example.com/foto-antiga-full.webp');
    } else {
      assert.equal(res.status, 503, JSON.stringify(res.body));
      assert.equal(res.body?.error, 'Blob não configurado (conecte a Store no Vercel ou defina BLOB_READ_WRITE_TOKEN/WDGESTOR_DB_DADOS_READ_WRITE_TOKEN)');
    }
  });
});

test('PUT full foto: preserva foto quando nao pede exclusao nem troca', async () => {
  await withHarness(async ({ unidadeB, contextualAgent }) => {
    const fotoOriginal = 'https://example.com/foto-preservada-full.webp';
    const funcionario = await createFuncionarioInTenant(unidadeB._id, {
      nome: 'Funcionario Foto Preservada',
      foto: fotoOriginal,
    });
    ...
    const res = await contextualAgent
      ...
      .set('Accept', 'application/json')
      .set('Connection', 'close')
      .field('nome', 'Funcionario Foto Preservada');
    ...
    const persisted = await getTenantModel(Funcionario, unidadeB._id).findById(funcionario._id).lean();
    assert.equal(String(persisted.foto || ''), fotoOriginal);
  });
});

test('PUT full foto: erro interno induzido no update retorna 500', async (t) => {
  await withHarness(async ({ unidadeB, contextualAgent }) => {
    const funcionario = await createFuncionarioInTenant(unidadeB._id, {
      foto: 'https://example.com/foto-erro-full.webp',
    });
    const FuncionarioModel = getTenantModel(Funcionario, unidadeB._id);

    t.mock.method(FuncionarioModel, 'findOneAndUpdate', async () => {
      throw new Error('forced update full foto failure');
    });
