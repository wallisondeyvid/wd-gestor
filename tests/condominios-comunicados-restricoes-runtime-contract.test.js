...
function buildUniqueEmail(prefix = 'comunicados-restricoes-runtime') {
  return `${prefix}.${Date.now()}.${nextSequence()}@example.com`;
}

function buildUniqueCpf() {
  return String(80000000000 + nextSequence()).slice(-11);
}
...
async function createUser({
  email = buildUniqueEmail('user-comunicados-restricoes'),
  nome = 'Usuario Restricoes Comunicados',
  role = 'diretor',
  unidadeId = null,
...
async function createDiretorAgent(app, { unidadeId } = {}) {
  const unidade = await Unidade.findById(unidadeId) || await createEnabledUnit(uniqueLabel('Unidade Diretor Restricoes'), unidadeId);
  const user = await createUser({
    email: buildUniqueEmail('diretor-comunicados-restricoes'),
    nome: 'Diretor Restricoes Comunicados',
    role: 'diretor',
  ...
  await UserMembership.create({
    ...
    status: 'active',
    origem: 'condominios-comunicados-restricoes-runtime-contract-test',
  });
...
async function createAdminAgent(app) {
  const user = await createUser({
    email: buildUniqueEmail('admin-comunicados-restricoes'),
    nome: 'Admin Restricoes Comunicados',
    role: 'admin',
    unidadeId: null,
    globalRole: 'admin',
  });
...
async function createMorador({
  ...
  email = buildUniqueEmail('morador-comunicados-restricoes'),
  cpf = buildUniqueCpf(),
} = {}) {
...
function listRestricoesHabitacoes(requester, query = {}) {
  return requester
    .get('/condominios/api/comunicados/restricoes/habitacoes')
    .query(query)
...
function listRestricoesMoradores(requester, query = {}) {
  return requester
    .get('/condominios/api/comunicados/restricoes/moradores')
    .query(query)
    .set('Accept', 'application/json')
    .set('Connection', 'close');
}
...
async function createOfflineSessionAgent(sessionUser) {
  ...
  built.app.get('/__tests__/seed-comunicados-restricoes-session', (req, res) => {
  ...
  const seed = await agent
    .get('/__tests__/seed-comunicados-restricoes-session')
    .set('Connection', 'close');
...
test('GET /condominios/api/comunicados/restricoes/habitacoes sem sessao preserva 401 com envelope atual', async () => {
...
test('GET /condominios/api/comunicados/restricoes/moradores sem sessao preserva 401 com envelope atual', async () => {
  const { app } = await getRuntimeContext();
...
test('GET /condominios/api/comunicados/restricoes/habitacoes sem unidade efetiva no ramo privilegiado preserva 400 atual', async () => {
  const { app } = await getRuntimeContext();
...
test('GET /condominios/api/comunicados/restricoes/moradores sem unidade efetiva no ramo privilegiado preserva 400 atual', async () => {
  const { app } = await getRuntimeContext();
  const { agent } = await createAdminAgent(app);
...
test('GET /condominios/api/comunicados/restricoes/habitacoes ignora query fora do escopo para usuario comum e preserva shape consumido pelo caller', async () => {
  const { app } = await getRuntimeContext();
  const unidadeA = await createEnabledUnit(uniqueLabel('Unidade A Restricoes Hab'));
  const unidadeB = await createEnabledUnit(uniqueLabel('Unidade B Restricoes Hab'));
...
test('GET /condominios/api/comunicados/restricoes/moradores ignora query fora do escopo para usuario comum e preserva shape consumido pelo caller', async () => {
  const { app } = await getRuntimeContext();
  const unidadeA = await createEnabledUnit(uniqueLabel('Unidade A Restricoes Mor'));
  const unidadeB = await createEnabledUnit(uniqueLabel('Unidade B Restricoes Mor'));
  ...
  const emailA = buildUniqueEmail('morador-a-comunicados-restricoes');
  ...
  await createMorador({
    ...
    nome: 'Outro Morador',
    email: buildUniqueEmail('morador-b-comunicados-restricoes'),
  });
...
test('GET /condominios/api/comunicados/restricoes/habitacoes preserva contrato offline autenticado', async () => {
...
test('GET /condominios/api/comunicados/restricoes/moradores preserva contrato offline autenticado', async () => {
  const { agent, close } = await createOfflineSessionAgent({
