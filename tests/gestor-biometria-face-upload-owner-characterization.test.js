...
test('face/upload: a rota real continua inline no router e compoe requireLogin + requireApiAuth antes do handler', () => {
  ...
  const route = loadFaceUploadRoute({
    sharp() {
      throw new Error('sharp nao deve ser executado nesta prova');
    },
    async put() {
      throw new Error('put nao deve ser executado nesta prova');
    },
    requireLogin,
    requireApiAuth,
  });
...
test('face/upload: o owner inline real continua dono dos gates, parsing base64, sharp, put e catch final', () => {
  ...
  assert.match(source, /\^data:\(image\\\/\(png\|jpeg\|webp\)\);base64,\(\.\+\)\$\/i/);
  assert.match(source, /const buf = Buffer\.from\(b64, 'base64'\);/);
  assert.match(source, /const id = uuid\(\);/);
  assert.match(source, /const image = sharp\(buf\);/);
  assert.match(source, /\.toFormat\('webp', \{ quality: 92 \}\)/);
  assert.match(source, /const \{ url \} = await put\(key, webpBuf, putOptions\);/);
...
test('face/upload: o handler inline real preserva o gate sem capturas', async () => {
  const route = loadFaceUploadRoute({
    sharp() {
...
test('face/upload: o handler inline real preserva o gate de blob não configurado antes do pipeline de imagem', async () => {
  let sharpCalls = 0;
  let putCalls = 0;
...
test('face/upload: o handler inline real continua dono do parsing base64, normalizacao com sharp, put no blob e resposta final de sucesso', async () => {
  const sharpCalls = [];
  const putCalls = [];
...
test('face/upload: o catch final continua no owner inline da rota', async () => {
  const logs = [];
  const route = loadFaceUploadRoute({
    sharp() {
      return {
