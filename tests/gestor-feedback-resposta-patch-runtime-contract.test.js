...
const TEST_SESSION_SEED_ENDPOINT = '/__tests__/seed-feedback-resposta-session';
  ...
  app.get(TEST_SESSION_SEED_ENDPOINT, async (req, res) => {
    try {
      ...
      req.session.user = {
        ...
        email: user.email,
        role: effectiveRole,
        nome: user.nome || 'Feedback Resposta Contract User',
...
async function createTestUser({ role, marker }) {
  ...
  const email = `${marker}.${role}.${unique}@feedback.resposta.contract.test`;
  ...
  return User.create({
    ...
    senha,
    cpf,
    nome: `Feedback Resposta Contract ${role}`,
    role,
    ativo: true,
...
async function createFeedbackDoc({ unidadeId, creatorUser, status = 'novo', mensagem, resposta = '' }) {
  const feedback = await Feedback.create({
    ...
    origem: {
      ...
      userAgent: 'feedback-resposta-runtime-contract',
...
test('PATCH /gestor/api/gestor/feedback/:feedbackId/resposta sem sessão retorna 401 JSON', async () => {
  await withHarness(async ({ app }) => {
    const res = await request(app)
      .patch(withRouteParam(CANONICAL_ADMIN_REPLY_ENDPOINT, 'ffffffffffffffffffffffff'))
      .set('Accept', 'application/json')
      .set('Connection', 'close')
      .send({ resposta: 'Resposta fora de sessão' });

    expectUnauthorizedJson(res);
  });
});

test('PATCH canônico com admin contextual atualiza a resposta e persiste o status respondido na unidade ativa', async () => {
  await withHarness(async ({ app }) => {
    const marker = `feedback_resposta_patch_${Date.now()}_${nextCounter()}`;
    ...
    const feedbackId = await createFeedbackDoc({
      ...
      creatorUser,
      mensagem: 'feedback da unidade A para patch canônico de resposta',
    });

    const res = await adminAgent
      .patch(withRouteParam(CANONICAL_ADMIN_REPLY_ENDPOINT, feedbackId))
      .set('Accept', 'application/json')
      .set('Connection', 'close')
      .send({ resposta: 'Resposta via PATCH focal' });

    expectApiSuccessEnvelope(res, 200);
    assert.equal(res.body?.data?.resposta, 'Resposta via PATCH focal');
    assert.equal(res.body?.data?.status, 'respondido');

    const persisted = await Feedback.findById(feedbackId).lean();
    assert.equal(String(persisted?.resposta || ''), 'Resposta via PATCH focal');
    assert.equal(String(persisted?.status || ''), 'respondido');
  });
});
...
test('PATCH canônico com usuário não admin preserva o gate 403 antes da mutação', async () => {
  await withHarness(async ({ app }) => {
    ...
    const feedbackId = await createFeedbackDoc({
      unidadeId: FEEDBACK_UNIT_A,
      creatorUser,
      mensagem: 'feedback da unidade A para gate 403 do patch de resposta',
    });
    ...
    const res = await creatorAgent
      ...
      .set('Accept', 'application/json')
      .set('Connection', 'close')
      .send({ resposta: 'Tentativa não admin' });

    expectApiFailEnvelope(res, 403);
    assert.equal(res.body?.error, 'Acesso negado.');

    const persisted = await Feedback.findById(feedbackId).lean();
    assert.equal(String(persisted?.resposta || ''), '');
...
test('PATCH canônico com admin contextual fora de escopo traduz o alvo para 404 e não altera o documento', async () => {
  await withHarness(async ({ app }) => {
    ...
    const feedbackId = await createFeedbackDoc({
      ...
      creatorUser,
      mensagem: 'feedback da unidade B fora do escopo admin contextual A para resposta',
    });
...
test('PATCH canônico com resposta em branco normaliza para vazio e preserva status novo', async () => {
  await withHarness(async ({ app }) => {
    const marker = `feedback_resposta_patch_${Date.now()}_${nextCounter()}`;
    const adminUser = await createTestUser({ role: 'admin', marker });
    const creatorUser = await createTestUser({ role: 'user', marker });
    ...
    const feedbackId = await createFeedbackDoc({
      unidadeId: FEEDBACK_UNIT_A,
      creatorUser,
      mensagem: 'feedback da unidade A para normalização de resposta em branco',
    });
    ...
    expectApiSuccessEnvelope(res, 200);
    assert.equal(res.body?.data?.resposta, '');
...
test('PATCH canônico com resposta acima do limite preserva a validação 400 antes da mutação', async () => {
  await withHarness(async ({ app }) => {
    const marker = `feedback_resposta_patch_${Date.now()}_${nextCounter()}`;
    const adminUser = await createTestUser({ role: 'admin', marker });
    const creatorUser = await createTestUser({ role: 'user', marker });
    ...
    const feedbackId = await createFeedbackDoc({
      unidadeId: FEEDBACK_UNIT_A,
      creatorUser,
      mensagem: 'feedback da unidade A para validação 400 do patch de resposta',
    });
    ...
    const res = await adminAgent
      ...
      .set('Accept', 'application/json')
      .set('Connection', 'close')
      .send({ resposta: 'x'.repeat(5001) });

    expectApiFailEnvelope(res, 400);
    assert.equal(res.body?.error, 'Resposta deve ter no máximo 4000 caracteres.');
