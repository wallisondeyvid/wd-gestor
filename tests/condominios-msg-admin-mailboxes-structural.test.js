...
test('GET /condominios/api/msg/admin/mailboxes usa helper canônico mínimo de unidade e autorização', async () => {
  const source = await readSource();
  assert.match(source, /function prepareMsgAdminSettingsContext\(/);

  const routeBlock = extractRouteBlock(source, "app.get('/api/msg/admin/mailboxes', async (req, res) => {");
  assert.match(
    routeBlock,
