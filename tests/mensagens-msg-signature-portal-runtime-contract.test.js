...
test('GET /mensagens/api/msg/signature sem sessao Portal preserva 401 json', async () => {
  const { app, close } = await createServer({ skipDb: true });
  const teardownGuard = installTeardownSuppression();

  try {
    const res = await request(app)
      .get('/mensagens/api/msg/signature')
...
test('GET /mensagens/api/msg/signature no Portal lê preferência por email do cookie Portal', async () => {
  const teardownGuard = installTeardownSuppression();
  ...
  await withEnv({ PORTAL_COOKIE_SECRET: 'portal-signature-runtime-test-secret', MONGO_MEMORY: '1' }, async () => {
    ...
    try {
      ...
      const res = await request(app)
        .get('/mensagens/api/msg/signature?mailboxId=pessoal')
        .set('Accept', 'application/json')
        .set('Connection', 'close')
        .set('x-wdg-portal', '1')
        .set('referer', 'http://localhost/portal-morador/mensagens/entrada')
...
test('PUT /mensagens/api/msg/signature no Portal canonicaliza owner pessoal e remove duplicata legado', async () => {
  const teardownGuard = installTeardownSuppression();
  const portalUserId = '507f1f77bcf86cd799439011';
  ...
  await withEnv({ PORTAL_COOKIE_SECRET: 'portal-signature-runtime-test-secret', MONGO_MEMORY: '1' }, async () => {
    ...
    try {
      ...
      const res = await request(app)
        .put('/mensagens/api/msg/signature?mailboxId=pessoal')
        .set('Accept', 'application/json')
        .set('Connection', 'close')
        .set('x-wdg-portal', '1')
        .set('referer', 'http://localhost/portal-morador/mensagens/entrada')
