...
async function createFuncionarioInTenant(unidadeId, overrides = {}) {
  ...
  return FuncionarioModel.create({
    ...
    nome: overrides.nome || `Funcionario Incremental JSON ${Date.now()}-${nextCounter()}`,
    ...
    email: overrides.email || uniqueEmail('func-inc-json'),
...
async function authenticateContextualAgent(app, { unidadeId, prefix }) {
  ...
  const user = await User.create({
    ...
    nome: `Incremental JSON Runtime ${nextCounter()}`,
  });
...
function buildDependente(overrides = {}) {
  return {
    nome: overrides.nome || 'Dependente Incremental Runtime',
...
test('PUT /gestor/api/funcionarios/:id/incremental json cadastros sem sessao retorna unauthorized', async () => {
...
test('PUT /gestor/api/funcionarios/:id/incremental json cadastros fora do escopo contextual retorna not found', async () => {
...
test('PUT /gestor/api/funcionarios/:id/incremental json cadastros sucesso com dependentes_json valido', async () => {
  await withHarness(async ({ unidadeB, contextualAgent }) => {
    ...
    const dependentes = [
      buildDependente({
        nome: 'Alice Dependente Incremental',
        parentesco: 'Filha',
    ...
    assert.equal(Array.isArray(persisted.dependentes), true, JSON.stringify(persisted));
    assert.equal(persisted.dependentes.length, 1, JSON.stringify(persisted));
    assert.equal(persisted.dependentes[0]?.nome, 'Alice Dependente Incremental', JSON.stringify(persisted));
    assert.equal(persisted.dependentes[0]?.parentesco, 'Filha', JSON.stringify(persisted));
    assert.equal(new Date(persisted.dependentes[0]?.data_nascimento).toISOString().slice(0, 10), '2016-04-05', JSON.stringify(persisted));
    assert.equal(persisted.dependentes[0]?.cpf, '12345678901', JSON.stringify(persisted));
    assert.equal(persisted.dependentes[0]?.salario_familia, true, JSON.stringify(persisted));
    assert.equal(persisted.dependentes[0]?.irpf, false, JSON.stringify(persisted));
  });
});

test('PUT /gestor/api/funcionarios/:id/incremental json cadastros sucesso com beneficios_json valido', async () => {
  await withHarness(async ({ unidadeB, contextualAgent }) => {
    const funcionario = await createFuncionarioInTenant(unidadeB._id, {
      beneficios: [],
    });
    const beneficios = [
      buildBeneficio({
        tipo: 'VA',
        nome: 'Vale Alimentacao Incremental',
        tipo_valor: 'R$',
        valor: 321.45,
        inicio: 'outra data',
    ...
    const persisted = await getTenantModel(Funcionario, unidadeB._id).findById(funcionario._id).lean();
    assert.equal(Array.isArray(persisted.beneficios), true, JSON.stringify(persisted));
    assert.equal(persisted.beneficios.length, 1, JSON.stringify(persisted));
    assert.equal(persisted.beneficios[0]?.tipo, 'VA', JSON.stringify(persisted));
    assert.equal(persisted.beneficios[0]?.nome, 'Vale Alimentacao Incremental', JSON.stringify(persisted));
    assert.equal(persisted.beneficios[0]?.tipo_valor, 'R$', JSON.stringify(persisted));
    assert.equal(persisted.beneficios[0]?.valor, 321.45, JSON.stringify(persisted));
    assert.equal(new Date(persisted.beneficios[0]?.data_inicio).toISOString().slice(0, 10), '2024-06-01', JSON.stringify(persisted));
  });
});

test('PUT /gestor/api/funcionarios/:id/incremental json cadastros sucesso com dependentes_json e beneficios_json juntos', async () => {
  await withHarness(async ({ unidadeB, contextualAgent }) => {
    const funcionario = await createFuncionarioInTenant(unidadeB._id, {
      dependentes: [],
      beneficios: [],
    ...
    const dependentes = [
      buildDependente({
        nome: 'Bruno Dependente Incremental',
        parentesco: 'Filho',
    ...
    const beneficios = [
      buildBeneficio({
        tipo: 'VR',
        nome: 'Vale Refeicao Incremental',
        valor: 25.5,
        inicio: 'admissao',
    ...
    assert.equal(persisted.dependentes?.length, 1, JSON.stringify(persisted));
    assert.equal(persisted.beneficios?.length, 1, JSON.stringify(persisted));
    assert.equal(persisted.dependentes[0]?.nome, 'Bruno Dependente Incremental', JSON.stringify(persisted));
    assert.equal(persisted.beneficios[0]?.tipo, 'VR', JSON.stringify(persisted));
  });
});

test('PUT /gestor/api/funcionarios/:id/incremental json cadastros dependentes_json invalido converge para array vazio', async () => {
  await withHarness(async ({ unidadeB, contextualAgent }) => {
    const funcionario = await createFuncionarioInTenant(unidadeB._id, {
...
test('PUT /gestor/api/funcionarios/:id/incremental json cadastros beneficios_json invalido converge para array vazio', async () => {
  await withHarness(async ({ unidadeB, contextualAgent }) => {
    const funcionario = await createFuncionarioInTenant(unidadeB._id, {
