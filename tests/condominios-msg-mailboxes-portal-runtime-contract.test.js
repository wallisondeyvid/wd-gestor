...
test('GET /condominios/api/msg/mailboxes sem sessao Portal preserva 401 json', async () => {
  const { app, close } = await createServer({ skipDb: true });
  const teardownGuard = installTeardownSuppression();

  try {
    const res = await request(app)
      .get('/condominios/api/msg/mailboxes')
...
test('GET /condominios/api/msg/mailboxes/recipients sem sessao Portal preserva 401 json', async () => {
  const { app, close } = await createServer({ skipDb: true });
  const teardownGuard = installTeardownSuppression();

  try {
    const res = await request(app)
      .get('/condominios/api/msg/mailboxes/recipients')
      .set('Accept', 'application/json')
      .set('Connection', 'close')
      .set('x-wdg-portal', '1');
...
test('GET /condominios/api/msg/mailboxes no Portal lista member, publica e habitacao correspondente, ocultando operadores da publica nao membro', async () => {
  const teardownGuard = installTeardownSuppression();
  ...
  await withEnv({ PORTAL_COOKIE_SECRET: 'portal-mailboxes-runtime-test-secret', MONGO_MEMORY: '1' }, async () => {
    ...
    try {
      ...
      assert.ok(byName.has('Caixa Publica'), JSON.stringify(res.body));
      assert.ok(byName.has('Hab 101'), JSON.stringify(res.body));
      ...
      assert.equal(byName.get('Caixa Publica')?.isMember, false, JSON.stringify(byName.get('Caixa Publica')));
      assert.deepEqual(byName.get('Caixa Publica')?.operators || [], [], JSON.stringify(byName.get('Caixa Publica')));
      assert.equal(String(byName.get('Caixa Publica')?.createdBy || ''), '', JSON.stringify(byName.get('Caixa Publica')));
      assert.equal(byName.get('Hab 101')?.isMember, true, JSON.stringify(byName.get('Hab 101')));
    } finally {
      try {
        await closeWithTeardownGuard(close, teardownGuard);
      } finally {
...
test('GET /condominios/api/msg/mailboxes/recipients no Portal lista publica e habitacao correspondente, ocultando habitacao alheia', async () => {
  const teardownGuard = installTeardownSuppression();
  const unitId = '507f1f77bcf86cd799439010';
  const habitacaoId = '507f1f77bcf86cd799439012';
  const otherHabitacaoId = '507f1f77bcf86cd799439013';
  ...
  await withEnv({ PORTAL_COOKIE_SECRET: 'portal-mailboxes-runtime-test-secret', MONGO_MEMORY: '1' }, async () => {
    ...
    try {
      ...
      assert.equal(names.includes('Hab 101'), true, JSON.stringify(res.body));
...
test('POST /condominios/api/msg/mailboxes no Portal cria caixa com identidade resolvida por preparo canônico mínimo', async () => {
  const teardownGuard = installTeardownSuppression();
  const unitId = '507f1f77bcf86cd799439010';
  ...
  await withEnv({ PORTAL_COOKIE_SECRET: 'portal-mailboxes-runtime-test-secret', MONGO_MEMORY: '1' }, async () => {
    ...
    try {
      ...
      const res = await request(app)
        .post('/condominios/api/msg/mailboxes')
        .set('Accept', 'application/json')
        .set('Connection', 'close')
        .set('x-wdg-portal', '1')
      ...
      assert.equal(String(res.body?.createdBy || ''), 'portal@example.com', JSON.stringify(res.body));
      assert.equal(String(res.body?.operators?.[0]?.user || ''), 'portal@example.com', JSON.stringify(res.body));
    } finally {
      try {
        await closeWithTeardownGuard(close, teardownGuard);
      } finally {
...
test('PATCH /condominios/api/msg/mailboxes/:id no Portal autoriza membro por email resolvido do preparo canônico mínimo', async () => {
  const teardownGuard = installTeardownSuppression();
  const unitId = '507f1f77bcf86cd799439010';
  const portalUserId = '507f1f77bcf86cd799439011';
  ...
  await withEnv({ PORTAL_COOKIE_SECRET: 'portal-mailboxes-runtime-test-secret', MONGO_MEMORY: '1' }, async () => {
    ...
    try {
      ...
      await CondMsgMailbox.create(mailboxDoc({
        ...
        name: 'Caixa Editavel',
        unitId,
        operators: [{ user: 'portal@example.com', perms: { administrar: true, editar: true } }],
      }));

      const res = await request(app)
        .patch(`/condominios/api/msg/mailboxes/${mailboxId}`)
        .set('Accept', 'application/json')
        .set('Connection', 'close')
        .set('x-wdg-portal', '1')
        .set('Cookie', buildPortalCookieHeader({
...
test('DELETE /condominios/api/msg/mailboxes/:id no Portal autoriza exclusão por email resolvido do preparo canônico mínimo', async () => {
  const teardownGuard = installTeardownSuppression();
  const unitId = '507f1f77bcf86cd799439010';
  const portalUserId = '507f1f77bcf86cd799439011';
  ...
  await withEnv({ PORTAL_COOKIE_SECRET: 'portal-mailboxes-runtime-test-secret', MONGO_MEMORY: '1' }, async () => {
    ...
    try {
      ...
      await CondMsgMailbox.create(mailboxDoc({
        ...
        unitId,
        operators: [{ user: 'portal@example.com', perms: { administrar: true, editar: true, excluir: true } }],
      }));

      const res = await request(app)
        .delete(`/condominios/api/msg/mailboxes/${mailboxId}`)
        .set('Accept', 'application/json')
        .set('Connection', 'close')
        .set('x-wdg-portal', '1')
        .set('Cookie', buildPortalCookieHeader({
