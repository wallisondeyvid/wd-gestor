...
function buildUniqueEmail(prefix = 'naturezas-busca-runtime') {
...
async function createUser({
  email = buildUniqueEmail('diretor-naturezas-busca'),
  nome = 'Diretor Teste',
  role = 'diretor',
  unidadeId = null,
} = {}) {
...
async function createDiretorAgent(app, { unidadeId } = {}) {
  ...
  await UserMembership.create({
    ...
    status: 'active',
    origem: 'condominios-materiais-naturezas-busca-runtime-contract-test',
  });
...
function listNaturezas(agent, query = '') {
  return agent
    .get(`/condominios/api/materiais/naturezas/busca${query}`)
    .set('Accept', 'application/json')
    .set('Connection', 'close');
}

test('GET /condominios/api/materiais/naturezas/busca retorna 403 quando unidade explicita esta fora do escopo do diretor', async () => {
  const { app } = await getRuntimeContext();
  const { agent } = await createDiretorAgent(app, { unidadeId: '507f1f77bcf86cd799439610' });

  const res = await listNaturezas(agent, '?unidade=507f1f77bcf86cd799439611');
...
test('GET /condominios/api/materiais/naturezas/busca retorna apenas naturezas permitidas e respeita filtros de tipo e nome', async () => {
  const { app } = await getRuntimeContext();
  const { agent, unidade } = await createDiretorAgent(app, { unidadeId: '507f1f77bcf86cd799439620' });
  const outraUnidade = await createEnabledUnit(uniqueLabel('Outra Unidade'), '507f1f77bcf86cd799439621');

  const permitida = await createNatureza({
    unidadeId: unidade._id,
    tipo: 'Móvel',
    nome: 'Cadeira Empilhavel Runtime'
  });
  await createNatureza({
    unidadeId: unidade._id,
    tipo: 'Fixo',
    nome: 'Cadeira Fixa Runtime'
  });
  await createNatureza({
    unidadeId: outraUnidade._id,
    tipo: 'Móvel',
    nome: 'Cadeira Externa Runtime'
  });

  const res = await listNaturezas(agent, '?tipo=Móvel&nome=Empilhavel');
  ...
  assert.equal(res.body[0]?.tipo, 'Móvel', JSON.stringify(res.body));
  assert.equal(res.body[0]?.nome, 'Cadeira Empilhavel Runtime', JSON.stringify(res.body));
});