...
function buildUniqueEmail(prefix = 'servico-aceitar-runtime') {
...
async function createDiretorAgent(app, { unidadeId } = {}) {
  ...
  const user = await createUser({
    email: buildUniqueEmail('diretor-servico-aceitar'),
    nome: 'Diretor Servico Aceitar',
    role: 'diretor',
  ...
  await UserMembership.create({
    ...
    status: 'active',
    origem: 'condominios-solicitacoes-servico-aceitar-runtime-contract-test',
  });
...
function acceptSolicitacao(agent, solicitacaoId) {
  return agent
    .post(`/condominios/api/solicitacoes-servico/${solicitacaoId}/aceitar`)
    .set('Accept', 'application/json')
    .set('Connection', 'close');
}

test('POST /condominios/api/solicitacoes-servico/:id/aceitar retorna 404 quando solicitacao nao existe', async () => {
  const { app } = await getRuntimeContext();
  const { agent } = await createDiretorAgent(app, { unidadeId: '507f1f77bcf86cd799439310' });

  const res = await acceptSolicitacao(agent, '507f1f77bcf86cd799439399');
...
test('POST /condominios/api/solicitacoes-servico/:id/aceitar retorna 403 sem alterar a solicitacao fora do escopo do diretor', async () => {
  const { app } = await getRuntimeContext();
  const unidadeDona = await createEnabledUnit(uniqueLabel('Unidade Dona'), '507f1f77bcf86cd799439320');
  const solicitacao = await createSolicitacao({ unidadeId: unidadeDona._id, nova: true, titulo: 'Interfone sem audio' });
  const { agent } = await createDiretorAgent(app, { unidadeId: '507f1f77bcf86cd799439321' });

  const res = await acceptSolicitacao(agent, solicitacao._id);
...
test('POST /condominios/api/solicitacoes-servico/:id/aceitar retorna 200 e persiste aceite para diretor autorizado', async () => {
  const { app } = await getRuntimeContext();
  const { unidade, agent } = await createDiretorAgent(app, { unidadeId: '507f1f77bcf86cd799439330' });
  const solicitacao = await createSolicitacao({ unidadeId: unidade._id, nova: true, titulo: 'Portao com defeito' });
  ...
  assert.ok(persisted?.aceita_em);
  assert.ok(String(persisted?.aceita_por || '').length > 0);
