...
test('GET /mensagens/api/msg/groups sem sessao Portal preserva 401 json', async () => {
  const { app, close } = await createServer({ skipDb: true });
  const teardownGuard = installTeardownSuppression();

  try {
    const res = await request(app)
      .get('/mensagens/api/msg/groups?mailboxId=pessoal')
...
test('GET /mensagens/api/msg/groups no Portal pessoal lista owner base e owner legado com prefixo', async () => {
  const teardownGuard = installTeardownSuppression();
...
test('POST /mensagens/api/msg/groups no Portal pessoal cria grupo com owner can̫nico', async () => {
  const teardownGuard = installTeardownSuppression();
...
test('PATCH e DELETE /mensagens/api/msg/groups/:id no Portal pessoal aceitam grupo legado por owner variant', async () => {
  const teardownGuard = installTeardownSuppression();
  const habitacaoId = '507f1f77bcf86cd799439012';
  ...
  await withEnv({ PORTAL_COOKIE_SECRET: 'portal-groups-runtime-test-secret', MONGO_MEMORY: '1' }, async () => {
    ...
    try {
      ...
      const patchRes = await request(app)
        .patch(`/mensagens/api/msg/groups/${patchDoc._id}`)
        .set('Accept', 'application/json')
        .set('Connection', 'close')
        .set('x-wdg-portal', '1')
        .set('Cookie', buildPortalCookieHeader({ session: { habitacao_id: habitacaoId } }))
      ...
      const deleteRes = await request(app)
        .delete(`/mensagens/api/msg/groups/${deleteDoc._id}`)
        .set('Accept', 'application/json')
        .set('Connection', 'close')
        .set('x-wdg-portal', '1')
        .set('Cookie', buildPortalCookieHeader({ session: { habitacao_id: habitacaoId } }));
