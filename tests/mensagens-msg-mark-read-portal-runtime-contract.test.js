...
test('POST /mensagens/api/msg/messages/:id/read sem sessao Portal preserva 401 json', async () => {
  const { app, close } = await createServer({ skipDb: true });
  const teardownGuard = installTeardownSuppression();

  try {
    const res = await request(app)
      .post('/mensagens/api/msg/messages/507f1f77bcf86cd799439099/read')
...
test('POST /mensagens/api/msg/messages/:id/read com cookie Portal valido preserva 404 json', async () => {
  const teardownGuard = installTeardownSuppression();
  ...
  await withEnv({ PORTAL_COOKIE_SECRET: 'portal-runtime-test-secret', MONGO_MEMORY: '1' }, async () => {
    ...
    try {
      ...
      assert.match(String(res.headers['content-type'] || ''), /^application\/json\b/i);
      assert.match(String(res.body?.error || ''), /^Mensagem n.o encontrada$/u);
    } finally {
      try {
        await closeWithTeardownGuard(close, teardownGuard);
      } finally {
...
test('POST /mensagens/api/msg/messages/:id/read com body raw marca mensagem do Portal pessoal como lida', async () => {
  const teardownGuard = installTeardownSuppression();
  ...
  await withEnv({ PORTAL_COOKIE_SECRET: 'portal-runtime-test-secret', MONGO_MEMORY: '1' }, async () => {
    ...
    try {
      ...
      const res = await request(app)
        .post(`/mensagens/api/msg/messages/${root._id}/read`)
        .set('Accept', 'application/json')
        .set('Connection', 'close')
      ...
      const rootAfter = await CondMsgMessage.findById(root._id).lean();

      const rootState = (rootAfter?.states || []).find((state) =>
        String(state?.mailbox_id || '') === 'pessoal' &&
        String(state?.owner || '') === 'portal@example.com'
      );
