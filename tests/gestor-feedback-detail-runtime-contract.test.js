...
const TEST_SESSION_SEED_ENDPOINT = '/__tests__/seed-feedback-detail-session';
  ...
  app.get(TEST_SESSION_SEED_ENDPOINT, async (req, res) => {
    try {
      ...
      req.session.user = {
        ...
        nome: user.nome || 'Feedback Detail Contract User',
...
async function createTestUser({ role, marker }) {
  ...
  const email = `${marker}.${role}.${unique}@feedback.detail.contract.test`;
  ...
  return User.create({
    ...
    senha,
    cpf,
    nome: `Feedback Detail Contract ${role}`,
    role,
    ativo: true,
...
async function createFeedbackDoc({ unidadeId, creatorUser, mensagem, resposta = '' }) {
  const feedback = await Feedback.create({
    ...
    origem: {
      ...
      userAgent: 'feedback-detail-runtime-contract',
...
test('GET /gestor/api/gestor/feedback/:feedbackId sem sessão retorna 401 JSON', async () => {
...
test('GET canônico com admin contextual em escopo retorna 200 e normaliza resposta legada', async () => {
  await withHarness(async ({ app }) => {
    const marker = `feedback_detail_${Date.now()}_${nextCounter()}`;
    ...
    const feedbackId = await createLegacyFeedbackDoc({
      unidadeId: FEEDBACK_UNIT_A,
      creatorUser,
      mensagem: 'feedback legado para GET canônico de detail admin',
    });

    const res = await adminAgent
      .get(withRouteParam(CANONICAL_ADMIN_DETAIL_ENDPOINT, feedbackId))
      .set('Accept', 'application/json')
      .set('Connection', 'close');

    expectApiSuccessEnvelope(res, 200);
    assert.equal(String(res.body?.data?._id || res.body?.data?.id || ''), feedbackId);
    assert.equal(typeof res.body?.data?.resposta, 'string');
    assert.equal(res.body?.data?.resposta, 'resposta legado objeto');
  });
});
...
test('GET canônico com usuário não admin preserva o gate 403 antes da leitura', async () => {
  await withHarness(async ({ app }) => {
    ...
    const feedbackId = await createFeedbackDoc({
      unidadeId: FEEDBACK_UNIT_A,
      creatorUser,
      mensagem: 'feedback da unidade A para gate 403 do detail admin',
    });
...
test('GET canônico com admin contextual fora de escopo traduz o alvo para 404', async () => {
  await withHarness(async ({ app }) => {
    const marker = `feedback_detail_${Date.now()}_${nextCounter()}`;
    const adminUser = await createTestUser({ role: 'admin', marker });
    const creatorUser = await createTestUser({ role: 'user', marker });
...
test('GET canônico com id malformado preserva a validação 400 antes da leitura', async () => {
  await withHarness(async ({ app }) => {
    const marker = `feedback_detail_${Date.now()}_${nextCounter()}`;
    const adminUser = await createTestUser({ role: 'admin', marker });
    const adminAgent = await seedAuthenticatedAgent(app, adminUser, { unidadeId: FEEDBACK_UNIT_A });
