...
test('GET /mensagens/api/msg/messages sem sessao Portal preserva 401 json', async () => {
  const { app, close } = await createServer({ skipDb: true });
  const teardownGuard = installTeardownSuppression();

  try {
    const res = await request(app)
      .get('/mensagens/api/msg/messages')
...
test('GET /mensagens/api/msg/messages com cookie Portal valido preserva 200 runtime minimo', async () => {
  const teardownGuard = installTeardownSuppression();
...
test('GET /mensagens/api/msg/messages/:id sem sessao Portal preserva 401 json', async () => {
  const { app, close } = await createServer({ skipDb: true });
  const teardownGuard = installTeardownSuppression();

  try {
    const res = await request(app)
      .get('/mensagens/api/msg/messages/507f1f77bcf86cd799439099')
      .set('Accept', 'application/json')
      .set('Connection', 'close')
      .set('x-wdg-portal', '1');
...
test('GET /mensagens/api/msg/messages/:id com cookie Portal valido preserva 404 runtime minimo', async () => {
  const teardownGuard = installTeardownSuppression();
