...
(() => {
  ...
  async function coreFetch(nome, options){
    ...
    console.debug('[wdgFetchGestorJson] tentando', cleanName, 'caminhos:', lista);
    let attemptIndex=0;
    for(const url of lista){
      ...
      try{
        ...
        console.debug('[wdgFetchGestorJson] sucesso', cleanName, '=>', url, '(tentativa', attemptIndex, ')');
  ...
  console.info('[wdgFetchGestorJson] inicializado (Alt+Shift+D abre painel)');
  // Painel diagnóstico
    ...
    window.fetch = async function(input, init){
      try {
        ...
        if(!internal && typeof url === 'string' && /^\/data\//.test(url) && window.wdgFetchGestorJson){
          console.warn('[wdgFetchGuard] Interceptando fetch direto', url);
      ...
      } catch (e) { console.debug('[wdgFetchGuard] erro interceptor', e); }
      return origFetch(input, init);
    };
  } catch(e){ console.debug('[wdgFetchGuard] não pôde instalar', e); }
})();
