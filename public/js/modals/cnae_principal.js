// public/js/modals/cnae_principal.js (LEGADO)
// Use /gestor/js/modals/cnae_principal.js
if (window && window.wdgFetchGestorJson) { console.warn('[legacy cnae_principal] ignorado'); } else {
document.addEventListener('DOMContentLoaded', () => {
  ...
  const ulLista       = document.getElementById('listaCnaePrincipalList');
  ...
  function renderLista(filtro = ''){
    ...
    ulLista.innerHTML = itens.map(c => {
      ...
      const nome = c.nome || c.descricao || c.DESCRICAO || '';
      return `\n        <li class=\"list-group-item\" style=\"padding:0;border:0;\">\n          <div class=\"item-grid item-flex\" data-codigo=\"${esc(c.codigo)}\" data-nome=\"${esc(nome)}\">\n            <div class=\"col-sel\">\n              <input class=\"form-check-input\" type=\"radio\" name=\"cnaePrincipalSel\"\n                     id=\"${id}\" value=\"${esc(c.codigo)}\" data-nome=\"${esc(nome)}\">\n            </div>\n            <div class=\"col-cod\">${esc(c.codigo)}</div>\n            <div class=\"col-desc\" title=\"${esc(nome)}\">${esc(nome) || '<span style=\\"color:#64748b\\">(sem descrição)</span>'}</div>\n          </div>\n        </li>`;
    }).join('');
    ulLista.querySelectorAll('.item-grid').forEach(row => {
  ...
  function preSelecionar(){
    ...
    if (row){ row.classList.add('is-selected'); row.scrollIntoView({ block: 'center' }); }
  }

  async function carregarCnaes(){
    if (baseListaCnaes.length){ listaCnaes = baseListaCnaes.slice(0); renderLista(filtroAtual); return; }
    try{
      ...
      console.error('[cnae_principal] Falha ao carregar:', err);
  ...
  async function buscarApiCnaes(search){
    try{
      ...
      console.warn('[cnae_principal legacy] busca API falhou:', err.message || err);
      return null;
    }
  }
  ...
  filtroInput?.addEventListener('input', e => {
    ...
    debounceTimer = setTimeout(async () => {
      if (q.length >= 2){
        ...
        if (api && api.length){ listaCnaes = api; renderLista(''); return; }
      }
  ...
  btnConfirmar?.addEventListener('click', () => {
    ...
    campoDestino.dataset.cnaeCodigo = codigo;
