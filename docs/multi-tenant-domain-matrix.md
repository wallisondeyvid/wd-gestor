...
| funcionários | híbrido | fluxo contextual por `req.unitScope`; ramo global explícito para master/admin | sim no contextual | média | sim | leitura global legítima já existe, mas o detalhe por ID continua exigindo unidade ativa; listagem global legítima não implica detalhe global e ações seguem estritamente contextuais | `docs/migration-status.md`; `tests/gestor-funcionarios-get-by-id-runtime-contract.test.js` |
...
| condomínios | híbrido | wrapper do módulo, `req.unitScope` no V2 e compatibilidade V1/V2 | sim no corredor V2 | baixa | sim | fallback residual e bifurcação V1/V2 ainda impedem consolidação final; a triagem read-only pos-Unidades foi pausada de forma deliberada apos fechar ou caracterizar os corredores fortes recentes, sem novo candidato que passasse pela regua de pequeno, local, consumo real, risco tenant-aware plausivel e ausencia de guardrail/cobertura; `GET /api/habitacoes/sugestoes` fica somente como excecao futura, dependente de novo enquadramento e caller vivo confirmado | `docs/checkpoints/condominios-congelamento-conservador-snapshot-atual.md`; `docs/migration-status.md`; `tests/condominios.v2.parity.test.js`; `tests/condominios.v2.parity.matrix.test.js` |
| unidades condominiais | híbrido | GET /api/unidades ainda apoiado em helper/service local com fallback legado de contexto; GETs por ID e relacionados no V2 dependem de `requireUnitScope` e repositórios escopados | parcial | baixa | sim | GET /api/unidades ficou caracterizado como read-only local, vivo no frontend e não unitScope-first, sem enumeração indevida observada: usuário comum permanece restrito à matriz e relacionadas do cluster permitido, admin/master preservam leitura ampla e o risco remanescente passa a ser reabrir writes, detalhe por ID fora do corredor V2 ou compatibilidades amplas sem matriz semântica própria | `docs/migration-status.md`; `tests/condominios-unidades-list-runtime-contract.test.js`; `tests/condominios.requireUnitScope.test.js` |
| blocos | híbrido | contexto do módulo e famílias específicas, com GET /api/blocos ainda apoiado em service/helper local de contexto | parcial | baixa | sim | GET /api/blocos ficou congelado como read-only local, não unitScope-first, com fallback legado controlado e fail-closed por array vazio para unidade explícita fora do escopo; o risco remanescente passa a ser reabrir compatibilidades amplas ou writes sem matriz semântica própria | `docs/migration-status.md`; `tests/condominios-blocos-get-runtime-contract.test.js`; `tests/condominios.blocos.microcut.test.js` |
...
| enquetes | híbrido | seams locais já auditadas em GETs específicos, incluindo GET /api/enquetes/:id por helper local de contexto | parcial | baixa | provável | GET /api/enquetes/:id ficou caracterizado como read-only local, mas não unitScope-first; privilegio continua dependente de unidade efetiva própria ou unidade_id explícita, e o risco principal segue sendo abrir votos, detalhamento amplo e writes sem matriz semântica | `docs/checkpoints/condominios-congelamento-conservador-snapshot-atual.md`; `memories/repo/enquetes-detalhes-guardrail.md`; `tests/condominios-enquetes-by-id-runtime-contract.test.js` |
| habitacoes | híbrido | GET /api/habitacoes/busca ainda apoiado em handler local com fallback legado de contexto e enriquecimento posterior do payload | parcial | baixa | provável | GET /api/habitacoes/busca ficou congelado como read-only local, não unitScope-first, mais largo que Materiais e Blocos por retornar payload enriquecido, com fail-closed por array vazio para unidade explícita fora do escopo; o risco remanescente passa a ser reabrir writes, detalhes ou buscas adjacentes sem matriz semântica própria | `docs/migration-status.md`; `tests/condominios-habitacoes-busca-runtime-contract.test.js` |
| materiais | híbrido | contexto do módulo e famílias específicas, com GET /api/materiais/busca ainda apoiado em helper local de contexto | parcial | baixa | provável | GET /api/materiais/busca ficou congelado como read-only local, não unitScope-first, com fallback legado controlado e fail-closed por array vazio para unidade explícita fora do escopo; o risco remanescente passa a ser reabrir writes, QR Code ou outros subdomínios sem matriz semântica própria | checkpoints e guardrails de Materiais no módulo Condomínios; `tests/condominios-materiais-busca-runtime-contract.test.js` |
...
| clínica | compat legado temporário | wrapper fino estável; APIs vivas herdadas do perfil compartilhado (`/api/usuario`, `/api/modulos`, `/api/usuario/foto`) | baixa no snapshot atual | média | não evidenciado de forma relevante nesta rodada | triagem read-only pausada de forma deliberada; reabrir agora duplicaria Gestor/Core, porque não há handler clínico próprio pequeno nem leitura contextual por unidade; reavaliar só com callers reais ou handlers próprios de empresas, pacientes, profissionais, planos, procedimentos ou outro cadastro clínico contextual por unidade | `docs/clinica-route-contract.md`; `docs/migration-status.md` |
