# Checkpoint: Gestor Auth Context Switch Unit Runtime Contract

Data: 2026-03-24
Escopo: caracterizacao runtime conservadora de POST /gestor/auth/switch-unit
Suite focal: tests/gestor-auth-context-switch-unit-runtime-contract.test.js
Execucao focal: node --test .\tests\gestor-auth-context-switch-unit-runtime-contract.test.js
...
- Worktree ja estava sujo apenas com artefatos locais novos de Funcoes e dos microcortes anteriores de auth context; nada foi limpo
...
- A rota canonica desta subfase e `POST /gestor/auth/switch-unit`
- O owner focal e `switchAuthUnit`, que delega para `mutateAuthUnitContext` com `requirePendingSelection: false`
- Nenhum endpoint fora de `POST /gestor/auth/switch-unit` foi aberto nesta rodada

## Contrato observado

### Sem sessao no app real

- `POST /gestor/auth/switch-unit` no app real responde `401` JSON
...
- Quando o resolvedor esta desligado e o contexto volta como `legacy`, `switchAuthUnit` nao executa a troca
- O owner responde `409` com o payload integral do auth context legacy e o codigo:

```json
"GESTOR_AUTH_CONTEXT_SWITCH_DISABLED"
```

### Auth-context-v1 global sem memberships

- Diferente de `select-unit`, o caminho de `switch-unit` nao bloqueia global role no gate de `source`
- Se o contexto for `auth-context-v1`, mas sem memberships ativas, o fluxo segue para a verificacao da unidade solicitada
- Nessa condicao, como nao existe membership correspondente, o runtime observado responde `403`
...
- O payload observado preserva o auth context atual

### Sucesso

- Quando `unidade_id` corresponde a uma membership ativa, `switchAuthUnit` permite a troca mesmo sem selecao pendente
- O owner:
  - persiste a nova selecao em `req.session.gestorAuthContext`
...
- Excecao interna em `switchAuthUnit` cai no catch do owner e responde `500`
...
- Diferente de `select-unit`, `switch-unit` nao exige `needsUnitSelection: true`
- O owner sempre exige `unidade_id` valido antes de resolver o contexto efetivo
- `switch-unit` so bloqueia em `409` quando o `source` nao e `auth-context-v1`; se o source for v1 e nao houver memberships compativeis, a falha observada e `403`
- A autorizacao da troca continua sendo puramente por pertinencia de `unidade_id` ao conjunto de memberships resolvidas; nao ha uso de `requireUnitScope` nesta rota
...
- `unidade_id` invalido
- resolvedor desligado
- contexto v1 global sem memberships
- unidade fora das memberships ativas
- sucesso com troca de unidade mesmo sem selecao pendente
- erro interno induzido

## Decisao final

- O microcorte seguro `POST /gestor/auth/switch-unit` ficou congelado com caracterizacao focal verde
- Nenhum outro endpoint fora de `POST /gestor/auth/switch-unit` foi aberto nesta rodada