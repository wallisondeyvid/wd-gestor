// Migracao: alinhar indice de email de funcionarios ao contrato por unidade.
// Cria primeiro o indice composto unico por unidade_id + email.
// Remove depois o indice global legado de email, se existir.
...
const TARGET_INDEX = {
  name: 'unidade_id_1_email_1',
  key: { unidade_id: 1, email: 1 },
  options: { unique: true, name: 'unidade_id_1_email_1' },
};

const LEGACY_EMAIL_KEY = { email: 1 };
...
function isLegacyGlobalEmailIndex(index) {
  return sameKey(index?.key, LEGACY_EMAIL_KEY) && index?.unique === true;
}

async function migrarIndiceEmailFuncionariosPorUnidade() {
  console.log('Iniciando migracao do indice de email de funcionarios por unidade...');
  ...
  const legacyIndexes = indexesAfterTarget.filter(isLegacyGlobalEmailIndex);

  if (!legacyIndexes.length) {
    console.log(`Nenhum indice global legado de email encontrado em ${COLLECTION}.`);
  }
...
if (import.meta.url === `file://${process.argv[1]}`) {
  ...
  (async () => {
    try {
      ...
      await migrarIndiceEmailFuncionariosPorUnidade();
      await mongoose.disconnect();
      console.log('Finalizado');
      process.exit(0);
    } catch (error) {
      console.error('Erro na migracao do indice de email de funcionarios:', error);
...
export default migrarIndiceEmailFuncionariosPorUnidade;
